...
if [[ -z ${BSH_NO_PACKAGES-} ]]; then
	...
	windows)
		if [[ $BSH_BUILD_PLATFORM-$BSH_HOST_LIBC == windows-mingw ]]; then
			...
			pacman -S --noconfirm --needed mingw-w64-"$variant"-gcc
			if [[ $BSH_STATIC_DYNAMIC == static ]]; then
				pacman -S --noconfirm --needed mingw-w64-"$variant"-{cmake,7zip,jq} patch
			else
				pacman -S --noconfirm --needed mingw-w64-"$variant"-{pkgconf,bzip2,luajit,jsoncpp,curl,SDL2,libpng,meson,fftw,jq}
			fi
