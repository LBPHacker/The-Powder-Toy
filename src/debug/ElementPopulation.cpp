...
void ElementPopulationDebug::Draw()
{
	...
	g->BlendFilledRect(
		RectSized(
			Vec2{ xStart - 5, yBottom - 263 }, Vec2{ bars + 10 + Graphics::TextSize(maxValString).X + 9, 255 + 13 }
		),
		0x000000_rgb .WithAlpha(180)
	);
	...
	for (int i = 0; i < PT_NUM; i++)
	{
		if (elements[i].Enabled)
		{
			...
			if (sim->elementCount[i])
			{
				...
				{
					g->BlendLine(
						{ xStart + barX, yBottom - barSize - 3 },
						{ xStart + barX, yBottom - barSize - 2 },
