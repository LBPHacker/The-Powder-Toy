...
{
	size_t i = 0;
	const String::value_type *rawWord = word.c_str();
	// Function
...
parseNumberHex:
	for (i++; i < word.length(); i++)
	{
		if (!((rawWord[i] >= '0' && rawWord[i] <= '9') || (rawWord[i] >= 'a' && rawWord[i] <= 'f') ||
		      (rawWord[i] >= 'A' && rawWord[i] <= 'F')))
		{
			goto parseString;
		}
	}
...
{
	const String::value_type *stringData = str.c_str();
	char cc;
	int base = 10;
	int currentNumber = 0;
	if (stringData[0] == '#')
...
AnyType CommandInterface::tptS_bubble(std::deque<String> *words)
{
	...
	for (int i = 1; i <= 30; i++)
	{
		rem2 = sim->create_part(
			-1, int(bubblePos.X + 18 * cosf(i / 5.0) + 0.5f), int(bubblePos.Y + 18 * sinf(i / 5.0) + 0.5f), PT_SOAP
		);
