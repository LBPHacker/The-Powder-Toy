...
#define LFUNC(v) { #v, v }
		LFUNC(showWindow),      LFUNC(closeWindow),   LFUNC(addComponent),       LFUNC(removeComponent),
		LFUNC(grabTextInput),   LFUNC(dropTextInput), LFUNC(textInputRect),      LFUNC(beginInput),
		LFUNC(beginMessageBox), LFUNC(beginConfirm),  LFUNC(beginThrowError),    LFUNC(activeMenu),
		LFUNC(menuEnabled),     LFUNC(numMenus),      LFUNC(perfectCircleBrush), LFUNC(console),
		LFUNC(windowSize),      LFUNC(brushID),       LFUNC(brushRadius),        LFUNC(mousePosition),
		LFUNC(activeTool),
#undef LFUNC
		{ NULL, NULL }
	};
