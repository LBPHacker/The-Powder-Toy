...
static int edgePressure(lua_State *L)
{
	...
	lsi->game.SetEdgePressure(edgePressure);
	return 0;
}
...
static int edgeVelocity(lua_State *L)
{
	...
	lsi->sim->air->edgeVelocityX = edgeVelocityX;
	lsi->sim->air->edgeVelocityY = edgeVelocityY;
	return 0;
}
...
static int convectionMode(lua_State *L)
{
	...
	if (acount == 0)
	{
		lua_pushnumber(L, lsi->sim->air->convectionMode);
		return 1;
	}
	lsi->AssertInterfaceEvent();
	...
	lsi->sim->air->convectionMode = convMode;
	return 0;
}
