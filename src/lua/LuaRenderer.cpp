...
static int hud(lua_State *L)
{
	...
	if (acount == 0)
	{
		lua_pushboolean(L, lsi->game.GetHud());
		return 1;
	}
	auto hudstate = lua_toboolean(L, 1);
	lsi->game.SetHud(hudstate);
	return 0;
}
...
static int debugHud(lua_State *L)
{
	...
	if (acount == 0)
	{
		lua_pushboolean(L, lsi->game.GetDebugHud());
		return 1;
	}
	auto debug = lua_toboolean(L, 1);
	lsi->game.SetDebugHud(debug);
	return 0;
}
...
static int showBrush(lua_State *L)
{
	...
	if (acount == 0)
	{
		lua_pushnumber(L, int(lsi->game.GetDrawBrush()));
		return 1;
	}
	int brush = luaL_optint(L, 1, -1);
	lsi->game.SetDrawBrush(bool(brush));
	return 0;
}
...
static int separateThread(lua_State *L)
{
	...
	if (lua_gettop(L))
	{
		lsi->game.SetThreadedRendering(lua_toboolean(L, 1));
		return 0;
	}
	lua_pushboolean(L, lsi->game.GetThreadedRendering());
	return 1;
}
