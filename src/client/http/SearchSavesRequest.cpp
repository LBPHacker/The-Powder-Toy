...
static ByteString Url(int start, int count, ByteString query, Period period, Sort sort, Category category)
{
	...
	if (period)
	{
		...
		afterQuery << "after:" << currentTimeData.tm_year + 1900 << "-" << (currentTimeData.tm_mon < 9 ? "0" : "")
				   << currentTimeData.tm_mon + 1 << "-" << (currentTimeData.tm_mday < 10 ? "0" : "")
...
SearchSavesRequest::SearchSavesRequest(
	int start, int count, ByteString query, Period period, Sort sort, Category category
) :
	APIRequest(Url(start, count, query, period, sort, category), authUse, false)
{
}
