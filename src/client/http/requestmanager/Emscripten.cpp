...
void RequestManagerImpl::RegisterRequestHandle(std::shared_ptr<RequestHandle> requestHandle)
{
	...
	if (std::holds_alternative<http::FormData>(postData) && std::get<http::FormData>(postData).size())
	{
		...
		EM_ASM({ Module.emscriptenRequestManager.requests[$0].fetchBody = new FormData(); }, handle->id);
				...
				EM_ASM(
					{
						Module.emscriptenRequestManager.requests[$0].fetchBody.append(
							UTF8ToString($1), new Blob([HEAP8.slice($2, $2 + $3)]), UTF8ToString($4)
						);
					},
					handle->id,
					field.name.c_str(),
	...
	else if (std::holds_alternative<http::StringData>(postData) && std::get<http::StringData>(postData).size())
	{
		...
		EM_ASM(
			{
				Module.emscriptenRequestManager.requests[$0].fetchBody =
					new DataView(HEAP8.buffer, HEAP8.byteOffset + $1, $2);
			},
			handle->id,
			stringData.data(),
			stringData.size()
	...
	if (handle->isPost)
	{
		EM_ASM({ Module.emscriptenRequestManager.requests[$0].fetchMethod = 'POST'; }, handle->id);
	}
	else
	{
		EM_ASM({ Module.emscriptenRequestManager.requests[$0].fetchMethod = 'GET'; }, handle->id);
	}
...
{
	request.responseHeaders.push({ name : name, value : value });
}
...
{
	assert(handle->id >= 0);
	handle->bytesTotal = EM_ASM_INT({ return Module.emscriptenRequestManager.requests[$0].bytesTotal; }, handle->id);
	handle->bytesDone = EM_ASM_INT({ return Module.emscriptenRequestManager.requests[$0].bytesDone; }, handle->id);
	handle->statusCode = EM_ASM_INT({ return Module.emscriptenRequestManager.requests[$0].status; }, handle->id);
	if (!handle->gotResponse && handle->statusCode)
	{
		...
		{
			handle->responseHeaders[i].name =
				ByteString(std::unique_ptr<char, decltype(&free)>(
							   (char *)EM_ASM_PTR(
								   {
									   return stringToNewUTF8(
										   Module.emscriptenRequestManager.requests[$0].responseHeaders[$1].name
									   );
								   },
								   handle->id,
								   i
			                   ...
			                   .get());
			handle->responseHeaders[i].value =
				ByteString(std::unique_ptr<char, decltype(&free)>(
							   (char *)EM_ASM_PTR(
								   {
									   return stringToNewUTF8(
										   Module.emscriptenRequestManager.requests[$0].responseHeaders[$1].value
									   );
								   },
								   handle->id,
								   i
