...
RequestManagerImpl::RequestManagerImpl(
	ByteString newProxy, ByteString newCafile, ByteString newCapath, bool newDisableNetwork
) :
	...
	while (auto msg = curl_multi_info_read(manager->curlMulti, &dontcare))
	{
		if (msg->msg == CURLMSG_DONE)
		{
			...
			case CURLE_COULDNT_RESOLVE_HOST:
				handle->statusCode = 602;
				break;
			case CURLE_OPERATION_TIMEDOUT:
				handle->statusCode = 605;
				break;
			case CURLE_URL_MALFORMAT:
				handle->statusCode = 606;
				break;
			case CURLE_COULDNT_CONNECT:
				handle->statusCode = 607;
				break;
			case CURLE_COULDNT_RESOLVE_PROXY:
				handle->statusCode = 608;
				break;
			case CURLE_TOO_MANY_REDIRECTS:
				handle->statusCode = 611;
				break;
			case CURLE_SSL_CONNECT_ERROR:
				handle->statusCode = 612;
				break;
			case CURLE_SSL_ENGINE_NOTFOUND:
				handle->statusCode = 613;
				break;
			case CURLE_SSL_ENGINE_SETFAILED:
				handle->statusCode = 614;
				break;
			case CURLE_SSL_CERTPROBLEM:
				handle->statusCode = 615;
				break;
			case CURLE_SSL_CIPHER:
				handle->statusCode = 616;
				break;
			case CURLE_SSL_ENGINE_INITFAILED:
				handle->statusCode = 617;
				break;
			case CURLE_SSL_CACERT_BADFILE:
				handle->statusCode = 618;
				break;
			case CURLE_SSL_CRL_BADFILE:
				handle->statusCode = 619;
				break;
			case CURLE_SSL_ISSUER_ERROR:
				handle->statusCode = 620;
				break;
			case CURLE_SSL_PINNEDPUBKEYNOTMATCH:
				handle->statusCode = 621;
				break;
			case CURLE_SSL_INVALIDCERTSTATUS:
				handle->statusCode = 609;
				break;
			case CURLE_HTTP2:
	...
	{
		{
			...
			// Actually unregister handles queued to be unregistered. They can be queued just above, or from another
			// thread. Thus, it's ok for them to be in the queue multiple times, but it's not ok to try to unregister
			// them multiple times.
		...
		}
		{
			...
			if (std::holds_alternative<http::FormData>(postData) && std::get<http::FormData>(postData).size())
			{
				...
				for (auto &field : formData)
				{
					if (field.filename.has_value())
					{
						HandleCURLFORMcode(curl_formadd(
							&handle->curlPostFieldsFirst,
							&handle->curlPostFieldsLast,
							CURLFORM_COPYNAME,
							field.name.c_str(),
							CURLFORM_BUFFER,
							field.filename->c_str(),
							CURLFORM_BUFFERPTR,
							field.value.data(),
			...
			else if (std::holds_alternative<http::StringData>(postData) && std::get<http::StringData>(postData).size())
			{
				...
				HandleCURLcode(
					curl_easy_setopt(handle->curlEasy, CURLOPT_POSTFIELDSIZE_LARGE, curl_off_t(stringData.size()))
				);
			}
			...
			else
			{
				...
				HandleCURLcode(
					curl_easy_setopt(handle->curlEasy, CURLOPT_REDIR_PROTOCOLS, CURLPROTO_HTTPS | CURLPROTO_HTTP)
				);
#endif
			}
			...
			HandleCURLcode(
				curl_easy_setopt(handle->curlEasy, CURLOPT_HEADERFUNCTION, &RequestHandleHttp::HeaderDataHandler)
			);
			HandleCURLcode(curl_easy_setopt(handle->curlEasy, CURLOPT_WRITEDATA, (void *)handle));
			HandleCURLcode(
				curl_easy_setopt(handle->curlEasy, CURLOPT_WRITEFUNCTION, &RequestHandleHttp::WriteDataHandler)
			);
		}
	}
...
RequestManagerPtr
	RequestManager::Create(ByteString newProxy, ByteString newCafile, ByteString newCapath, bool newDisableNetwork)
{
...
void SetupCurlEasyCiphers(CURL *easy)
{
	if constexpr (SECURE_CIPHERS_ONLY)
	{
		...
		{
			HandleCURLcode(curl_easy_setopt(
				easy,
