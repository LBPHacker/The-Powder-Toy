...
namespace game
{
	...
	public:
		...
		void Tick() final override
		{
			if (Game::Ref().CurrentCategory() == index)
			{
				...
				Position(gui::Point{ panelPosition, 0 });
			}
			else
			{
				Visible(false);
		...
		void DrawAfterChildren() const final override
		{
			...
			g.DrawRect(gui::Rect { abs + gui::Point{ thumbPosition, 20 }, gui::Point{ thumbSize, 2 } }, gui::Appearance::colors.inactive.idle.border);
			auto tl = abs;
	...
	{
		ClipChildren(true);
		Position(gui::Point{ 1, simulationSize.y + 1 });
		Size(gui::Point{ simulationSize.x - 1, panelSize.y });
		...
		for (auto i = 0U; i < SC_TOTAL; ++i)
		{
			...
			for (auto j = sections[i].size(); j > 0U; --j)
			{
				...
				button->ToolTipPosition(gui::Point{ simulationSize.x - gui::SDLWindow::Ref().TextSize(tool->Description()).x - 9, simulationSize.y - 12 });
				button->Position(gui::Point{ width - int(buttonShade.size()) + 1, 0 });
			...
			panel->Size(gui::Point{ width, panelSize.y });
		}
	}
}
}
