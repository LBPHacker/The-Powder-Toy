...
class WindowsClipboardImpl : public ClipboardImpl
{
	...
	void Transfer()
	{
		...
		auto handle =
			std::unique_ptr<void, decltype(&::GlobalFree)>(::GlobalAlloc(GMEM_MOVEABLE, saveData.size()), GlobalFree);
		if (!handle)
		{
	...
	int TransferWatch(SDL_Event *event)
	{
		...
		case SDL_SYSWMEVENT:
			...
			case WM_RENDERALLFORMATS:
			{
				...
				{
					std::cerr << "cannot place save on clipboard: OpenClipboard failed: " << ::GetLastError()
							  << std::endl;
				}
			}
			break;
...
public:
	WindowsClipboardImpl()
	{
		...
		if (!saveClipboardFormat)
		{
			std::cerr << "cannot register save clipboard format: RegisterClipboardFormatW failed: " << ::GetLastError()
					  << std::endl;
			return;
		}
