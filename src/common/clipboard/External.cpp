...
static std::optional<Preset> GetPreset()
{
	...
	if (name == "auto")
	{
		...
		if (!name)
		{
			std::cerr << "no built-in external clipboard command preset for SDL window subsystem " << currentSubsystem
					  << std::endl;
			return std::nullopt;
		}
	}
...
public:
	...
	void SetClipboardData() final override
	{
		...
		if (status)
		{
			std::cerr << "failed to set clipboard data: " << preset->inCommand << ": wait4 status code " << status
					  << std::endl;
			return;
		}
	}
	...
	GetClipboardDataResult GetClipboardData() final override
	{
		...
		auto getTarget = [](ByteString command) -> std::optional<std::vector<char>> {
			...
			if (status)
			{
				std::cerr << "cannot get save from clipboard: " << command << ": wait4 status code " << status
						  << std::endl;
				return std::nullopt;
			}
			return data;
