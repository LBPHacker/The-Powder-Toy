		...
		case 4:
			{
				...
				if (top.nonconformance && top.nonconformance->IsObject())
				{
					nonconformance = top.nonconformance->Get(elementName);
				}
				if (nonconformance && nonconformance->IsArray() && (*nonconformance)[0].AsString() == "objectEncodedAsArray")
				{
					elementType = 3;
				}
				auto &document = push(elementType == 3 ? objectValue : arrayValue);
		...
		case 5:
			{
				...
				auto &user = push(User(size)).AsUser();
...
std::vector<char> Bson::Dump(const Bson *rootNonconformance) const
{
	...
	std::stack<StackEntry> stack;
	{
		...
		stack.push({ this, rootNonconformance, sizeAt, 0, AsObject().begin() });
	}
	while (!stack.empty())
	{
		auto &top = stack.top();
		...
		if (nonconformance && nonconformance->IsArray() && (*nonconformance)[0].AsString() == "arrayEncodedWithBadKeys")
		{
			elementName = (*nonconformance)[1].AsString();
		}
		...
		default:
			{
				elementType = childType == objectValue ? 3 : 4;
				if (nonconformance && nonconformance->IsArray() && (*nonconformance)[0].AsString() == "objectEncodedAsArray")
				{
					elementType = 4;
				}
				auto sizeAt = writer.GetHave();
				writer.Write(int32_t(0));
				auto *childNonconformance = (nonconformance && nonconformance->IsObject()) ? nonconformance->Get(elementName) : nullptr;
				auto childIterator = child->IsObject() ? child->AsObject().begin() : Object::const_iterator{};
		...
		case doubleValue: elementType =  1; writer.Write(        child->AsDouble() ); break;
		case int32Value : elementType = 16; writer.Write(int32_t(child->AsInt32 ())); break;
		case int64Value : elementType = 18; writer.Write(int64_t(child->AsInt64 ())); break;
		case boolValue  : elementType =  8; writer.Write(uint8_t(child->AsBool  ())); break;
		}
