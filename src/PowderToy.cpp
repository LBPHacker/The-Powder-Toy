...
int Main(int argc, char *argv[])
{
	...
	auto resetsimconfigArg = arguments["resetsimconfig"];
	if (resetsimconfigArg.has_value())
	{
		configResetMessage = "Simulation configuration reset upon request, sane defaults loaded";
		auto config = SimulationConfig::Default();
	...
	else
	{
		...
		config.NPART        = prefs.Get("Simulation.PartCount"    , defaultConfig.NPART       );
		config.MAX_TEMP     = prefs.Get("Simulation.MaxTemp"      , defaultConfig.MAX_TEMP    );
		config.MIN_TEMP     = prefs.Get("Simulation.MinTemp"      , defaultConfig.MIN_TEMP    );
		config.MAX_PRESSURE = prefs.Get("Simulation.MaxPressure"  , defaultConfig.MAX_PRESSURE);
		config.MIN_PRESSURE = prefs.Get("Simulation.MinPressure"  , defaultConfig.MIN_PRESSURE);
