...
namespace Powder
{
	namespace
	{
		...
		void HandleWindowParameters(Gui::Host &host, Prefs &prefs, Arguments &arguments)
		{
			...
			if (auto scaleArg = arguments["scale"])
			{
				try
				{
					windowParameters.fixedScale = scaleArg.value().ToNumber<int>();
					setPrefs = true;
				}
	...
	void Stacks::SetHostStack(Gui::Stack *stack)
	{
	...
	void Main(std::span<const ByteString> args)
	{
		...
		auto browserStack = std::make_shared<Gui::Stack>(host);
		auto game = std::make_shared<Activity::Game>(host);
		gameStack->Push(game);
		auto onlineBrowser = std::make_shared<Activity::OnlineBrowser>(host);
		browserStack->Push(onlineBrowser);
		onlineBrowser->SetThreadPool(globals->threadPool.get());
		onlineBrowser->SetGameStack(gameStack.get());
		onlineBrowser->SetStacks(globals->stacks.get());
		game->SetOnlineBrowserStack(browserStack.get());
		...
		game->SetThreadPool(globals->threadPool.get());
		globals->stacks->AddStack(gameStack);
		globals->stacks->AddStack(browserStack);
		globals->stacks->SetHostStack(gameStack.get());
		host.Start();
