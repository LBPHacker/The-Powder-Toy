...
namespace Powder::Gui
{
	...
	Number MomentumAnimation<Number>::GetValue() const
	{
		...
		{
			auto distance = Momentum::referenceValue - Momentum::target;
			auto maxDiffTick = BiasedTicks(Number(std::log(exponentialProfile->margin / Number(std::abs(distance))) / Number(std::log(exponentialProfile->decay)) * tickBias));
			auto momentum = Number(0);
			if (diffTick >= maxDiffTick)
			{
				if (Momentum::referenceValue != Momentum::target)
				{
					momentum = Number(std::copysign(exponentialProfile->margin, distance));
				}
			}
			else
			{
			...
			return referenceValue + (momentum - distance) / Number(std::log(exponentialProfile->decay));
		}
		return Number(0);
	}
