...
namespace Powder::Gui
{
	...
	View::Component &View::GetOrAllocComponent(ComponentKey key)
	{
		...
		component.rect.pos = component.unscrolledPos;
		if (!componentStack.empty())
		{
			auto &item = componentStack.back();
			auto &parentComponent = componentStore[item.index];
			for (AxisBase psAxis = 0; psAxis < 2; ++psAxis)
			{
				auto xyAxis = AxisBase(parentComponent.layout.primaryAxis) ^ psAxis;
				component.ownScroll % xyAxis = std::clamp(parentComponent.scroll % psAxis, -(parentComponent.overflow % psAxis), 0) + parentComponent.ownScroll % xyAxis;
				component.rect.pos % xyAxis += component.ownScroll % xyAxis;
			}
		}
