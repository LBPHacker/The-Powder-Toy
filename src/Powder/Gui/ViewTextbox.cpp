...
namespace Powder::Gui
{
	...
	)
	{
		...
		if (HasInputFocus())
		{
			...
			updateSt(); // just in case before the first iteration; TODO-REDO_UI: figure out when this should matter
			while (!inputFocus->events.empty())
			{
				...
				auto replace = [&](TextWrapper::ClearIndex begin, TextWrapper::ClearIndex end, std::string_view istr) {
					...
					if (validLimit)
					{
						...
						if (int32_t(istr.size()) > maxIstrSize)
						{
							istr = istr.substr(0, maxIstrSize);
						}
					}
