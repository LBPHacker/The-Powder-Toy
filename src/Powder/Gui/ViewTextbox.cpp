...
namespace Powder::Gui
{
	...
	)
	{
		...
		if (HasInputFocus())
		{
			// TODO-REDO_UI-TEXTBOX: scrolling
			// TODO-REDO_UI-TEXTBOX: word selection with double click
			// TODO-REDO_UI-TEXTBOX: paragraph selection with triple click
			...
			while (!inputFocus->events.empty())
			{
				...
				if (keyDown && !keyDown->alt)
				{
					...
					case SDLK_HOME:
						...
						{
							Log("nyi"); // TODO-REDO_UI-TEXTBOX: line edges (visually, so not just \n)
							break;
						}
						adjustSelection(keyDown->shift);
						break;
