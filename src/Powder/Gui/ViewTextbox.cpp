...
namespace Powder::Gui
{
	...
	{
		BeginTextboxInternal(key, str, std::nullopt, textboxFlags, color);
	}

	void View::BeginTextbox(ComponentKey key, std::string &str, StringView placeholderText, TextboxFlags textboxFlags, Rgba8 color)
	{
		BeginTextboxInternal(key, str, placeholderText, textboxFlags, color);
	}

	void View::BeginTextboxInternal(ComponentKey key, std::string &str, std::optional<StringView> placeholderText, TextboxFlags textboxFlags, Rgba8 color)
	{
		BeginComponent(key);
		...
		if (multiline)
		{
			...
			SetMaxSizeSecondary(maxSize % (AxisBase(parentComponent.prevLayout.primaryAxis) ^ 1));
		...
		BeginTextInternal("text", str, std::nullopt, placeholderText, textFlags, color);
		SetEnabled(enabled);
		auto &component = componentStore[(componentStack.end() - parentDepth)->index];
