	...
	{
		{
			...
			if ((exitEvent == ExitEventType::exit ||
			    (exitEvent != ExitEventType::none && canApplyKind == CanApplyKind::onlyCancel)) && Cancel())
			{
				return true;
			}
			if ((exitEvent == ExitEventType::ok   && canApplyKind == CanApplyKind::yes      ) ||
			    (exitEvent != ExitEventType::none && canApplyKind == CanApplyKind::onlyApply))
			{
				Apply();
				return true;
			}
...
# endif
	...
	View::CanApplyKind View::CanApply() const
	{
