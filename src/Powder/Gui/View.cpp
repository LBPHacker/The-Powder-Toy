...
# endif
	...
	void View::UpdateUnderMouse(Component &component)
	{
		if (component.rect.Contains(*mousePos)) // TODO-REDO_UI: clip to parent
		{
			...
			{
				handleWheelIndex = GetComponentIndex(component);
			}
...
# endif
			...
			component.contentSpace % axis = space;
			component.contentSize % axis = spaceUsed;
	...
	void View::EndScrollpanel()
	{
		...
		for (AxisBase axis = 0; axis < 2; ++axis)
		{
			auto effectiveAxis = AxisBase(component.prevLayout.primaryAxis) ^ axis;
			auto &dimension = context.dimension % effectiveAxis;
			auto show = (dimension.visibility == ScrollpanelDimension::Visibility::always) ||
		                (dimension.visibility == ScrollpanelDimension::Visibility::ifUseful && (component.overflow % effectiveAxis));
			auto &side = showSide % axis;
			side.show = show;
			side.size = r.size % axis;
		}
		{
		...
		for (AxisBase axis = 0; axis < 2; ++axis)
		{
			...
			{
				auto effectiveAxis = AxisBase(component.prevLayout.primaryAxis) ^ axis;
				auto contentSpace = component.contentSpace      % effectiveAxis;
				auto contentSize  = component.contentSize       % effectiveAxis;
				auto overflow     = component.overflow          % effectiveAxis;
				auto scroll       = component.prevLayout.scroll % effectiveAxis;
				Rect trackRect{ r.TopLeft(), { thickness, thickness } };
				trackRect.pos  % axis += side.pos;
				trackRect.size % axis =  side.size;
				if (side.reverseSide)
				{
					component.layout.paddingBefore % (effectiveAxis ^ 1) += thickness;
				}
				else
				{
					trackRect.pos % (axis ^ 1) += r.size % (axis ^ 1) - thickness;
					component.layout.paddingAfter % (effectiveAxis ^ 1) += thickness;
				}
				auto &g = *GetHost();
				g.FillRect(trackRect, scrollbarTrackColor);
				Rect thumbRect = trackRect;
				auto &thumbLength = thumbRect.size % axis;
				auto &trackLength = trackRect.size % axis;
				thumbLength = std::min(trackLength, std::max(minLength, contentSpace * trackLength / contentSize));
				thumbRect.pos % axis = (trackRect.pos % axis) + scroll * (trackLength - thumbLength) / -overflow;
				g.FillRect(thumbRect, scrollbarThumbColor);
				auto &stickyScroll = context.stickyScroll % axis;
				if (IsMouseDown(SDL_BUTTON_LEFT))
				{
					...
					if (context.mouseDownPos)
					{
						...
						{
							auto wantThumbPos = *m - *context.mouseDownPos;
							stickyScroll.SetMomentum(0.f);
							stickyScroll.SetValue(float(((wantThumbPos % axis) - (trackRect.pos % axis)) * -overflow / (trackLength - thumbLength)));
						}
					}
					else
					{
						if (auto m = GetMousePos())
						{
							context.mouseDownPos = *m - thumbRect.TopLeft();
						}
					}
				}
				if (scrollDistance)
				{
					auto distance = ((*scrollDistance) % axis);
				...
				component.layout.scroll % effectiveAxis = Pos(stickyScroll.GetValue());
			}
		}
	...
	void View::BeginCheckbox(ComponentKey key, bool &checked, StringView text, CheckboxFlags checkboxFlags, Rgba8 color)
	{
		constexpr Size boxSize    = 12;
		constexpr Size boxSpacing = 3;
		auto &g = *GetHost();
		auto edgeColor = 0xFFFFFFFF_argb;
		BeginComponent(key);
		SetHandleButtons(true);
		...
		auto r = GetRect();
		SetPadding(boxSize + boxSpacing, 0, 0, 0);
		Rect rr{ r.TopLeft() + Pos2{ 0, (r.size.Y - boxSize) / 2 }, { boxSize, boxSize } };
	...
	{
		BeginCheckbox(text, checked, text, checkboxFlags);
		SetSize(size);
		return EndCheckbox();
	}
	...
	bool View::EndDialog(bool allowOk)
	{
		...
		BeginHPanel("dialogend");
		{
			...
			if (Button("\boOK", SpanAll{}, allowOk ? ButtonFlags::none : ButtonFlags::disabled)) // TODO-REDO_UI-TRANSLATE
			{
				if (allowOk)
				{
					tryOk = true;
