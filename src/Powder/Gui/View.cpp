	...
	{
		{
			...
			if (ClassifyExitEvent(event) == ExitEventType::exit && Cancel())
			{
				return true;
			}
			if (ClassifyExitEvent(event) == ExitEventType::ok)
			{
				auto canApplyKind = CanApply();
				if (canApplyKind == CanApplyKind::yes || canApplyKind == CanApplyKind::onlyApply)
				{
					Apply();
					return true;
				}
