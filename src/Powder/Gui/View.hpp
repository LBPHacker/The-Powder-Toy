...
namespace Powder::Gui
{
	...
	private:
		...
		struct SliderContext
		{
			...
			std::optional<Pos2> mouseDownPos;
			std::optional<Pos2> thumbPosAtMouseDown;
		...
		struct Component : public CrossFrameItem
		{
			...
			struct Layout
			{
				...
				bool layered = false;
				bool immaterial = false; // TODO-REDO_UI: make the forest multi-rooted
	...
	public:
		...
		virtual CanApplyKind CanApply() const; // No need to call this from overriding functions.
		virtual void Apply(); // No need to call this from overriding functions.
		...
		void BeginModal(ComponentKey key, Rect rootRect);
		void EndModal();

		void BeginDialog(ComponentKey key, StringView title, Rect rootRect, std::optional<Size> size = std::nullopt);
		void EndDialog();

		struct CommondialogStateProgress
		{
		};
		struct CommondialogStateFailed
		{
			StringView reason;
		};
		using CommondialogState = std::variant<
			CommondialogStateProgress,
			CommondialogStateFailed
		>;
		void BeginCommondialog(ComponentKey key, StringView title, CommondialogState state);
		void EndCommondialog();
		void Commondialog(ComponentKey key, StringView title, CommondialogState state);

		void BeginText(ComponentKey key, StringView text, TextFlags textFlags, Rgba8 color = 0xFFFFFFFF_argb);
		void BeginText(ComponentKey key, StringView text, StringView overflowText, TextFlags textFlags, Rgba8 color = 0xFFFFFFFF_argb);
		void EndText();
		...
		void BeginButton(ComponentKey key, StringView text, ButtonFlags buttonFlags, Rgba8 color = 0xFFFFFFFF_argb, Rgba8 backgroundColor = 0x00000000_argb);
		bool EndButton();
		...
		void BeginTextbox(ComponentKey key, std::string &str, StringView placeholderText, TextboxFlags textboxFlags, Rgba8 color = 0xFFFFFFFF_argb);
		bool EndTextbox();
		...
		void BeginTextSeparator(ComponentKey key, StringView text, Size size, Rgba8 color);
		...
		void BeginSlider(ComponentKey key);
		bool EndSlider(Pos &value, Pos minValue, Pos maxValue, Size thumbSize, bool maxInclusive = true);
		bool EndSlider(Pos2 &value, Pos2 minValue, Pos2 maxValue, Size thumbSize, ExtendedSize2<bool> maxInclusive = { true, true });
