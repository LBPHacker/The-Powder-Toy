...
namespace Powder::Gui
{
	...
	private:
		...
		struct ScrollpanelContext
		{
			...
			ExtendedSize2<MomentumAnimation<float>> stickyScroll{
				{ MomentumAnimation<float>::ExponentialProfile{ 0.297553f, 30.f } }, // 0.297553f = std::pow(0.98f, 60.f)
				{ MomentumAnimation<float>::ExponentialProfile{ 0.297553f, 30.f } },
			};
			std::optional<Pos2> mouseDownPos;
		};
	...
	public:
		...
		void BeginCheckbox(ComponentKey key, bool &checked, StringView text, CheckboxFlags checkboxFlags, Rgba8 color = 0xFFFFFFFF_argb);
		bool EndCheckbox();
		...
		void Separator(ComponentKey key, int32_t size = 1);
	};
