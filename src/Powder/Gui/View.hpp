...
namespace Powder::Gui
{
	...
	private:
		...
		{
			std::vector<InternedTextIndex> lastItems;
			std::vector<InternedTextIndex> items;
		...
		Context &SetContext(Component &component)
		{
			...
			{
				component.context = std::make_unique<ComponentContext>(std::in_place_type_t<Context>{});
			}
		...
		bool shouldDoLayout = false;
		void UpdateShouldDoLayout(Component &component);
		void DoLayout();
		void DoLayoutContentSize(Axis parentPrimaryAxis, Component &component);
		void DoLayoutFillSpace(Component &component);
		...
		struct ComponentMouseEvent
		{
			ComponentIndex component;
			MouseButtonIndex button;
		};
		std::optional<ComponentMouseEvent> componentMouseDownEvent;
		std::optional<ComponentMouseEvent> componentMouseClickEvent;
	...
	public:
		...
		Host *GetHost() const; // TODO-REDO_UI: decide if this makes sense as opposed to Host::Ref()
		...
		void BeginText(ComponentKey key, StringView text, TextFlags textFlags, Rgba8 color = 0xFFFFFFFF_rgba);
		void EndText();
		void Text(StringView text, Rgba8 color = 0xFFFFFFFF_rgba);
		void Text(ComponentKey key, StringView text, Rgba8 color = 0xFFFFFFFF_rgba);
		...
		void BeginButton(ComponentKey key, StringView text, ButtonFlags buttonFlags, Rgba8 color = 0xFFFFFFFF_rgba);
		bool EndButton();
		...
		void BeginTextbox(ComponentKey key, std::string &str, TextboxFlags textboxFlags, Rgba8 color = 0xFFFFFFFF_rgba);
		bool EndTextbox();
		...
		void Separator(ComponentKey key);
	};
