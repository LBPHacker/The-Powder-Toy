...
namespace Powder::Gui
{
	...
	public:
		...
		static CharacterClass ClassifyCharacter(UnicodeCodePoint codePoint);
	...
	private:
		...
		std::vector<OverflowRegion> overflowRegions;
	...
	public:
		...
		{
			return { rawSize, unformattedSize, ClearIndex(regions.size()) };
		}

		Point GetShapedSize() const
		{
		...
		{
			return { wrappedWidth, wrappedLines * fontTypeSize - 1 };
		}

		Point GetContentSize() const
		{
			return { wrappedWidth - letterSpacing, (wrappedLines - 1) * fontTypeSize + fontCapHeight };
		}
		...
		const std::vector<OverflowRegion> &GetOverflowRegions() const
		{
