	...
	void Host::CreateShapedTextTexture(ShapedText &shapedText)
	{
		...
		auto blend = [&](Point p, Rgba8 color) {
			if (size.OriginRect().Contains(p))
			{
				...
				{
					pixel = Rgba8(
						uint8_t(std::min(0xFF, int32_t(color.Red  ) * int32_t(color.Alpha) / 0xFF + int32_t(old.Red  ))),
						uint8_t(std::min(0xFF, int32_t(color.Green) * int32_t(color.Alpha) / 0xFF + int32_t(old.Green))),
						uint8_t(std::min(0xFF, int32_t(color.Blue ) * int32_t(color.Alpha) / 0xFF + int32_t(old.Blue ))),
						uint8_t(std::min(0xFF, int32_t(color.Alpha)                               + int32_t(old.Alpha)))
					).Pack();
				}
				else
				{
					...
					//   [  0    0   1-a  ab ]
					//   [  0    0    0   1  ]
					auto outAlpha = 0xFE01 - (0xFF - color.Alpha) * (0xFF - old.Alpha);
					auto outRed   = outAlpha ? ((0xFF - color.Alpha) * old.Alpha * old.Red   + 0xFF * color.Alpha * color.Red  ) / outAlpha : 0;
					auto outGreen = outAlpha ? ((0xFF - color.Alpha) * old.Alpha * old.Green + 0xFF * color.Alpha * color.Green) / outAlpha : 0;
					auto outBlue  = outAlpha ? ((0xFF - color.Alpha) * old.Alpha * old.Blue  + 0xFF * color.Alpha * color.Blue ) / outAlpha : 0;
					outAlpha /= 0xFF;
					pixel = Rgba8(
						uint8_t(outRed),
						uint8_t(outGreen),
		...
		for (auto it = range.begin(); it != range.end() && regionIt != regionEnd; ++it)
		{
			auto ch = *it;
			if (!ch.valid)
			{
			...
			default:
				takeRegion(ch.codePoint, regionIt);
				break;
			}
		}
		{
