...
namespace Powder::Gui
{
	...
	bool InputMapper::HandleEvent(const SDL_Event &event)
	{
		...
		case SDL_KEYDOWN:
			...
			if (BeginInput(KeyboardKeyInput{ CanonicalScancode(event.key.keysym.scancode) }) == InputDisposition::exclusive)
			{
				return true;
			}
			break;
		...
		case SDL_KEYUP:
			EndInput(KeyboardKeyInput{ CanonicalScancode(event.key.keysym.scancode) });
			break;
		}
		return false;
	}
