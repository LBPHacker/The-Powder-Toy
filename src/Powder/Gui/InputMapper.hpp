...
namespace Powder::Gui
{
	...
	public:
		...
		struct ActionGroup
		{
			...
			using DisplayIndex = int32_t;
			DisplayIndex displayIndex = -1;
			std::shared_ptr<ActionCategory> category;
		};
		struct Action
		{
			using Name = std::string;
			...
			using DisplayIndex = int32_t;
			DisplayIndex displayIndex = -1;
			std::shared_ptr<ActionGroup> group;
			std::function<InputDisposition ()> begin;
			std::function<void ()> end;
			bool active = false;
			std::vector<std::shared_ptr<Action>> modifierFor;
		};

		struct ModifierOnly
		{
		};
		using InputOrModifierOnly = std::variant<
			Input,
			ModifierOnly
		>;
		struct InputToAction
		{
			InputOrModifierOnly input;
	...
	private:
		...
		std::vector<Input> knownModifiers;
		void InitKnownModifiers();
