...
namespace Powder::Activity
{
	...
	{
		auto makeActionCategory = [&](std::string name) {
			auto group = std::make_shared<ActionCategory>();
			group->name = actionCategoryNamePrefix + name;
			group->displayIndex = int32_t(shortcutMapperInfo.groups.size());
			shortcutMapperInfo.categories.push_back(group);
			return group;
		};
		auto makeActionGroup = [&](std::string name, std::shared_ptr<ActionCategory> category) {
		...
		auto editingCategory     = makeActionCategory("EDITING");
		auto renderingCategory   = makeActionCategory("RENDERING");
		auto simGroup            = makeActionGroup("SIM"           , editingCategory);
		auto drawingGroup        = makeActionGroup("DRAWING"       , editingCategory);
		auto placezoomctxGroup   = makeActionGroup("PLACEZOOMCTX"  , editingCategory);
		auto brushGroup          = makeActionGroup("BRUSH"         , editingCategory);
		auto historyGroup        = makeActionGroup("HISTORY"       , editingCategory);
		auto stampGroup          = makeActionGroup("STAMP"         , editingCategory);
		auto clipboardGroup      = makeActionGroup("CLIPBOARD"     , editingCategory);
		auto selectctxGroup      = makeActionGroup("SELECTCTX"     , editingCategory);
		auto pastectxGroup       = makeActionGroup("PASTECTX"      , editingCategory);
		auto toolGroup           = makeActionGroup("TOOL"          , editingCategory);
		auto miscGroup           = makeActionGroup("MISC"          , editingCategory);
		auto rendererpresetGroup = makeActionGroup("RENDERERPRESET", renderingCategory);
		auto renderGroup         = makeActionGroup("RENDER"        , renderingCategory);

		auto initToggle = [&](std::string name, std::shared_ptr<ActionGroup> group, auto member, std::set<Input> modifiers, std::vector<std::shared_ptr<Action>> modifierFor) {
		...
		auto initToolsize = [&](std::string name, int32_t scancode, MouseWheelInput::Direction direction, int32_t diff) {
			...
			action->begin = [this, diff]() {
				...
				{
					AdjustZoomSize(diffX, diffY, toolSizeLogarithmic);
				}
				else
				{
					AdjustBrushSize(diffX, diffY, toolSizeLogarithmic);
				}
				return InputDisposition::exclusive;
			};
		...
		initToggle("TOOLSIZELOGARITHMIC", brushGroup, &Game::toolSizeLogarithmic, { KeyboardKeyInput{ SDL_SCANCODE_LALT } }, toolsizeActions);
		initToggle("TOOLSIZECONSTRAINX", brushGroup, &Game::toolSizeConstrainX, { KeyboardKeyInput{ SDL_SCANCODE_LCTRL } }, toolsizeActions);
		initToggle("TOOLSIZECONSTRAINY", brushGroup, &Game::toolSizeConstrainY, { KeyboardKeyInput{ SDL_SCANCODE_LSHIFT } }, toolsizeActions);

		std::vector<std::shared_ptr<Action>> drawActions;
		auto initDraw = [&](int32_t index, int32_t button) {
			auto action = makeAction(ByteString::Build("DRAW", index), drawingGroup);
			drawActions.push_back(action);
			action->begin = [this, index]() {
				if (lastHoveredTool)
				{
					SelectTool(index, lastHoveredTool);
					return InputDisposition::exclusive;
				}
				if (ClickSign(index))
				{
				...
				if (brushModeFill)
				{
					BeginBrush(index, DrawMode::fill);
				}
				else if (brushModeRect)
				{
					BeginBrush(index, DrawMode::rect);
				}
				else if (brushModeLine)
				{
					BeginBrush(index, DrawMode::line);
				}
				else
				{
					BeginBrush(index, DrawMode::free);
				}
				return InputDisposition::exclusive;
			};
			action->end = [this, index]() {
				EndBrush(index);
			};
			rootContext->inputToActions.push_back({ MouseButtonInput{ button }, {}, action });
		};
		initDraw(0, SDL_BUTTON_LEFT);
		initDraw(1, SDL_BUTTON_RIGHT);
		initDraw(2, SDL_BUTTON_MIDDLE);
		initToggle("DRAWLINE", drawingGroup, &Game::brushModeLine, { KeyboardKeyInput{ SDL_SCANCODE_LSHIFT } }, drawActions);
		initToggle("DRAWRECT", drawingGroup, &Game::brushModeRect, { KeyboardKeyInput{ SDL_SCANCODE_LCTRL } }, drawActions);
		initToggle("DRAWFILL", drawingGroup, &Game::brushModeFill, { KeyboardKeyInput{ SDL_SCANCODE_LSHIFT }, KeyboardKeyInput{ SDL_SCANCODE_LCTRL } }, drawActions);
		{
			constexpr int32_t index = 3;
			auto action = makeAction(ByteString::Build("SELECT", index), drawingGroup);
			action->begin = [this]() {
				if (lastHoveredTool)
				{
					SelectTool(index, lastHoveredTool);
			...
			rootContext->inputToActions.push_back({ MouseButtonInput{ SDL_BUTTON_LEFT }, { KeyboardKeyInput{ SDL_SCANCODE_LCTRL }, KeyboardKeyInput{ SDL_SCANCODE_LALT } }, action });
		}
