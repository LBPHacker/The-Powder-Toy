...
namespace Powder::Activity
{
	...
	struct GameToolInfo
	{
		...
		struct TextureInfo
		{
			PlaneAdapter<std::vector<pixel>> data; // TODO-REDO_UI: persist in the Tool itself
	...
	private:
		...
		struct DrawSimFrame : public RasterDrawMethods<DrawSimFrame> // TODO-REDO_UI: resize to RES.X * RES.Y
		{
			RendererFrame video;

			Rect GetClipRect() const // TODO-REDO_UI: remove
			{
		...
		void EndSelect(Pos2 first, Pos2 second, SelectMode mode, bool includePressure);
	...
	public:
		...
		void SetDebugFlags(uint32_t newDebugFlags) { debugFlags = newDebugFlags; }
		bool GetHud() const { return hud; }
		void SetHud(bool newHud) { hud = newHud; }
		bool GetWantTextInputOverride() const { return wantTextInputOverride; }
		void SetWantTextInputOverride(bool newWantTextInputOverride) { wantTextInputOverride = newWantTextInputOverride; }
		bool GetPerfectCircleBrush() const { return perfectCircleBrush; }
		void SetPerfectCircleBrush(bool newPerfectCircleBrush);
		bool GetDebugHud() const { return debugHud; }
		void SetDebugHud(bool newDebugHud) { debugHud = newDebugHud; }
		bool GetDrawBrush() const { return drawBrush; }
		void SetDrawBrush(bool newDrawBrush) { drawBrush = newDrawBrush; }
		bool GetThreadedRendering() const { return threadedRendering; }
		void SetThreadedRendering(bool newThreadedRendering) { threadedRendering = newThreadedRendering; }
		std::function<void (String)> GetVisualLogSink() const { return visualLogSink; }
		void SetVisualLogSink(std::function<void (String)> newVisualLogSink) { visualLogSink = newVisualLogSink; }
		Rgba8 GetDecoColor() const { return decoColor; }
		void SetDecoColor(Rgba8 newDecoColor) { decoColor = newDecoColor; }
		MenuSectionIndex GetActiveMenuSection() const { return activeMenuSection; }
		void SetActiveMenuSection(MenuSectionIndex newActiveMenuSection) { activeMenuSection = newActiveMenuSection; }
		...
		bool AddCustomGol(String ruleString, String nameString, Rgb8 color1, Rgb8 color2);
		...
		// TODO-REDO_UI: replace simple accessors with public members
		// TODO-REDO_UI: replace dumb *Action functions with lambdas
	};
}
