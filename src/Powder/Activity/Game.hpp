...
namespace Powder::Activity
{
	...
	private:
		...
		void DrawSim();
		void DrawBrush(void *pixels, int32_t pitch);
	...
	public:
		...
		void SetOnlineBrowserStack(Gui::Stack *newOnlineBrowserStack)
		{
			onlineBrowserStack = newOnlineBrowserStack;
		}

		void SetOnlineBrowser(OnlineBrowser *newOnlineBrowser)
		{
			onlineBrowser = newOnlineBrowser;
		}

		void SetLocalBrowserStack(Gui::Stack *newLocalBrowserStack)
		{
			localBrowserStack = newLocalBrowserStack;
		}

		void SetLocalBrowser(LocalBrowser *newLocalBrowser)
		{
			localBrowser = newLocalBrowser;
		}
		...
		bool GetLoadWithoutPressure() const
		{
			return loadWithoutPressure;
		}

		std::unique_ptr<Gui::StaticTexture> MakeToolAtlasTexture(View &view) const;
		void AddNotification(ServerNotification serverNotification);
	};
}
