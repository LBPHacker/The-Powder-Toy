...
namespace Powder::Activity
{
	...
	void Preview::HandleTick()
	{
		...
		if (quickOpen && HasQuickOpenSucceeded())
		{
			Ok();
		}
		if (HasQuickOpenFailed())
		{
			...
			Exit();
			PushMessage("Failed to open save", message, true, nullptr); // TODO-REDO_UI-TRANSLATE
			return;
		}
	}
	...
	void Preview::GuiManage()
	{
		...
		if (EndButton())
		{
			...
			auto requestView = MakeRequestView(GetHost(), "Saving favorite", "Failed to save favorite", std::move(request)); // TODO-REDO_UI-TRANSLATE
	...
	void Preview::GuiCommentsPost()
	{
		...
		if (user)
		{
			...
			if (EndButton())
			{
				...
				auto requestView = MakeRequestView(GetHost(), "Submitting comment", "Failed to submit comment", std::move(request)); // TODO-REDO_UI-TRANSLATE
	...
	bool Preview::HasQuickOpenSucceeded() const
	{
		return std::holds_alternative<ThumbnailData>(thumbnail) ||
			   std::holds_alternative<InfoData>(info);
	}
	...
	void Preview::Gui()
	{
		if (quickOpen)
		{
			Progressdialog("dialog", "Opening save"); // TODO-REDO_UI-TRANSLATE
			return;
		}
