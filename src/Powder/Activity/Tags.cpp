	...
	void Tags::Gui()
	{
		...
		}
		{
			...
			for (auto &tag : saveInfo.tags)
			{
				...
				if (saveInfo.CanManage() && Button("button", "Remove", g.GetCommonMetrics().smallButton)) // TODO-REDO_UI-TRANSLATE
				{
					...
					auto requestView = MakeRequestView(GetHost(), "Removing tag", "Failed to remove tag", std::move(request)); // TODO-REDO_UI-TRANSLATE
		...
		if (user)
		{
			...
			if (EndButton())
			{
				...
				auto requestView = MakeRequestView(GetHost(), "Adding tag", "Failed to add tag", std::move(request)); // TODO-REDO_UI-TRANSLATE
