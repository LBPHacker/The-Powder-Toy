...
namespace Powder::Activity
{
	...
	void Game::GuiHud()
	{
		...
		auto m = GetMousePos();
		auto p = ResolveZoom(m ? *m : Pos2(-1, -1)); // TODO-REDO_UI: pass std::optional to GetSample
	...
	void Game::GuiSimSigns()
	{
		...
		for (auto i = int32_t(simulation->signs.size()) - 1; i >= 0; i--)
		{
			...
			rect.size.X += 1; // TODO-REDO_UI: figure out why this is needed
			if (!rect.Contains(*p))
			{
				continue;
			}
	...
	void Game::GuiMenuSections()
	{
		...
		Assert(sd.msections.size() == iconOverrides.size()); // TODO-REDO_UI: constexpr, but see previous TODO
	...
	void Game::GuiSave()
	{
		...
		}
		{
			...
			ByteString buttonText = "[untitled simulation]"; // TODO-REDO_UI: StringView // TODO-REDO_UI-TRANSLATE
			ByteString title;
			bool merge = false;
			if (onlineSave)
			{
