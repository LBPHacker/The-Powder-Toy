...
namespace Powder::Activity
{
	...
	void Game::InitActions()
	{
		...
		auto initRendererPreset = [this, &root, actionKeyPrefix](int32_t index, int32_t scancode, std::set<Input> modifiers = {}) {
		...
		{
			ActionKey actionKey = ByteString::Build(actionKeyPrefix, "SAVEBUTTONSALTFUNCTION");
		...
		{
			ActionKey actionKey = ByteString::Build(actionKeyPrefix, "TOGGLEFAVORITE");
			AddAction(actionKey, Action{ [this]() {
				if (lastHoveredTool)
				{
			...
			root.inputToActions.push_back({ MouseButtonInput{ SDL_BUTTON_LEFT }, { shiftInput, ctrlInput }, actionKey });
		}
	...
	void Game::DrawBrush(void *pixels, int32_t pitch)
	{
		auto m = GetMousePos();
	...
	void Game::UpdateToolTexture(ToolIndex index)
	{
		...
		if (auto textureData = tools[index]->tool->GetTexture(toolTextureDataSize))
		{
			...
			tools[index]->texture->data = PlaneAdapter<std::vector<pixel>>(toolTextureDataSize);
			for (auto p : toolTextureDataSize.OriginRect())
			{
				tools[index]->texture->data[p] = textureData->Data()[p.Y * toolTextureDataSize.X + p.X];
			}
		}
		shouldUpdateToolAtlas = true;
	}
	...
	void Game::GuiTools()
	{
		...
		if (GetShowingDecoTools())
		{
			...
			BeginHPanel("colorpicker");
			{
				...
				if (IsClicked(SDL_BUTTON_LEFT))
				{
					PushAboveThis(std::make_shared<ColorPicker>(*this));
				}
				EndComponent();
			}
			EndPanel();
	...
	void Game::OpenOnlinePreview()
	{
		...
		auto preview = std::make_shared<Preview>(GetHost(), saveInfo->GetID(), saveInfo->Version, false, std::nullopt);
		preview->SetGame(this);
	...
	void Game::GuiSave()
	{
		...
		{
			ByteString title = "\bg[untitled simulation]"; // TODO-REDO_UI: StringView // TODO-REDO_UI-TRANSLATE
			if (auto *onlineSave = save ? std::get_if<OnlineSave>(&*save) : nullptr)
			{
				title = (*onlineSave)->GetName().ToUtf8();
			}
			else if (auto *localSave = save ? std::get_if<LocalSave>(&*save) : nullptr)
			{
				title = (*localSave)->GetDisplayName().ToUtf8();
			}
			SetTitle(title);
			auto saveButtonFlags = saveButtonsAltFunction ? ButtonFlags::stuck : ButtonFlags::none;
			saveButtonFlags = saveButtonFlags | ButtonFlags::truncateText;
			BeginButton("savesim", ByteString::Build(Gui::iconSave, "\u0011\u0005", title), saveButtonFlags); // TODO-REDO_UI-TRANSLATE
		}
		SetSize(150);
		...
		EndButton();
		{
			...
			if (execVoteRequest)
			{
				BeginButton("cancelvote", "", ButtonFlags::none); // TODO-REDO_UI-TRANSLATE
		...
		if (Button("clear", ByteString::Build(" ", Gui::iconNew, " "), 17)) // TODO-REDO_UI: remove padding when icon alignment has been fixed
		{
			ClearSim();
		}
	...
	void Game::GuiBottom()
	{
		...
		SetSpacing(1);
		{
			...
			BeginButton("renderersettings", ByteString::Build(
				"\u0012\u000F\u00FF\u0001\u0001", // TODO-REDO_UI: figure out nulls
				Gui::iconColors1,
				"\u0011\u00FB\u000F\u0001\u00FF\u0001",
				Gui::iconColors2,
				"\u0011\u00F8\u000F\u0001\u0001\u00FF",
	...
	{
		PushAboveThis(std::make_shared<Activity::ConfirmQuit>(GetHost()));
	}
	...
	{
		stacks->SetHostStack(onlineBrowserStack);
		onlineBrowser->Open();
	}

	void Game::OpenLocalBrowser()
	{
		stacks->SetHostStack(localBrowserStack);
		localBrowser->Open();
	}
	...
	void Game::AddNotification(ServerNotification serverNotification)
	{
		// TODO-REDO_UI
	}
}
