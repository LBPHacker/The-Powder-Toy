...
namespace Powder::Activity
{
		...
		introTextAlpha{ GetHost(), Gui::TargetAnimation<int32_t>::LinearProfile{ 300 } }
	{
		...
		simContext->simulation = std::make_unique<Simulation>();
		{
			...
			auto saveFile = std::make_unique<SaveFile>("Save1844659");
			saveFile->SetGameSave(std::make_unique<GameSave>(std::vector(save1844659.begin(), save1844659.end())));
			saveFile->SetFileName("(none)");
			save = std::move(saveFile);
			ReloadSim(true);
			SetSimPaused(false);
		}
		InitActions();
	...
	void Game::GuiTools()
	{
		...
		if (GetShowingDecoTools())
		{
			...
			BeginHPanel("colorpicker");
			{
				...
				BeginButton(0, "", ButtonFlags::none);
	...
	void Game::GuiSave()
	{
		...
		auto hasOnlineSave = save && std::holds_alternative<OnlineSave>(*save);
		BeginButton("reloadsim", Gui::iconReload, (saveButtonsAltFunction && hasOnlineSave) ? ButtonFlags::stuck : ButtonFlags::none);
		...
		{
			ByteString title = "[untitled simulation]"; // TODO-REDO_UI: StringView
			...
			BeginButton("savesim", ByteString::Build(Gui::iconSave, "\u0011\u000A", title), saveButtonFlags); // TODO-REDO_UI-TRANSLATE
		}
		SetSize(150);
		SetTextAlignment(Gui::Alignment::left, Gui::Alignment::center);
		EndButton();
		BeginButton("upvote", ByteString::Build(Gui::iconUpvote, "\u0011\u0003Vote"), ButtonFlags::none); // TODO-REDO_UI-TRANSLATE
		SetEnabled(hasOnlineSave);
		SetSize(39);
		EndButton();
		AddSpacing(-2);
		BeginButton("downvote", Gui::iconDownvote, ButtonFlags::none);
		SetEnabled(hasOnlineSave);
		SetSize(15);
		EndButton();
	...
	void Game::GuiBottom()
	{
		...
		SetSpacing(1);
		{
			...
			if (Button(ByteString::Build(
				...
				"\u0011\u00F8\u000F\u0001\u0001\u00FF",
				Gui::iconColors3
			), 15, showRenderer ? ButtonFlags::stuck : ButtonFlags::none))
			{
			...
			std::visit([this, includePressure](const auto &save) {
				...
				if constexpr (std::is_same_v<std::remove_cvref_t<decltype(save)>, OnlineSave>)
				{
					...
					if (newAuthors.empty())
					{
						...
						newAuthors["published"]   = int(save->GetPublished());
