	...
	void Game::DrawBrush()
	{
		...
		if (drawState)
		{
			...
			case DrawMode::line:
				...
				RasterizeLine<true>(mPrev, mCurr, [this](Pos2 p) { drawSimFrame.XorPixel(p); });
				break;
			...
			case DrawMode::rect:
				{
					...
					RasterizeRect(rect, [this](Pos2 p) { drawSimFrame.XorPixel(p); });
					drawBrush = false;
				}
				break;
		...
		else if (brushModeFill) // DrawMode::fill only appears in drawState when we're actually drawing, so this will have to do
		{
			constexpr int32_t crosshairSize = 5;
			RasterizeLine<true>(mCurr + Vec2( 1,  0), mCurr + Vec2( crosshairSize,  0), [this](Pos2 p) { drawSimFrame.XorPixel(p); });
			RasterizeLine<true>(mCurr + Vec2(-1,  0), mCurr + Vec2(-crosshairSize,  0), [this](Pos2 p) { drawSimFrame.XorPixel(p); });
			RasterizeLine<true>(mCurr + Vec2( 0,  1), mCurr + Vec2( 0,  crosshairSize), [this](Pos2 p) { drawSimFrame.XorPixel(p); });
			RasterizeLine<true>(mCurr + Vec2( 0, -1), mCurr + Vec2( 0, -crosshairSize), [this](Pos2 p) { drawSimFrame.XorPixel(p); });
			drawBrush = false;
		}
