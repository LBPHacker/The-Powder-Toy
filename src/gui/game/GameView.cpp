...
public:
	...
	inline const SplitButtonAction &GetSplitActionCallback()
	{
		return actionCallback;
	}

	inline void SetSplitActionCallback(const SplitButtonAction &action)
	{
	...
	void Draw(const ui::Point &screenPos) override
	{
		...
		{
			g->DrawLine(
				screenPos + Vec2{ splitPosition, 1 }, screenPos + Vec2{ splitPosition, Size.Y - 2 }, 0xB4B4B4_rgb
			);
		}
	}
};
	...
	mousePosition(0, 0)
{
	...
	searchButton = new ui::Button(
		...
		ui::Point(17, 15),
		"",
		"Find & open a simulation. Hold Ctrl to load offline saves."
	); // Open
	...
	currentX += 18;
	reloadButton->SetActionCallback({ [this] {
										 c->ReloadSim();
									 },
	                                  [this] {
										  c->OpenSavePreview();
	...
	saveSimulationButton =
		new SplitButton(ui::Point(currentX, Size.Y - 16), ui::Point(150, 15), "[untitled simulation]", "", "", 19);
	saveSimulationButton->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;
	saveSimulationButton->SetIcon(IconSave);
	currentX += 151;
	saveSimulationButton->SetSplitActionCallback({ [this] {
	...
	tagSimulationButton = new ui::Button(
		ui::Point(currentX, Size.Y - 16), ui::Point(WINDOWW - 402, 15), "[no tags set]", "Add simulation tags"
	);
	tagSimulationButton->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;
	tagSimulationButton->SetIcon(IconTag);
	// currentX+=252;
	tagSimulationButton->SetActionCallback({ [this] {
		c->OpenTags();
	} });
	...
	clearSimButton->SetActionCallback({ [this] {
		c->ClearSim();
	} });
	AddComponent(clearSimButton);

	loginButton = new SplitButton(
		ui::Point(Size.X - 141, Size.Y - 16),
		ui::Point(92, 15),
		"[sign in]",
		"Sign into simulation server",
		"Edit Profile",
		19
	);
	loginButton->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;
	loginButton->SetIcon(IconLogin);
	loginButton->SetSplitActionCallback({ [this] {
											 c->OpenLogin();
										 },
	                                      [this] {
											  c->OpenProfile();
	...
	simulationOptionButton =
		new ui::Button(ui::Point(Size.X - 48, Size.Y - 16), ui::Point(15, 15), "", "Simulation options");
	simulationOptionButton->SetIcon(IconSimulationSettings);
	simulationOptionButton->Appearance.Margin.Left += 2;
	simulationOptionButton->SetActionCallback({ [this] {
		c->OpenOptions();
	} });
	...
	displayModeButton->SetActionCallback({ [this] {
		c->OpenRenderOptions();
	} });
	AddComponent(displayModeButton);

	pauseButton = new ui::Button(
		ui::Point(Size.X - 16, Size.Y - 16), ui::Point(15, 15), "", "Pause/Resume the simulation"
	); // Pause
	pauseButton->SetIcon(IconPause);
	pauseButton->SetTogglable(true);
	pauseButton->SetActionCallback({ [this] {
		c->SetPaused(pauseButton->GetToggleState());
	} });
	AddComponent(pauseButton);

	ui::Button *tempButton =
		new ui::Button(ui::Point(WINDOWW - 16, WINDOWH - 32), ui::Point(15, 15), 0xE065, "Search for elements");
	...
	colourPicker->SetActionCallback({ [this] {
		c->OpenColourPicker();
	} });
}
...
public:
	OptionListener(ui::Button *_button)
	{
		button = _button;
	}
...
void GameView::NotifyQuickOptionsChanged(GameModel *sender)
{
	...
	for (auto *option : optionList)
	{
		ui::Button *tempButton = new ui::Button(
			ui::Point(WINDOWW - 16, currentY), ui::Point(15, 15), option->GetIcon(), option->GetDescription()
		);
		// tempButton->Appearance.Margin = ui::Border(0, 2, 3, 2);
...
void GameView::NotifyMenuListChanged(GameModel *sender)
{
	...
	for (int i = (int)menuList.size() - 1; i >= 0; i--)
	{
		if (menuList[i]->GetVisible())
		{
			...
			auto *tempButton =
				new MenuButton(ui::Point(WINDOWW - 16, currentY), ui::Point(15, 15), tempString, description);
			tempButton->Appearance.Margin = ui::Border(0, 2, 3, 2);
			tempButton->menuID = i;
			...
			auto mouseEnterCallback = [this, tempButton] {
				...
				// if we change it here it causes components to be removed, which causes the window to stop sending
				// events and then the previous menusection button never gets sent the OnMouseLeave event and is never
				// unhighlighted
...
void GameView::NotifyActiveMenuToolListChanged(GameModel *sender)
{
	...
	for (size_t i = 0; i < toolList.size(); i++)
	{
		...
		if (tempTexture)
		{
			tempButton = new ToolButton(
				ui::Point(currentX, YRES + 1), ui::Point(30, 18), "", tool->Identifier, tool->Description
			);
		}
		else
		{
			tempButton = new ToolButton(
				ui::Point(currentX, YRES + 1), ui::Point(30, 18), tool->Name, tool->Identifier, tool->Description
			);
		}
		...
		tempButton->SetActionCallback({ [this, tempButton] {
			...
			if (ShiftBehaviour() && CtrlBehaviour() && !AltBehaviour())
			{
				...
				else if (tempButton->GetSelectionState() == 1)
				{
					...
					{
						new ConfirmPrompt(
							"Remove custom GOL type",
							"Are you sure you want to remove " + identifier.Substr(20).FromUtf8() + "?",
							{ [this, identifier]() {
...
void GameView::NotifyColourPresetsChanged(GameModel *sender)
{
	...
	{
		ToolButton *tempButton =
			new ToolButton(ui::Point(currentX, YRES + 1), ui::Point(30, 18), "", "", "Decoration Presets.");
...
void GameView::NotifySaveChanged(GameModel *sender)
{
	...
	if (sender->GetSave())
	{
		...
		upVoteButton->Enabled = sender->GetSave()->GetID() && sender->GetUser().UserID &&
			sender->GetUser().Username != sender->GetSave()->GetUserName();
		...
		{
			upVoteButton->SetActionCallback({ [this] {
				c->Vote(0);
			} });
		}
		else
		...
		if (sender->GetSave()->GetVote() == -1)
		{
			downVoteButton->SetActionCallback({ [this] {
				c->Vote(0);
			} });
		}
		else
		...
		if (sender->GetSave()->GetID())
		{
			...
			if (tags.size())
			{
				for (std::list<ByteString>::const_iterator iter = tags.begin(), begin = tags.begin(), end = tags.end();
				     iter != end;
				     iter++)
				{
					if (iter != begin)
	...
	else
	{
		...
		upVoteButton->Enabled = false;
		upVoteButton->Appearance.BackgroundDisabled = (ui::Colour(0, 0, 0));
		upVoteButton->Appearance.BorderDisabled = ui::Colour(100, 100, 100), downVoteButton->Enabled = false;
		downVoteButton->Appearance.BackgroundDisabled = (ui::Colour(0, 0, 0));
		downVoteButton->Appearance.BorderDisabled = ui::Colour(100, 100, 100), tagSimulationButton->Enabled = false;
		tagSimulationButton->SetText("[no tags set]");
		currentSaveType = 0;
	}
...
ByteString GameView::TakeScreenshot(int captureUI, int fileType)
{
	...
	{
		filename += ".bmp";
		// We should be able to simply use SDL_PIXELFORMAT_XRGB8888 here with a bit depth of 32 to convert RGBA data to
		// RGB data, and save the resulting surface directly. However, ubuntu-18.04 ships SDL2 so old that it doesn't
		// have SDL_PIXELFORMAT_XRGB8888, so we first create an RGBA surface and then convert it.
		auto *rgbaSurface = SDL_CreateRGBSurfaceWithFormatFrom(
			screenshot->Data(),
			screenshot->Size().X,
			screenshot->Size().Y,
			32,
			screenshot->Size().X * sizeof(pixel),
...
void GameView::updateToolButtonScroll()
{
	if (toolButtons.size())
	{
		...
		if (totalWidth > XRES - 15)
		{
			...
			// if (mouseX < 15) // makes scrolling a little nicer at edges but apparently if you put hundreds of
			// elements in a menu it makes the end not show ... 	mouseX = 15;
			...
			mouseLocation = (float)(XRES - 3) /
				(float)((XRES - 2) - mouseX
			    ); // mouseLocation adjusted slightly in case you have 200 elements in one menu
		...
		// Ensure that mouseLeave events are make their way to the buttons should they move from underneath the mouse
		// pointer
...
void GameView::OnMouseUp(int x, int y, unsigned button)
{
	...
	else if (isMouseDown)
	{
		...
		if (selectMode != SelectNone)
		{
			if (button == SDL_BUTTON_LEFT && selectPoint1.X != -1 && selectPoint1.Y != -1 && selectPoint2.X != -1 &&
			    selectPoint2.Y != -1)
			{
				if (selectMode == PlaceSave)
				{
...
void GameView::OnKeyPress(int key, int scan, bool repeat, bool shift, bool ctrl, bool alt)
{
	...
	case SDL_SCANCODE_B:
		...
		else if (colourPicker->GetParentWindow())
		{
...
{
	Window::DoDraw();
	constexpr std::array<int, 9> fadeout = {
		{ // * Gamma-corrected.
		  255, 195,
         145, 103,
         69, 42,
...
void GameView::NotifyNotificationsChanged(GameModel *sender)
{
	...
	for (auto *notification : notifications)
	{
		...
		ui::Button *tempButton =
			new ui::Button(ui::Point(XRES - width - 22, currentY), ui::Point(width, 15), notification->Message);
...
void GameView::NotifyTransformedPlaceSaveChanged(GameModel *sender)
{
	...
	{
		placeSaveThumb =
			SaveRenderer::Ref().Render(sender->GetTransformedPlaceSave(), true, sender->GetRendererSettings());
...
void GameView::enableCtrlBehaviour()
{
	if (!ctrlBehaviour)
	{
		...
		saveSimulationButton->Appearance.BackgroundInactive = saveSimulationButton->Appearance.BackgroundHover =
			ui::Colour(255, 255, 255);
		saveSimulationButton->Appearance.TextInactive = saveSimulationButton->Appearance.TextHover =
			ui::Colour(0, 0, 0);

		saveSimulationButton->Enabled = true;
		SetSaveButtonTooltips();

		searchButton->Appearance.BackgroundInactive = searchButton->Appearance.BackgroundHover =
			ui::Colour(255, 255, 255);
...
void GameView::SetSaveButtonTooltips()
{
	...
	{
		saveSimulationButton->SetToolTips(
			"Overwrite the open simulation on your hard drive.", "Save the simulation to your hard drive."
		);
	}
	...
	{
		saveSimulationButton->SetToolTips(
			"Re-upload the current simulation", "Upload a new simulation. Hold Ctrl to save offline."
		);
	}
}
...
void GameView::OnDraw()
{
	...
	if (showBrush && selectMode == SelectNone && (!zoomEnabled || zoomCursorFixed) && activeBrush &&
	    (isMouseDown || (currentMouse.X >= 0 && currentMouse.X < XRES && currentMouse.Y >= 0 && currentMouse.Y < YRES)))
	{
		...
		if (drawMode == DrawPoints || drawMode == DrawLine || (drawMode == DrawRect && !isMouseDown))
		{
			if (wallBrush)
			{
				...
				g->XorLine(
					{ topLeft.X, topLeft.Y + 1 }, { topLeft.X, bottomRight.Y - 1 }
				); // offset by 1 so the corners don't get xor'd twice
				g->XorLine(
					{ bottomRight.X, topLeft.Y + 1 }, { bottomRight.X, bottomRight.Y - 1 }
				); // offset by 1 so the corners don't get xor'd twice
			}
			else
			{
	...
	if (selectMode != SelectNone)
	{
		...
		else
		{
			...
			else
			{
				...
				g->BlendFilledRect(
					RectSized(Vec2{ 0, y2 + 1 }, Vec2{ XRES, YRES - y2 - 1 }), 0x000000_rgb .WithAlpha(100)
				);

				g->BlendFilledRect(RectSized(Vec2{ 0, y1 }, Vec2{ x1, (y2 - y1) + 1 }), 0x000000_rgb .WithAlpha(100));
				g->BlendFilledRect(
					RectSized(Vec2{ x2 + 1, y1 }, Vec2{ XRES - x2 - 1, (y2 - y1) + 1 }), 0x000000_rgb .WithAlpha(100)
				);
	...
	if (recording)
	{
		...
		ByteString filename = ByteString::Build(
			"recordings",
			PATH_SEP_CHAR,
			recordingFolder,
			PATH_SEP_CHAR,
			"frame_",
			Format::Width(recordingIndex++, 6),
			".ppm"
		);
	...
	if (logEntries.size())
	{
		...
		for (iter = logEntries.begin(); iter != logEntries.end(); iter++)
		{
			...
			startY -= 14;
			g->BlendFilledRect(
				RectSized(Vec2{ startX - 3, startY - 3 }, Vec2{ Graphics::TextSize(message).X + 5, 14 }),
				0x000000_rgb .WithAlpha(std::min(100, alpha))
			);
	...
	if (recording)
	{
		...
		g->BlendFilledRect(
			RectSized(Vec2{ XRES - 20 - textWidth, 12 }, Vec2{ textWidth + 8, 15 }), 0x000000_rgb .WithAlpha(127)
		);
	...
	else if (showHud)
	{
		...
		if (type)
		{
			...
			if (type == PT_PHOT || type == PT_BIZR || type == PT_BIZRG || type == PT_BIZRS || type == PT_FILT ||
			    type == PT_BRAY || type == PT_C5)
			{
			...
			if (showDebug)
			{
				...
				else if ((type == PT_PIPE || type == PT_PPIP) && c->IsValidElement(ctype))
				{
					...
					{
						sampleInfo << c->ElementResolve(type, 0) << " with molten "
								   << c->ElementResolve(sample.particle.tmp4, -1);
					}
					else
					{
						sampleInfo << c->ElementResolve(type, 0) << " with "
								   << c->ElementResolve(ctype, sample.particle.tmp4);
					}
				}
				...
				else if (type == PT_FILT)
				{
					sampleInfo << c->ElementResolve(type, ctype);
					String filtModes[] = {
						"set colour", "AND", "OR",  "subtract colour",     "red shift",          "blue shift",
						"no effect",  "XOR", "NOT", "old QRTZ scattering", "variable red shift", "variable blue shift"
					};
				...
				else
				{
					...
					else if (c->IsValidElement(ctype) && type != PT_GLOW && type != PT_WIRE && type != PT_SOAP &&
					         type != PT_LITH)
					{
				...
				if (sample.particle.type != PT_RFRG && sample.particle.type != PT_RFGL &&
				    sample.particle.type != PT_LIFE)
				{
					if (sample.particle.type == PT_CONV)
					{
						String elemName = c->ElementResolve(TYP(sample.particle.tmp), ID(sample.particle.tmp));
						if (elemName == "")
						{
				...
				if (type == PT_CRAY || type == PT_DRAY || type == PT_EXOT || type == PT_LIGH || type == PT_SOAP ||
				    type == PT_TRON || type == PT_VIBR || type == PT_VIRS || type == PT_WARP || type == PT_LCRY ||
				    type == PT_CBNW || type == PT_TSNS || type == PT_DTEC || type == PT_LSNS || type == PT_PSTN ||
				    type == PT_LDTC || type == PT_VSNS || type == PT_LITH || type == PT_CONV || type == PT_ETRD)
				{
		...
		g->BlendFilledRect(
			RectSized(Vec2{ XRES - 20 - textWidth, 12 }, Vec2{ textWidth + 8, 15 }),
			0x000000_rgb .WithAlpha(int(alpha * 0.5f))
		);
			...
			for (i = 0; i < 30; i++)
			{
				if ((wavelengthGfx >> i) & 1)
				{
					...
					for (j = 0; j < h; j++)
					{
						g->BlendPixel(
							{ x + 29 - i, y + j },
							RGBA<uint8_t>(cr > 255 ? 255 : cr, cg > 255 ? 255 : cg, cb > 255 ? 255 : cb, alpha)
						);
					}
				}
			}
		...
		if (showDebug)
		{
			...
			auto gravtot = std::abs(sample.GravityVelocityX) + std::abs(sample.GravityVelocityY);
			if (gravtot)
			{
			...
			g->BlendFilledRect(
				RectSized(Vec2{ XRES - 20 - textWidth, 27 }, Vec2{ textWidth + 8, 14 }),
				0x000000_rgb .WithAlpha(int(alpha * 0.5f))
			);
			g->BlendText(
				{ XRES - 16 - textWidth, 30 }, sampleInfo.Build(), 0xFFFFFF_rgb .WithAlpha(int(alpha * 0.75f))
			);
		}
	}
	...
	if (showHud && introText < 51)
	{
		...
		int alpha = 255 - introText * 5;
		g->BlendFilledRect(
			RectSized(Vec2{ 12, 12 }, Vec2{ textWidth + 8, 15 }), 0x000000_rgb .WithAlpha(int(alpha * 0.5))
		);
	...
	if (infoTipPresence)
	{
		...
		g->BlendTextOutline(
			{ (XRES - (Graphics::TextSize(infoTip).X - 1)) / 2, YRES / 2 - 2 },
			infoTip,
			0xFFFFFF_rgb .WithAlpha(infoTipAlpha)
		);
	}
	...
	if (toolTipPresence && toolTipPosition.X != -1 && toolTipPosition.Y != -1 && toolTip.length())
	{
		...
		{
			g->BlendTextOutline(
				toolTipPosition, toolTip, 0xFFFFFF_rgb .WithAlpha(toolTipPresence > 51 ? 255 : toolTipPresence * 5)
			);
		}
		else
		{
	...
	{
		g->BlendText(
			{ 16, Size.Y - MENUSIZE - 24 },
			buttonTip,
			0xFFFFFF_rgb .WithAlpha(buttonTipShow > 51 ? 255 : buttonTipShow * 5)
		);
	}
	...
	{
		g->BlendFilledRect(
			RectSized(Vec2{ 0, 0 }, WINDOW), 0x000000_rgb .WithAlpha(introText > 51 ? 102 : introText * 2)
		);
...
std::optional<FindingElement> GameView::FindingElementCandidate() const
{
	...
	else if (active->Identifier == "DEFAULT_UI_PROPERTY")
	{
		...
		if (configuration)
		{
			return FindingElement{ properties[configuration->changeProperty.propertyIndex],
				                   configuration->changeProperty.propertyValue };
		}
	}
	return std::nullopt;
}
