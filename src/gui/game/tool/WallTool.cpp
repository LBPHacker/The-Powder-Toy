...
void WallTool::Draw(Simulation *sim, const Brush &brush, ui::Point position)
{
	sim->CreateWalls(position.X, position.Y, 1, 1, ToolID, &brush);
}

void WallTool::DrawLine(Simulation *sim, const Brush &brush, ui::Point position1, ui::Point position2, bool dragging)
{
...
void WallTool::DrawRect(Simulation *sim, const Brush &brush, ui::Point position1, ui::Point position2)
{
	sim->CreateWallBox(position1.X, position1.Y, position2.X, position2.Y, ToolID);
}

void WallTool::DrawFill(Simulation *sim, const Brush &brush, ui::Point position)
{
	if (ToolID != WL_STREAM)
	{
