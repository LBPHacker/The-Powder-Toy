...
BitmapBrush::BitmapBrush(ui::Point inputSize, const unsigned char *inputBitmap)
{
...
BitmapBrush::BitmapBrush(const BitmapBrush &other) :
	BitmapBrush(other.origSize, other.origBitmap.data())
{
}
	...
	else
	{
		...
		for (int y = 0; y < size.Y; y++)
		{
			for (int x = 0; x < size.X; x++)
			{
				...
				float bottom =
					LinearInterpolate<float>(bottomLeft, bottomRight, float(lowerX), float(upperX), originalX);
