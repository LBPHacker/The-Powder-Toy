...
namespace gui
{
	...
	int TextWrapper::Update(const String &text, bool doWrapping, int maxWidth)
	{
		...
		auto wrapIfNeeded = [&](int widthToConsider) -> bool {
			...
			default:
				...
				else
				{
					...
					if (wrapIfNeeded(lineWidth))
					{
						...
						//   the record at position wordBeginsAt (0-based).
						std::rotate(
		...
		for (const auto &record : records)
		{
			...
			cmr.pos1 = Point{ x, l * FONT_H };
			cmr.line1 = l;
			cmr.index = Index{ record.rawIndex, counter, record.clearIndex };
			counter += 1;
