...
namespace gui
{
	void MomentumAnimation::Start(int position, float momentum, int32_t time)
	{
	...
	{
		return initialMomentum * exp2f(-((SDLWindow::Ticks() - initialTime) / 1000.f) / halfLife); // a * .5 ^ (t / t_{1/2})
	}
	...
	{
		Start(position, 0.f, SDLWindow::Ticks());
	}
	...
	void MomentumAnimation::HalfLife(float newHalfLife)
	{
		...
		Start(effectivePosition, effectiveMomentum, SDLWindow::Ticks());
	}
	...
	void MomentumAnimation::Limit(float newLimit)
	{
		...
		{
			Start(EffectivePosition(), copysignf(limit, effectiveMomentum), SDLWindow::Ticks());
		}
		CancelIfSmall();
	}
