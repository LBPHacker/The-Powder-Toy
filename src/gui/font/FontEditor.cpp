...
void FontEditor::ReadDataFile(ByteString dataFile)
{
	...
	{
		fontRanges.push_back({
			{ (unsigned int)rng[0], (unsigned int)rng[1] }
        });
	}
}

void FontEditor::WriteDataFile(
	ByteString dataFile,
...
)
{
	...
	for (size_t range = 0; fontRanges[range][1]; range++)
	{
		...
		{
			const unsigned char *pointer = &fontData[fontPtrs[pos]];
...
)
{
	...
	for (std::map<String::value_type, unsigned char>::const_iterator it = fontWidths.begin(); it != fontWidths.end();
	     it++)
	{
	...
	rulers(1)
{
	...
	for (int i = 0; i < 6; i++)
	{
		ui::Textbox *colorComponent =
			new ui::Textbox(ui::Point(currentX, baseline), ui::Point(27, 17), String::Build(*refs[i]));
		currentX += 28;
	...
	savedButton->SetActionCallback({ [this] {
		Save();
	} });
	...
	ui::ScrollPanel *outputPanel =
		new ui::ScrollPanel(ui::Point(Size.X / 2, baseline), ui::Point(Size.X / 2, Size.Y - baseline));
	...
	ui::Window(ui::Point(0, 0), ui::Point(WINDOWW, WINDOWH))
{
	...
	for (const auto &p : srcFontPixels)
	{
		if (tgtFontPixels.count(p.first))
		{
			...
			if (!same)
			{
				std::cout << "U+" << std::hex << (unsigned int)p.first << " is present in both files and is different!"
						  << std::endl;
			}
		}
		else
...
void FontEditor::OnDraw()
{
	...
	if (fontWidths.find(currentChar) != fontWidths.end())
	{
		...
		const std::array<std::array<char, MAX_WIDTH>, FONT_H> &pixels = fontPixels[currentChar];

		int areaWidth = 8 + width * FONT_SCALE + 8;
		g->DrawFilledRect(
			RectSized(Vec2{ 0, 0 }, Vec2{ areaWidth, 8 + FONT_H * FONT_SCALE + 4 + FONT_H + 4 }),
			RGB<uint8_t>(bgR, bgG, bgB)
		);
		for (int j = 0; j < FONT_H; j++)
		{
			for (int i = 0; i < width; i++)
			{
				g->BlendFilledRect(
					RectSized(
						Vec2{ 8 + i * FONT_SCALE, 8 + j * FONT_SCALE }, Vec2{ FONT_SCALE - grid, FONT_SCALE - grid }
					),
					RGBA<uint8_t>(fgR, fgG, fgB, pixels[j][i] * 255 / 3)
				);
			}
		}
		...
		for (int j = 0; j < FONT_H; j++)
		{
			...
			{
				g->BlendPixel(
					{ 8 + i, 8 + FONT_H * FONT_SCALE + 4 + j }, RGBA<uint8_t>(fgR, fgG, fgB, pixels[j][i] * 255 / 3)
				);
			}
		}
		...
		if (rulers)
		{
			...
			g->DrawLine(
				{ 7 + width * FONT_SCALE, 8 }, { 7 + width * FONT_SCALE, 7 + FONT_H * FONT_SCALE }, 0x808080_rgb
			);
		}
	}
	else
