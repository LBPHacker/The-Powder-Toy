...
OptionsView::OptionsView() : ui::Window(ui::Point(-1, -1), ui::Point(320, 340))
{
	...
	partCount = addTextbox("Particle count", [this]() {
		UpdateSimConfig();
		SaveSimConfig();
	});
	maxTemp = addTextbox("Maximum temperature", [this]() {
		UpdateSimConfig();
		SaveSimConfig();
	});
	minTemp = addTextbox("Minimum temperature", [this]() {
		UpdateSimConfig();
		SaveSimConfig();
	});
	maxPressure = addTextbox("Maximum pressure", [this]() {
		UpdateSimConfig();
		SaveSimConfig();
	});
	minPressure = addTextbox("Minimum pressure", [this]() {
		UpdateSimConfig();
		SaveSimConfig();
	});
	{
...
void OptionsView::LoadSimConfig(SimulationConfig config)
{
	...
	if (!partCount->IsFocused())
	{
		partCount->SetText(String::Build(config.NPART));
	}
	if (!maxTemp->IsFocused())
	{
		maxTemp->SetText(String::Build(config.MAX_TEMP));
	}
	if (!minTemp->IsFocused())
	{
		minTemp->SetText(String::Build(config.MIN_TEMP));
	}
	if (!maxPressure->IsFocused())
	{
		maxPressure->SetText(String::Build(config.MAX_PRESSURE));
	}
	if (!minPressure->IsFocused())
	{
		minPressure->SetText(String::Build(config.MIN_PRESSURE));
	}
}
...
void OptionsView::UpdateSimConfig()
{
	try
	{
		...
		newConfig->NPART        = partCount          ->GetText().ToNumber<int  >();
		newConfig->MAX_TEMP     = maxTemp            ->GetText().ToNumber<float>();
		newConfig->MIN_TEMP     = minTemp            ->GetText().ToNumber<float>();
		newConfig->MAX_PRESSURE = maxPressure        ->GetText().ToNumber<float>();
		newConfig->MIN_PRESSURE = minPressure        ->GetText().ToNumber<float>();
		newConfig->Check();
	}
