...
void OptionsView::LoadSimConfig(SimulationConfig config)
{
	...
	{
		horizontalCellCount->SetText(String::Build(config.vCELLS.X));
	}
	if (!verticalCellCount->IsFocused())
	{
		verticalCellCount->SetText(String::Build(config.vCELLS.Y));
	}
	if (!cellSize->IsFocused())
	{
		cellSize->SetText(String::Build(config.vCELL));
	}
}
...
void OptionsView::UpdateSimConfig()
{
	try
	{
		...
		newConfig->vCELLS.X = horizontalCellCount->GetText().ToNumber<int>();
		newConfig->vCELLS.Y = verticalCellCount->GetText().ToNumber<int>();
		newConfig->vCELL = cellSize->GetText().ToNumber<int>();
		newConfig->Check();
	}
