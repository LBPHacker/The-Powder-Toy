...
void OptionsModel::SetNextSimulationConfig(SimulationConfig config)
{
	...
	prefs.Set("Simulation.MaxTemp"    , config.MAX_TEMP    );
	prefs.Set("Simulation.MinTemp"    , config.MIN_TEMP    );
	prefs.Set("Simulation.MaxPressure", config.MAX_PRESSURE);
	prefs.Set("Simulation.MinPressure", config.MIN_PRESSURE);
	notifySettingsChanged();
}
