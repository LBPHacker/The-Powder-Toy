...
void ProfileActivity::setUserInfo(UserInfo newInfo)
{
	...
	// username label
	ui::Label *title =
		new ui::Label(ui::Point(4, currentY), ui::Point(Size.X - 8 - (40 + 16 + 75), 15), info.username.FromUtf8());
	...
	if (editable)
	{
		ui::Button *editAvatar =
			new ui::Button(ui::Point(Size.X - (40 + 16 + 75), currentY), ui::Point(75, 15), "Edit Avatar");
	...
	ui::Label *age = new ui::Label(
		ui::Point(5 + ageTitle->Size.X, currentY),
		ui::Point(Size.X - ageTitle->Size.X - 56, 15),
		info.age ? String::Build(info.age) : "\bgNot Provided"
	);
	...
	if (editable)
	{
		location = new ui::Textbox(
			ui::Point(5 + locationTitle->Size.X, currentY),
			ui::Point(Size.X - locationTitle->Size.X - 16, 17),
			info.location
		);
	...
	{
		location = new ui::Label(
			ui::Point(5 + locationTitle->Size.X, currentY),
			ui::Point(Size.X - locationTitle->Size.X - 14, 17),
			info.location
		);
	}
	...
	ui::Label *website = new ui::Label(
		ui::Point(2 + websiteTitle->Size.X, currentY),
		ui::Point(Size.X - websiteTitle->Size.X - 16, 15),
		info.website.FromUtf8()
	);
	...
	ui::Label *savesCount = new ui::Label(
		ui::Point(13 + saveCountTitle->Size.X, currentY),
		ui::Point(Size.X - saveCountTitle->Size.X - 24, 15),
		String::Build(info.saveCount)
	);
	...
	ui::Label *averageScore = new ui::Label(
		ui::Point(13 + averageScoreTitle->Size.X, currentY),
		ui::Point(Size.X - averageScoreTitle->Size.X - 24, 15),
		String::Build(info.averageScore)
	);
	...
	ui::Label *highestScore = new ui::Label(
		ui::Point(13 + highestScoreTitle->Size.X, currentY),
		ui::Point(Size.X - highestScoreTitle->Size.X - 24, 15),
		String::Build(info.highestScore)
	);
	...
	if (editable)
	{
		...
		((ui::Textbox *)bio)->SetActionCallback({ [this] {
			ResizeArea();
		} });
