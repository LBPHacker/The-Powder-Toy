	...
	exit(false)
{
	...
	searchField->SetActionCallback({ [this] {
		searchTools(searchField->GetText());
	} });
	...
	scrollPanel = new ui::ScrollPanel(
		...
		{ searchField->Size.X - 2, Size.Y - (searchField->Position.Y + searchField->Size.Y + 6) - 23 }
	);
...
void ElementSearchActivity::searchTools(String query)
{
	...
	struct Match
	{
		...
		{
			return std::tie(favouritePriority, haystackOrigin, needlePosition, toolIndex) <
				std::tie(other.favouritePriority, other.haystackOrigin, other.needlePosition, other.toolIndex);
		}
	};
	...
	auto pushIfMatches = [&queryLower,
	                      &push](String infoLower, int toolIndex, int favouritePriority, int haystackRelevance) {
	...
	for (auto &match : matches)
	{
		...
		{
			tempButton =
				new ToolButton(current + viewPosition, ui::Point(30, 18), "", tool->Identifier, tool->Description);
		}
		else
		{
			tempButton = new ToolButton(
				current + viewPosition, ui::Point(30, 18), tool->Name, tool->Identifier, tool->Description
			);
		}
...
void ElementSearchActivity::SetActiveTool(int selectionState, Tool *tool)
{
	...
	else if (ctrlPressed && altPressed && !shiftPressed && tool->Identifier.BeginsWith("DEFAULT_PT_"))
	{
...
void ElementSearchActivity::OnDraw()
{
	...
	{
		g->BlendText(
			{ 10, Size.Y + 70 }, toolTip, 0xFFFFFF_rgb .WithAlpha(toolTipPresence > 51 ? 255 : toolTipPresence * 5)
		);
	}
}
