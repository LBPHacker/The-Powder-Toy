	...
	commentHelpText(false)
{
	...
	openButton->SetActionCallback({ [this] {
		c->DoOpen();
	} });
	...
	browserOpenButton->SetActionCallback({ [this] {
		c->OpenInBrowser();
	} });
	...
	loadErrorButton->SetActionCallback({ [this] {
		ShowLoadError();
	} });
	...
	missingElementsButton->SetActionCallback({ [this] {
		ShowMissingCustomElements();
	} });
	...
	if (showAvatars)
	{
		saveDescriptionLabel = new ui::Label(
			ui::Point(5, (YRES / 2) + 4 + 15 + 21),
			ui::Point((XRES / 2) - 10, Size.Y - ((YRES / 2) + 4 + 15 + 17) - 25),
			""
		);
	}
	else
	{
		saveDescriptionLabel = new ui::Label(
			ui::Point(5, (YRES / 2) + 4 + 15 + 19),
			ui::Point((XRES / 2) - 10, Size.Y - ((YRES / 2) + 4 + 15 + 17) - 23),
			""
		);
	}
	...
	commentsPanel = new ui::ScrollPanel(
		ui::Point((XRES / 2) + 1, 1), ui::Point((Size.X - (XRES / 2)) - 2, Size.Y - commentBoxHeight)
	);
...
void PreviewView::AttachController(PreviewController *controller)
{
	...
	saveIDLabel = new ui::Label(
		ui::Point((Size.X - textWidth - 20) / 2, Size.Y + 5),
		ui::Point(textWidth + 20, 16),
		"Click the box below to copy the save ID"
	);
	...
	saveIDLabel2 =
		new ui::Label(ui::Point((Size.X - textWidth - 20) / 2 - 37, Size.Y + 22), ui::Point(40, 16), "Save ID:");
	AddComponent(saveIDLabel2);

	saveIDButton = new ui::CopyTextButton(
		ui::Point((Size.X - textWidth - 10) / 2, Size.Y + 20),
		ui::Point(textWidth + 10, 18),
		String::Build(c->SaveID()),
		saveIDLabel
	);
...
void PreviewView::commentBoxAutoHeight()
{
	...
	if (commentHelpText || textWidth + 15 > Size.X - (XRES / 2) - 48)
	{
		...
		if (commentWarningLabel && commentHelpText && !commentWarningLabel->Visible &&
		    addCommentBox->Position.Y + addCommentBox->Size.Y < Size.Y - 14)
		{
...
void PreviewView::DoDraw()
{
	...
	if (!c->GetFromUrl())
	{
		...
		{
			int linePos = commentTextComponents[i]->Position.Y + commentsPanel->ViewportPosition.Y +
				commentTextComponents[i]->Size.Y + 4;
	...
	if (c->GetDoOpen())
	{
		...
		g->BlendText(
			Position + Vec2{ (Size.X / 2) - ((Graphics::TextSize("Loading save...").X - 1) / 2), (Size.Y / 2) - 5 },
			"Loading save...",
			style::Colour::InformationTitle.NoAlpha().WithAlpha(255)
		);
	}
	if (!c->GetFromUrl())
	{
...
void PreviewView::OnDraw()
{
	...
	if (savePreview)
	{
		g->BlendImage(
			savePreview->Data(),
			0xFF,
			RectSized(Position + Vec2(1, 1) + (RES / 2 - savePreview->Size()) / 2, savePreview->Size())
		);
	}
...
void PreviewView::OnTick(float dt)
{
	if (addCommentBox)
	{
		ui::Point positionDiff =
			ui::Point(int(commentBoxPositionX), int(commentBoxPositionY)) - addCommentBox->Position;
...
void PreviewView::ShowMissingCustomElements()
{
	...
	if (missingElements.identifiers.size())
	{
		...
		for (auto &[identifier, id] : missingElements.identifiers)
		{
			...
			); // remove ids from the missing id set that are already covered by unknown identifiers
		}
	}
	...
	if (save)
	{
		...
		if (showAvatars)
		{
			...
			authorDateLabel->SetText(
				"\bw" + save->userName.FromUtf8() + " \bg" + dateType + " \bw" +
				format::UnixtimeToDateMini(save->updatedDate).FromAscii()
			);
		}
		else
		{
			authorDateLabel->SetText(
				"\bgAuthor:\bw " + save->userName.FromUtf8() + " \bg" + dateType + " \bw" +
				format::UnixtimeToDateMini(save->updatedDate).FromAscii()
			);
		}
...
void PreviewView::NotifyCommentBoxEnabledChanged(PreviewModel *sender)
{
	...
	if (sender->GetCommentBoxEnabled())
	{
		...
		addCommentBox = new ui::Textbox(
			ui::Point((XRES / 2) + 4, Size.Y - 19), ui::Point(Size.X - (XRES / 2) - 48, 17), "", "Add Comment"
		);
		...
		submitCommentButton->SetActionCallback({ [this] {
			submitComment();
		} });
		AddComponent(submitCommentButton);

		commentWarningLabel = new ui::Label(
			ui::Point((XRES / 2) + 4, Size.Y - 19),
			ui::Point(Size.X - (XRES / 2) - 48, 16),
	...
	{
		submitCommentButton =
			new ui::Button(ui::Point(XRES / 2, Size.Y - 19), ui::Point(Size.X - (XRES / 2), 19), "Login to comment");
		submitCommentButton->SetActionCallback({ [this] {
			c->ShowLogin();
		} });
		AddComponent(submitCommentButton);
...
void PreviewView::NotifyCommentsChanged(PreviewModel *sender)
{
	...
	if (commentsPtr)
	{
		...
		for (size_t i = 0; i < comments.size(); i++)
		{
			...
			{
				tempAvatar =
					new ui::AvatarButton(ui::Point(2, currentY + 7), ui::Point(26, 26), comments[i].authorName);
			...
			else if (comments[i].authorIsBanned)
			{
				authorNameFormatted = "\bg" + authorNameFormatted;
			         ...
			         Client::Ref().GetAuthUser().Username == comments[i].authorName)
			{
			...
			if (showAvatars)
			{
				tempUsername = new ui::Label(
					ui::Point(31, currentY + 8), ui::Point(Size.X - ((XRES / 2) + 13 + 26), 14), authorNameFormatted
				);
			}
			else
			{
				tempUsername = new ui::Label(
					ui::Point(5, currentY + 8), ui::Point(Size.X - ((XRES / 2) + 13), 14), authorNameFormatted
				);
			}
			...
			if (showAvatars)
			{
				tempComment = new ui::Label(
					ui::Point(31, currentY + 5), ui::Point(Size.X - ((XRES / 2) + 13 + 26), -1), comments[i].content
				);
			}
			else
			{
				tempComment = new ui::Label(
					ui::Point(5, currentY + 5), ui::Point(Size.X - ((XRES / 2) + 13), -1), comments[i].content
				);
			}
