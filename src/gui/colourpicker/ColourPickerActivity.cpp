	...
	onPicked(onPicked_)
{
	...
	RGB_to_HSV(
		initialColour.Red, initialColour.Green, initialColour.Blue, &currentHue, &currentSaturation, &currentValue
	);
...
void ColourPickerActivity::OnDraw()
{
	...
	for (int rx = 0; rx < gradientWidth; rx++)
	{
		...
		{
			g->BlendPixel(offset + hSlider->Position + Vec2{ rx, ry }, RGBA<uint8_t>(red, green, blue, currentAlpha));
		}
	}
	...
	g->XorLine(
		offset + Vec2{ currentHueX, currentSaturationY - 5 }, offset + Vec2{ currentHueX, currentSaturationY - 1 }
	);
	g->XorLine(
		offset + Vec2{ currentHueX, currentSaturationY + 1 }, offset + Vec2{ currentHueX, currentSaturationY + 5 }
	);
	g->XorLine(
		offset + Vec2{ currentHueX - 5, currentSaturationY }, offset + Vec2{ currentHueX - 1, currentSaturationY }
	);
	g->XorLine(
		offset + Vec2{ currentHueX + 1, currentSaturationY }, offset + Vec2{ currentHueX + 5, currentSaturationY }
	);
}
