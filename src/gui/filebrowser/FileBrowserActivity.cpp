...
// Currently, reading is done on another thread, we can't render outside the main thread due to some bullshit with
// OpenGL
	...
	totalFiles(0)
{
	...
	textField->SetActionCallback({ [this, textField] {
		DoSearch(textField->GetText().ToUtf8());
	} });
	...
	infoText =
		new ui::Label(ui::Point((Size.X - 200) / 2, 45 + (Size.Y - 66) / 2), ui::Point(200, 17), "No saves found");
...
{
	new TextPrompt(
		"Rename", "Change save name", files[index]->GetDisplayName(), "", 0, { [this, index](const String &input) {
			auto &file = files[index];
...
void FileBrowserActivity::OnTick(float dt)
{
	...
	if (createButtons)
	{
		...
		for (auto i = 0; i < int(files.size()); ++i)
		{
			...
			saveButton->Tick(dt);
			saveButton->SetActionCallback({ [this, i] {
											   SelectSave(i);
										   },
			                                [this, i] {
												RenameSave(i);
	...
	{
		for (std::vector<ui::Component *>::iterator iter = componentsQueue.begin(), end = componentsQueue.end();
		     iter != end;
		     ++iter)
		{
