...
public:
	...
	void SetPeriod(http::Period period)
	{
		if (!searchSaves)
		{
	...
	http::Period GetPeriod()
	{
		return currentPeriod;
	}

	void SetSort(http::Sort sort)
	{
		if (!searchSaves)
		{
			currentSort = sort;
	...
	void SetShowFavourite(bool show)
	{
		if (show != showFavourite && !searchSaves)
		{
			showFavourite = show;
		}
		notifyShowFavouriteChanged();
	}

	bool GetShowFavourite()
	{
