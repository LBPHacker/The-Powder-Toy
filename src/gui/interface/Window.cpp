...
void Window::AddComponent(Component *c)
{
	if (c->GetParentWindow() == NULL)
	{
		...
		if (Engine::Ref().GetMouseX() > Position.X + c->Position.X &&
		    Engine::Ref().GetMouseX() < Position.X + c->Position.X + c->Size.X &&
		    Engine::Ref().GetMouseY() > Position.Y + c->Position.Y &&
		    Engine::Ref().GetMouseY() < Position.Y + c->Position.Y + c->Size.Y)
		{
			c->MouseInside = true;
			c->OnMouseEnter(
				Engine::Ref().GetMouseX() - (Position.X + c->Position.X),
				Engine::Ref().GetMouseY() - (Position.Y + c->Position.Y)
			);
		}
	}
...
void Window::DoDraw()
{
	...
	for (auto child : Components)
	{
		if ((child != focusedComponent_ && child != hoverComponent) || child->GetParent())
		{
			...
			if (debugMode)
			{
				GetGraphics()->BlendFilledRect(
					RectSized(Position + child->Position, child->Size),
	...
	if (debugMode && focusedComponent_)
	{
		...
		String sizeText = String::Build("Size: ", focusedComponent_->Size.X, ", ", focusedComponent_->Size.Y);
...
void Window::DoTick(float dt)
{
	...
	if (DoesTextInput ||
	    (focusedComponent_ && focusedComponent_->Visible && focusedComponent_->Enabled &&
	     focusedComponent_->DoesTextInput))
	{
	...
	{
		if (Components[i]->Enabled && ui::Engine::Ref().GetMouseX() >= Components[i]->Position.X + Position.X &&
...
void Window::DoMouseDown(int x_, int y_, unsigned button)
{
	...
	for (int i = Components.size() - 1; i > -1 && !halt; --i)
	{
		...
		{
			if (x >= Components[i]->Position.X && y >= Components[i]->Position.Y &&
...
void Window::DoMouseMove(int x_, int y_, int dx, int dy)
{
	...
	for (int i = Components.size() - 1; i > -1 && !halt; --i)
	{
		if (Components[i]->Enabled && Components[i]->Visible)
		{
			...
			if (local.X >= 0 && local.Y >= 0 && local.X < Components[i]->Size.X && local.Y < Components[i]->Size.Y &&
			    !halt)
			{
				// entering?
...
void Window::DoMouseUp(int x_, int y_, unsigned button)
{
	...
	for (int i = Components.size() - 1; i >= 0 && !halt; --i)
	{
		if (Components[i]->Enabled && Components[i]->Visible)
		{
			if (Components[i]->MouseDownInside && x >= Components[i]->Position.X && y >= Components[i]->Position.Y &&
			    x < Components[i]->Position.X + Components[i]->Size.X &&
			    y < Components[i]->Position.Y + Components[i]->Size.Y)
			{
