...
SaveButton::SaveButton(Point position, Point size, SaveInfo *newSave /* non-owning */) :
	SaveButton(position, size)
{
	save = newSave;
	if (save)
	{
...
SaveButton::SaveButton(Point position, Point size, SaveFile *newFile /* non-owning */) :
	SaveButton(position, size)
{
	file = newFile;
	if (file)
	...
	if (!thumbnail)
	{
		if (!triedThumbnail && wantsDraw && ThumbnailRendererTask::QueueSize() < 10)
		{
			...
			if (save)
			{
				if (save->GetGameSave())
				{
					thumbnailRenderer = new ThumbnailRendererTask(
						*save->GetGameSave(), thumbBoxSize, RendererSettings::decorationEnabled, true
					);
				...
				{
					thumbnailRequest =
						std::make_unique<http::ThumbnailRequest>(save->GetID(), save->GetVersion(), thumbBoxSize);
			...
			{
				thumbnailRenderer = new ThumbnailRendererTask(
					*file->GetGameSave(), thumbBoxSize, RendererSettings::decorationEnabled, false
				);
				thumbnailRenderer->Start();
				triedThumbnail = true;
			}
...
void SaveButton::Draw(const Point &screenPos)
{
	...
	if (thumbnail)
	{
		...
		g->BlendImage(
			...
			RectSized(
				screenPos +
					((save && save->id) ? ((space - thumbBoxSize) / 2 - Vec2{ 3, 0 }) : (space - thumbSize) / 2),
				tex->Size()
			)
		);
	}
	...
	{
		g->BlendText(
			screenPos + Vec2{ (Size.X - (Graphics::TextSize("Error loading save").X - 1)) / 2, (Size.Y - 28) / 2 },
			"Error loading save",
			0xB4B4B4_rgb .WithAlpha(255)
		);
	}
	if (save)
	{
		...
		{
			g->DrawRect(
				RectSized(screenPos + Vec2{ -3, 0 } + (Size - thumbBoxSize - Vec2{ 0, 21 }) / 2, thumbBoxSize),
				isMouseInside ? 0xD2E6FF_rgb : 0xB4B4B4_rgb
			);
			g->DrawRect(
				RectSized(
					screenPos + Vec2{ thumbBoxSize.X - 4, 0 } + (Size - thumbBoxSize - Vec2{ 0, 21 }) / 2,
					Vec2{ 7, thumbBoxSize.Y }
				),
				isMouseInside ? 0xD2E6FF_rgb : 0xB4B4B4_rgb
			);
			...
			g->DrawFilledRect(
				RectSized(
					screenPos +
						Vec2{ -3 + thumbBoxSize.X + (Size.X - thumbBoxSize.X) / 2,
			                  1 + (Size.Y - 20 - thumbBoxSize.Y) / 2 },
					Vec2{ 5, (thumbBoxSize.Y + 1) / 2 - 1 }
				),
				0x006B0A_rgb
			);
			g->DrawFilledRect(
				RectSized(
					screenPos + Vec2{ -3 + thumbBoxSize.X + (Size.X - thumbBoxSize.X) / 2, (Size.Y - 20) / 2 },
					Vec2{ 5, thumbBoxSize.Y / 2 - 1 }
				),
				0x6B0A00_rgb
			);
			...
			g->DrawFilledRect(
				RectSized(
					screenPos +
						Vec2{ -2 + thumbBoxSize.X + (Size.X - thumbBoxSize.X) / 2,
			                  (Size.Y - 20) / 2 - voteBarHeightUp },
					Vec2{ 3, voteBarHeightUp }
				),
				0x39BB39_rgb
			); // green
			g->DrawFilledRect(
				RectSized(
					screenPos + Vec2{ -2 + thumbBoxSize.X + (Size.X - thumbBoxSize.X) / 2, (Size.Y - 20) / 2 },
					Vec2{ 3, voteBarHeightDown }
				),
				0xBB3939_rgb
			); // red
		}
		...
		{
			g->DrawRect(
				RectSized(screenPos + (Size - thumbBoxSize - Vec2{ 0, 21 }) / 2, thumbBoxSize),
				isMouseInside ? 0xD2E6FF_rgb : 0xB4B4B4_rgb
			);
		}

		g->BlendText(
			screenPos + Vec2{ (Size.X - (Graphics::TextSize(name).X - 1)) / 2, Size.Y - 21 },
			name,
			(isMouseInside && !isMouseInsideAuthor) ? 0xFFFFFF_rgb .WithAlpha(255) : 0xB4B4B4_rgb .WithAlpha(255)
		);
		g->BlendText(
			screenPos + Vec2{ (Size.X - (Graphics::TextSize(save->userName.FromUtf8()).X - 1)) / 2, Size.Y - 10 },
			save->userName.FromUtf8(),
			isMouseInsideAuthor ? 0xC8E6FF_rgb .WithAlpha(255) : 0x6482A0_rgb .WithAlpha(255)
		);
		if (showVotes) // && !isMouseInside)
		{
			int x = screenPos.X - 7 + (Size.X - thumbBoxSize.X) / 2 + thumbBoxSize.X -
				(Graphics::TextSize(votesBackground).X - 1);
		...
		if (isMouseInside)
		{
			...
			g->BlendText(
				{ x, y }, 0xE026, isMouseInsideHistory ? 0xC86450_rgb .WithAlpha(255) : 0xA04632_rgb .WithAlpha(255)
			);
		}
	...
	else if (file)
	{
		...
		g->BlendText(
			screenPos + Vec2{ (Size.X - (Graphics::TextSize(name).X - 1)) / 2, Size.Y - 21 },
			name,
			isMouseInside ? 0xFFFFFF_rgb .WithAlpha(255) : 0xB4B4B4_rgb .WithAlpha(255)
		);
	}
