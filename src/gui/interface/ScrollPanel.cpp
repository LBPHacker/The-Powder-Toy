...
void ScrollPanel::Draw(const Point &screenPos)
{
	...
	if (maxOffset.Y > 0 && InnerSize.Y > 0)
	{
		...
		g->BlendFilledRect(
			RectSized(screenPos + Vec2{ Size.X - scrollBarWidth, 0 }, { scrollBarWidth, Size.Y }),
			0x7D7D7D_rgb .WithAlpha(100)
		);
		g->DrawFilledRect(
			RectSized(
				screenPos + Vec2{ Size.X - scrollBarWidth, int(scrollPos) }, { scrollBarWidth, int(scrollHeight) + 1 }
			),
			0xFFFFFF_rgb
		);
	}
...
void ScrollPanel::XOnMouseMoved(int x, int y)
{
	if (maxOffset.Y > 0 && InnerSize.Y > 0)
	{
		...
		if (scrollbarSelected)
		{
			...
			{
				auto scrollY =
					int(float(y - scrollbarInitialYClick) / float(Size.Y) * float(InnerSize.Y) + scrollbarInitialYOffset
				    );
