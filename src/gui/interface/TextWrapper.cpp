...
namespace ui
{
int TextWrapper::Update(const String &text, bool do_wrapping, int max_width)
{
	...
	for (auto it = text.begin(); it != text.end(); ++it)
	{
		...
		case ' ':
			...
			else
			{
				...
				records.push_back(wrap_record{ *it, char_width, (int)(it - text.begin()), clear_count, false, false });
		...
		default:
			...
			else
			{
				...
				if (wrap_if_needed(line_width))
				{
					...
					std::rotate(records.begin() + word_begins_at, records.end() - 1, records.end());
					word_begins_at += 1;
	...
	{
		regions.push_back(clickmap_region{
			x, l * FONT_H, record.width, l + 1, Index{ record.raw_index, counter, record.clear_index }
        });
		++counter;
		x += record.width;
		if (record.wraps)
