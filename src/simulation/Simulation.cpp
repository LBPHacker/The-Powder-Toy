...
void Simulation::Load(const GameSave *save, bool includePressure, Vec2<int> blockP) // block coordinates
{
	...
	for (int n = 0; n < NPART && n < save->particlesCount; n++)
	{
		...
		{
			if (!(*(elements[tempPart.type].CreateAllowed))(
					this, -3, int(tempPart.x + 0.5f), int(tempPart.y + 0.5f), tempPart.type
				))
			{
				continue;
			}
	...
	for (std::map<unsigned int, unsigned int>::iterator iter = soapList.begin(), end = soapList.end(); iter != end;
	     ++iter)
	{
		int i = (*iter).second;
...
std::unique_ptr<GameSave> Simulation::Save(bool includePressure, Rect<int> partR) // particle coordinates
{
	...
	if (storedParts && elementCount[PT_SOAP])
	{
		...
		for (std::map<unsigned int, unsigned int>::iterator iter = particleMap.begin(), end = particleMap.end();
		     iter != end;
		     ++iter)
		{
			int i = (*iter).second;
...
int Simulation::FloodINST(int x, int y)
{
	...
	try
	{
		do
		{
			...
			if (y >= CELL + 1 && x1 == x2 && isInst(x1 - 1, y - 1) && isInst(x1, y - 1) && isInst(x1 + 1, y - 1) &&
			...
			else if (y >= CELL + 1)
			{
				for (x = x1; x <= x2; x++)
				{
					...
					{
						if (x == x1 || x == x2 || y >= YRES - CELL - 1 || !isInst(x, y + 1) || isInst(x + 1, y + 1) ||
						    isInst(x - 1, y + 1))
						{
			...
			if (y < YRES - CELL - 1 && x1 == x2 && isInst(x1 - 1, y + 1) && isInst(x1, y + 1) &&
			    isInst(x1 + 1, y + 1) && !isInst(x1 - 1, y + 2) && isInst(x1, y + 2) && !isInst(x1 + 1, y + 2))
			{
			...
			else if (y < YRES - CELL - 1)
			{
				for (x = x1; x <= x2; x++)
				{
					if (isSparkableInst(x, y + 1))
					{
						if (x == x1 || x == x2 || y < 0 || !isInst(x, y - 1) || isInst(x + 1, y - 1) ||
						    isInst(x - 1, y - 1))
						{
							// if at the end of a horizontal section, or if it's a T junction or not a 1px wire crossing
							cs.push(x, y + 1);
	...
	try
	{
		...
		do
		{
			...
			for (int x = x1; x <= x2; x++)
			{
				...
				{
					// Try to move the water to a random position on this line, because there's probably a free location
					// somewhere
...
{
	return bmap[y][x] == WL_DETECT || bmap[y][x] == WL_EWALL || bmap[y][x] == WL_ALLOWLIQUID ||
		bmap[y][x] == WL_WALLELEC || bmap[y][x] == WL_ALLOWALLELEC || bmap[y][x] == WL_EHOLE || bmap[y][x] == WL_STASIS;
}

inline int Simulation::is_wire_off(int x, int y)
{
	return (bmap[y][x] == WL_DETECT || bmap[y][x] == WL_EWALL || bmap[y][x] == WL_ALLOWLIQUID ||
	        bmap[y][x] == WL_WALLELEC || bmap[y][x] == WL_ALLOWALLELEC || bmap[y][x] == WL_EHOLE ||
	        bmap[y][x] == WL_STASIS) &&
		emap[y][x] < 8;
}
...
void Simulation::set_emap(int x, int y)
{
	...
	if (y > 1 && x1 == x2 && is_wire(x1 - 1, y - 1) && is_wire(x1, y - 1) && is_wire(x1 + 1, y - 1) &&
	...
	else if (y > 0)
	{
		for (x = x1; x <= x2; x++)
		{
			...
			{
				if (x == x1 || x == x2 || y >= YCELLS - 1 || is_wire(x - 1, y - 1) || is_wire(x + 1, y - 1) ||
	...
	if (y < YCELLS - 2 && x1 == x2 && is_wire(x1 - 1, y + 1) && is_wire(x1, y + 1) && is_wire(x1 + 1, y + 1) &&
	    !is_wire(x1 - 1, y + 2) && is_wire(x1, y + 2) && !is_wire(x1 + 1, y + 2))
	{
		set_emap(x1, y + 2);
	}
	else if (y < YCELLS - 1)
	{
		for (x = x1; x <= x2; x++)
		{
			if (is_wire_off(x, y + 1))
			{
				if (x == x1 || x == x2 || y < 0 || is_wire(x - 1, y + 1) || is_wire(x + 1, y + 1) ||
				    is_wire(x - 1, y - 1) || !is_wire(x, y - 1) || is_wire(x + 1, y - 1))
				{
					set_emap(x, y + 1);
				}
...
int Simulation::parts_avg(int ci, int ni, int t)
{
	...
	{
		int pmr = pmap[((int)(parts[ci].y + 0.5f) + (int)(parts[ni].y + 0.5f)) / 2]
					  [((int)(parts[ci].x + 0.5f) + (int)(parts[ni].x + 0.5f)) / 2];
		if (pmr)
		{
	...
	{
		int pmr2 = pmap[(int)((parts[ci].y + parts[ni].y) / 2 + 0.5f)]
					   [(int)((parts[ci].x + parts[ni].x) / 2 + 0.5f)]; // seems to be more accurate.
		if (pmr2)
		{
...
int Simulation::eval_move(int pt, int nx, int ny, unsigned *rr) const
{
	...
	if (bmap[ny / CELL][nx / CELL])
	{
		...
		if (bmap[ny / CELL][nx / CELL] == WL_EHOLE && !emap[ny / CELL][nx / CELL] &&
		    !(elements[pt].Properties & TYPE_SOLID) && !(elements[TYP(r)].Properties & TYPE_SOLID))
		{
			return 2;
		}
	}
...
int Simulation::try_move(int i, int x, int y, int nx, int ny)
{
		...
		if (!legacy_enable && parts[i].type == PT_PHOT && r) // PHOT heat conduction
		{
			...
			if (rt < PT_NUM && elements[rt].HeatConduct && (rt != PT_HSWC || parts[ID(r)].life == 10) && rt != PT_FILT)
			{
				parts[i].temp = parts[ID(r)].temp =
					restrict_flt((parts[ID(r)].temp + parts[i].temp) / 2, MIN_TEMP, MAX_TEMP);
			}
		}
		else if ((parts[i].type == PT_NEUT || parts[i].type == PT_ELEC) &&
		         (rt == PT_CLNE || rt == PT_PCLN || rt == PT_BCLN || rt == PT_PBCN))
		{
			if (!parts[ID(r)].ctype)
			{
		...
		case PT_PHOT:
		{
			...
			case PT_C5:
				...
				else if (!parts[ID(r)].ctype && parts[i].ctype & 0xFFFFFFC0)
				{
					...
					parts[ID(r)].tmp =
						(0xFFFF & (int)(parts[i].vx * 255.0f)) | (0xFFFF0000 & (int)(parts[i].vy * 16711680.0f));
					parts[ID(r)].tmp2 = (0xFFFF & (int)((parts[i].x - x) * 255.0f)) |
						(0xFFFF0000 & (int)((parts[i].y - y) * 16711680.0f));
					kill_part(i);
				}
				break;
			case PT_INVIS:
	...
	// else e=1 , we are trying to swap the particles, return 0 no swap/move, 1 is still overlap/move, because the swap
	// takes place later
	...
	case PT_NWHL:
		...
		if (parts[i].type == PT_ANAR)
		{
			...
			{
				parts[ID(r)].temp =
					restrict_flt(parts[ID(r)].temp - (MAX_TEMP - parts[i].temp) / 2, MIN_TEMP, MAX_TEMP);
			}
			kill_part(i);
			return 0;
		}
	...
	case PT_CNCT:
	{
		...
		else if (cnctGravY < 0.0f)
		{
			offsetY--;
		}
		if ((offsetX != 0) != (offsetY != 0) && // Is this a different position (avoid diagonals, doesn't work well)
		    ((nx - x) * offsetX > 0 || (ny - y) * offsetY > 0
		    ) && // Is the destination particle below the moving particle
		    (TYP(pmap[y + offsetY][x + offsetX]) == PT_CNCT || TYP(pmap[y + offsetY][x + offsetX]) == PT_ROCK
	...
	if ((bmap[y / CELL][x / CELL] == WL_EHOLE && !emap[y / CELL][x / CELL]) &&
	    !(bmap[ny / CELL][nx / CELL] == WL_EHOLE && !emap[ny / CELL][nx / CELL]))
	{
		return 0;
	}
	...
	if (r)          // the swap part, if we make it this far, swap
	{
		if (parts[i].type == PT_NEUT)
		{
			...
			{
				return 1; // if the element currently underneath neutron isn't NEUTPENETRATE, don't move anything except
				          // the neutron
			}
...
void Simulation::photoelectric_effect(int nx, int ny) // create sparks from PHOT when hitting PSCN and NSCN
{
	...
	{
		if (TYP(pmap[ny][nx - 1]) == PT_NSCN || TYP(pmap[ny][nx + 1]) == PT_NSCN || TYP(pmap[ny - 1][nx]) == PT_NSCN ||
...
unsigned static direction_to_map(float dx, float dy, int t)
{
	...
	return (dx >= 0) | (((dx + dy) >= 0) << 1) | /*  567  */
		((dy >= 0) << 2) |                       /*  4+0  */
		(((dy - dx) >= 0) << 3) |                /*  321  */
		((dx <= 0) << 4) | (((dx + dy) <= 0) << 5) | ((dy <= 0) << 6) | (((dy - dx) <= 0) << 7);
	/*
	return (dx >= -0.001) |
	       (((dx + dy) >= -0.001) << 1) |     //  567
	       ((dy >= -0.001) << 2) |            //  4+0
...
int Simulation::is_boundary(int pt, int x, int y) const
{
	...
	if (is_blocking(pt, x, y - 1) && is_blocking(pt, x, y + 1) && is_blocking(pt, x - 1, y) &&
...
template Simulation::GetNormalResult Simulation::get_normal_interp<false, const Simulation>(
	const Simulation &sim, int pt, float x0, float y0, float dx, float dy
);
...
// the function for creating a particle, use p=-1 for creating a new particle, -2 is from a brush, or a particle number
// to replace a particle. tv = Type (PMAPBITS bits) + Var (32-PMAPBITS bits), var is usually 0
int Simulation::create_part(int p, int x, int y, int t, int v)
{
	...
	if (t == PT_SPRK && !(p == -2 && elements[TYP(pmap[y][x])].CtypeDraw))
	{
		...
		if (parts[index].temp + 10.0f < 673.0f && !legacy_enable &&
		    (type == PT_METL || type == PT_BMTL || type == PT_BRMT || type == PT_PSCN || type == PT_NSCN ||
		     type == PT_ETRD || type == PT_NBLE || type == PT_IRON))
		{
	...
	{
		// If there is a particle, only allow creation if the new particle can occupy the same space as the existing
		// particle If there isn't a particle but there is a wall, check whether the new particle is allowed to be in it
...
Simulation::PlanMoveResult Simulation::PlanMove(Sim &sim, int i, int x, int y)
{
	...
	else
	{
		...
		fin_x = (int)(fin_xf + 0.5f);
		fin_y = (int)(fin_yf + 0.5f);
		bool closedEholeStart = InBounds(fin_x, fin_y) &&
			(bmap[fin_y / CELL][fin_x / CELL] == WL_EHOLE && !emap[fin_y / CELL][fin_x / CELL]);
		while (1)
		{
			mv -= ISTP;
			fin_xf += dx;
			...
			// block if particle can't move (0), or some special cases where it returns 1 (can_move = 3 but returns 1
			// meaning particle will be eaten) also photons are still blocked (slowed down) by any particle (even ones
			// it can move through), and absorb wall also blocks particles
			int eval = sim.eval_move(t, fin_x, fin_y, NULL);
			if (!eval || (can_move[t][TYP(pmap[fin_y][fin_x])] == 3 && eval == 1) ||
			    (t == PT_PHOT && pmap[fin_y][fin_x]) || bmap[fin_y / CELL][fin_x / CELL] == WL_DESTROYALL ||
	...
	return {
		fin_x, fin_y, clear_x, clear_y, fin_xf, fin_yf, clear_xf, clear_yf, vx, vy,
	};
}

template Simulation::PlanMoveResult
	Simulation::PlanMove<false, const Simulation>(const Simulation &sim, int i, int x, int y);
...
void Simulation::UpdateParticles(int start, int end)
{
	...
	for (auto i = start; i < end && i <= parts_lastActiveIndex; i++)
	{
		if (parts[i].type)
		{
			...
			if (bmap[y / CELL][x / CELL] &&
			    (bmap[y / CELL][x / CELL] == WL_WALL || bmap[y / CELL][x / CELL] == WL_WALLELEC ||
			     bmap[y / CELL][x / CELL] == WL_ALLOWAIR || (bmap[y / CELL][x / CELL] == WL_DESTROYALL) ||
			     ...
			     ) || //&& elements[t].Falldown!=0 && parts[i].type!=PT_FIRE && parts[i].type!=PT_SMKE &&
					...
					if (x + CELL < XRES)
					{
						...
						if (y + CELL < YRES && pv[y / CELL + 1][x / CELL + 1] < 3.5f)
						{
							pv[y / CELL + 1][x / CELL + 1] +=
								elements[t].HotAir * (3.5f - pv[y / CELL + 1][x / CELL + 1]);
						}
					}
				}
			...
			if (elements[t].Diffusion) // the random diffusion that gasses have
			{
				if constexpr (LATENTHEAT)
				{
					...
					parts[i].vx +=
						0.05 * sqrtf(parts[i].temp) * elements[t].Diffusion * (2.0f * rng.uniform01() - 1.0f);
					parts[i].vy +=
						0.05 * sqrtf(parts[i].temp) * elements[t].Diffusion * (2.0f * rng.uniform01() - 1.0f);
				}
				else
			...
			auto nt = 0; // if nt is greater than 1 after this, then there is a particle around the current particle,
			             // that is NOT the current particle's type, for water movement.
			{
				auto j = 0;
			...
			if (!legacy_enable)
			{
				if ((elements[t].Properties & TYPE_LIQUID) && (t != PT_GEL || gel_scale > (1 + rng.between(0, 254))))
				{
					...
					if ((offsetX != x || offsetY != y) && offsetX >= 0 && offsetX < XRES && offsetY >= 0 &&
					    offsetY < YRES)
				...
				{
					cond = t && (t != PT_HSWC || parts[i].life == 10) &&
						rng.chance(int(elements[t].HeatConduct * gel_scale), 250);
				}
				if (cond)
				{
					if (aheat_enable && !(elements[t].Properties & PROP_NOAMBHEAT))
					{
						if constexpr (LATENTHEAT)
						{
							auto c_heat = parts[i].temp * 96.645 / elements[t].HeatConduct * gel_scale *
									std::fabs(elements[t].Weight) +
								hv[y / CELL][x / CELL] * 100 * (pv[y / CELL][x / CELL] - MIN_PRESSURE) /
									(MAX_PRESSURE - MIN_PRESSURE) * 2;
							float c_Cm = 96.645 / elements[t].HeatConduct * gel_scale * std::fabs(elements[t].Weight) +
								100 * (pv[y / CELL][x / CELL] - MIN_PRESSURE) / (MAX_PRESSURE - MIN_PRESSURE) * 2;
							auto pt = c_heat / c_Cm;
					...
					for (auto j = 0; j < 8; j++)
					{
						...
						if (rt && elements[rt].HeatConduct && (rt != PT_HSWC || parts[ID(r)].life == 10) &&
						    ...
						    (t != PT_ELEC || rt != PT_DEUT) && (t != PT_DEUT || rt != PT_ELEC) &&
						    ...
						    (t != PT_FILT || rt != PT_HSWC || parts[ID(r)].tmp != 1))
						{
							...
							if constexpr (LATENTHEAT)
							{
								...
								c_heat += parts[ID(r)].temp * 96.645 / elements[rt].HeatConduct * gel_scale *
									std::fabs(elements[rt].Weight);
					...
					if constexpr (LATENTHEAT)
					{
						...
						{
							pt = (c_heat +
							      parts[i].temp * 96.645 / elements[t].HeatConduct * gel_scale *
							          std::fabs(elements[t].Weight)) /
								(c_Cm + 96.645 / elements[t].HeatConduct * gel_scale * std::fabs(elements[t].Weight));
						}

						c_heat += parts[i].temp * 96.645 / elements[t].HeatConduct * gel_scale *
							std::fabs(elements[t].Weight);
						c_Cm += 96.645 / elements[t].HeatConduct * gel_scale * std::fabs(elements[t].Weight);
					...
					if (((elements[t].Properties & TYPE_LIQUID) &&
					     sd.IsElementOrNone(elements[t].HighTemperatureTransition) &&
					     (elements[elements[t].HighTemperatureTransition].Properties & TYPE_GAS)) ||
					    t == PT_LNTG || t == PT_SLTW)
					{
						ctemph -= 2.0f * pv[y / CELL][x / CELL];
					}
					else if (((elements[t].Properties & TYPE_GAS) &&
					          sd.IsElementOrNone(elements[t].LowTemperatureTransition) &&
					          (elements[elements[t].LowTemperatureTransition].Properties & TYPE_LIQUID)) ||
					         t == PT_WTRV)
					{
					...
					if ((t == PT_ICEI || t == PT_SNOW) &&
					    (!sd.IsElement(parts[i].ctype) || parts[i].ctype == PT_ICEI || parts[i].ctype == PT_SNOW))
					{
					...
					if (elements[t].HighTemperatureTransition != NT && ctemph >= elements[t].HighTemperature)
					{
							...
							if (parts[i].ctype > 0 && parts[i].ctype < PT_NUM && parts[i].ctype != t)
							{
								...
								if (s)
								{
									if constexpr (LATENTHEAT)
									{
										...
										if (elements[t].LatentHeat <=
										    (c_heat - (elements[parts[i].ctype].LowTemperature - dbt) * c_Cm))
										{
											pt = (c_heat - elements[t].LatentHeat) / c_Cm;
											t = parts[i].ctype;
										...
										{
											parts[i].temp = restrict_flt(
												elements[parts[i].ctype].LowTemperature - dbt, MIN_TEMP, MAX_TEMP
											);
											s = 0;
										}
									}
						...
						{
							float pres = std::max(
								(pv[y / CELL][x / CELL] + pv[(y - 2) / CELL][x / CELL] + pv[(y + 2) / CELL][x / CELL] +
							     pv[y / CELL][(x - 2) / CELL] + pv[y / CELL][(x + 2) / CELL]) *
									2.0f,
								0.0f
							);
					...
					else if (elements[t].LowTemperatureTransition != NT && ctempl < elements[t].LowTemperature)
					{
						...
						if (elements[t].LowTemperatureTransition != ST)
						{
							if constexpr (LATENTHEAT)
							{
								if (elements[elements[t].LowTemperatureTransition].LatentHeat >=
								    (c_heat - (elements[t].LowTemperature - dbt) * c_Cm))
								{
						...
						else if (t == PT_LAVA)
						{
							if (parts[i].ctype > 0 && parts[i].ctype < PT_NUM && parts[i].ctype != PT_LAVA &&
							    elements[parts[i].ctype].Enabled)
							{
								...
								{
									// TUNG does its own melting in its update function, so HighTemperatureTransition is
									// not LAVA so it won't be handled by the code for
									// HighTemperatureTransition==PT_LAVA below However, the threshold is stored in
									// HighTemperature to allow it to be changed from Lua
								...
								{
									float pres = std::max(
										(pv[y / CELL][x / CELL] + pv[(y - 2) / CELL][x / CELL] +
									     pv[(y + 2) / CELL][x / CELL] + pv[y / CELL][(x - 2) / CELL] +
									     pv[y / CELL][(x + 2) / CELL]) *
											2.0f,
										0.0f
									);
								...
								{
									s = 0; // freezing point for lava with any other (not listed in ptransitions as
									       // turning into lava) ctype
								}
								if (s)
								{
									t = parts[i].ctype;
					...
					if (s) // particle type change occurred
					{
						...
						if (t == PT_FIRE)
						{
							...
							// idealy transitions should use create_part(i) but some elements rely on properties staying
							// constant and I don't feel like checking each one right now
							parts[i].tmp = 0;
						}
			...
			if ((elements[t].Properties & PROP_CONDUCTS) || t == PT_SPRK)
			{
				...
				if (nx >= 0 && ny >= 0 && nx < XCELLS && ny < YCELLS)
				{
					...
					else if (bmap[ny][nx] == WL_DETECT || bmap[ny][nx] == WL_EWALL || bmap[ny][nx] == WL_ALLOWLIQUID ||
					         bmap[ny][nx] == WL_WALLELEC || bmap[ny][nx] == WL_ALLOWALLELEC || bmap[ny][nx] == WL_EHOLE)
					{
						set_emap(nx, ny);
					}
				}
			...
			if ((elements[t].Explosive & 2) && pv[y / CELL][x / CELL] > 2.5f)
			{
				...
				parts[i].temp = restrict_flt(
					elements[PT_FIRE].DefaultProperties.temp + (elements[t].Flammable / 2), MIN_TEMP, MAX_TEMP
				);
				t = PT_FIRE;
...

			{
				auto s = 1;
				auto gravtot =
					std::abs(gravOut.forceX[Vec2{ x, y } / CELL]) + std::abs(gravOut.forceY[Vec2{ x, y } / CELL]);
				...
				else if (elements[t].LowPressureTransition != NT && pv[y / CELL][x / CELL] < elements[t].LowPressure &&
				         gravtot <= (elements[t].LowPressure / 4.0f))
				{
		...
		killed:
			...
			if (t == PT_STKM || t == PT_STKM2 || t == PT_FIGH)
			{
				...
				if (edgeMode == EDGE_LOOP)
				{
					...
					if (!x_ok ||
					    !y_ok) // when moving from left to right stickmen might be able to fall through solid things,
					           // fix with "eval_move(t, nx+diffx, ny+diffy, NULL)" but then they die instead
					{
			...
			else if (elements[t].Properties & TYPE_ENERGY)
			{
				if (t == PT_PHOT)
				{
					...
					if (eval_move(PT_PHOT, fin_x, fin_y, NULL))
					{
						...
						{
							auto gn = get_normal_interp<true>(
								*this, REFRACT | t, parts[i].x, parts[i].y, parts[i].vx, parts[i].vy
							);
							if (!gn.success)
							{
								kill_part(i);
				...
				else if (!do_move(i, x, y, fin_xf, fin_yf))
				{
					...
					if (gn.success)
					{
						...
						// leave the actual movement until next frame so that reflection of fast particles and
						// refraction happen correctly
					}
					else
					{
						if (t != PT_NEUT)
			...
			else
			{
				...
				if (!do_move(i, x, y, fin_xf, fin_yf))
				{
					...
					else
					{
						...
						if (elements[t].Falldown > 1 && !grav && gravityMode == GRAV_VERTICAL &&
						    parts[i].vy > fabsf(parts[i].vx))
						{
							auto s = 0;
							// stagnant is true if FLAG_STAGNANT was set for this particle in previous frame
							int rt;
							if (!stagnant || nt) // nt is if there is an something else besides the current particle
							                     // type, around the particle
							{
							...
							auto nx = -1, ny = -1;
							for (auto j = clear_x + r; j >= 0 && j >= clear_x - rt && j < clear_x + rt && j < XRES;
							     j += r)
							{
								...
								if (fin_y != clear_y &&
								    (TYP(pmap[clear_y][j]) != t || bmap[clear_y / CELL][j / CELL]) &&
								...
								if (TYP(pmap[clear_y][j]) != t ||
								    (bmap[clear_y / CELL][j / CELL] && bmap[clear_y / CELL][j / CELL] != WL_STREAM))
								{
									break;
								}
							}
							...
							if (s == 1)
							{
								...
								{
									if ((TYP(pmap[j][nx]) != t || bmap[j / CELL][nx / CELL]) &&
									    do_move(i, nx, ny, (float)nx, (float)j))
									{
										break;
									}
									if (TYP(pmap[j][nx]) != t ||
									    (bmap[j / CELL][nx / CELL] && bmap[j / CELL][nx / CELL] != WL_STREAM))
									{
										break;
									}
								}
						...
						else if (elements[t].Falldown > 1 &&
						         fabsf(pGravX * parts[i].vx + pGravY * parts[i].vy) >
						             fabsf(pGravY * parts[i].vx - pGravX * parts[i].vy))
						{
							...
							// clear_xf, clear_yf is the last known position that the particle should almost certainly
							// be able to move to
							nxf = clear_xf;
							nyf = clear_yf;
							auto nx = clear_x;
							auto ny = clear_y;
							// Look for spaces to move horizontally (perpendicular to gravity direction), keep going
							// until a space is found or the number of positions examined = rt
							for (auto j = 0; j < rt; j++)
							{
								...
								if (j)
								{
									...
									// This makes liquid movement a bit less frothy, particularly for balls of liquid in
									// radial gravity. With radial gravity, instead of just moving along a tangent, the
									// attempted movement will follow the curvature a bit better.
								...
								if (TYP(pmap[ny][nx]) != t || bmap[ny / CELL][nx / CELL])
								{
									...
									// A particle of a different type, or a wall, was found. Stop trying to move any
									// further horizontally unless the wall should be completely invisible to particles.
							...
							if (s == 1)
							{
								// The particle managed to move horizontally, now try to move vertically (parallel to
								// gravity direction) Keep going until the particle is blocked (by something that isn't
								// the same element) or the number of positions examined = rt
								clear_x = nx;
								clear_y = ny;
								for (auto j = 0; j < rt; j++)
								{
									...
									// If the space is anything except the same element (a wall, empty space, or
									// occupied by a particle of a different element), try to move into it
									if (TYP(pmap[ny][nx]) != t || bmap[ny / CELL][nx / CELL])
									{
										s = do_move(i, clear_x, clear_y, nxf, nyf);
										if (s || TYP(pmap[ny][nx]) != t || bmap[ny / CELL][nx / CELL] != WL_STREAM)
										{
											break; // found the edge of the liquid and movement into it succeeded, so
											       // stop moving down
										}
									}
								}
		...
		if (parts[i].type)
		{
			...
			if (x >= 0 && y >= 0 && x < XRES && y < YRES)
			{
				...
				else
				{
					...
					// To make particles collide correctly when inside these elements, these elements must not overwrite
					// an existing pmap entry from particles inside them
				...
				if (!(bmap[y / CELL][x / CELL] == WL_STASIS && emap[y / CELL][x / CELL] < 8))
				{
					...
					else
					{
						...
						for (int l = 0; l < 5; ++l)
						{
							...
							if ((ruleset >> (neighbours + 8)) & 1 && ((neighbourList[l] >> 21) & 7) + 1 >= majority &&
							    golnum < golnumToCreate)
							{
	...
	if (!sys_pause || framerender)
	{
		...
		for (y = 0; y < YCELLS; y++)
		{
			for (x = 0; x < XCELLS; x++)
			{
				...
				air->bmap_blockair[y][x] =
					(bmap[y][x] == WL_WALL || bmap[y][x] == WL_WALLELEC || bmap[y][x] == WL_BLOCKAIR ||
				     (bmap[y][x] == WL_EWALL && !emap[y][x]));
				air->bmap_blockairh[y][x] =
					(bmap[y][x] == WL_WALL || bmap[y][x] == WL_WALLELEC || bmap[y][x] == WL_BLOCKAIR ||
				     bmap[y][x] == WL_GRAV || (bmap[y][x] == WL_EWALL && !emap[y][x])) ?
					0x8 :
					0;
			}
		}
		...
		if (elementCount[PT_LOVE] > 0 || elementCount[PT_LOLZ] > 0)
		{
			...
			for (ny = 0; ny < YRES - 4; ny++)
			{
				for (nx = 0; nx < XRES - 4; nx++)
				{
					...
					else if ((ny < 9 || nx < 9 || ny > YRES - 7 || nx > XRES - 10) &&
					         (parts[ID(r)].type == PT_LOVE || parts[ID(r)].type == PT_LOLZ))
					{
						kill_part(ID(r));
					}
