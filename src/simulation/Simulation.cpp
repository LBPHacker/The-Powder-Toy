...
void SimulationImpl::UpdateParticles(int start, int end)
{
	...
	for (auto i = start; i < end && i < parts.active; i++)
	{
		...
		parts[i].vx += elements[t].Advection*vx[{ x/CELL, y/CELL }] + neighbourhood.pGravX;
		parts[i].vy += elements[t].Advection*vy[{ x/CELL, y/CELL }] + neighbourhood.pGravY;
...
bool SimulationImpl::TransitionPhase(int i, const Neighbourhood &neighbourhood)
{
	...
	if (!legacy_enable)
	{
		...
		if (t && !sd.IsHeatInsulator(parts[i]) && rng.chance(int(elements[t].HeatConduct*gel_scale), 250))
		{
			...
			{
				auto dtemp = hv[{ x/CELL, y/CELL }] - parts[i].temp; // Temperature difference
				...
				hv[{ x/CELL, y/CELL }] = restrict_flt(hv[{ x/CELL, y/CELL }] - alpha*dtemp, MIN_TEMP, MAX_TEMP);
			}
