...
bool Simulation::flood_water(int x, int y, int i)
{
	...
	try
	{
		...
		do
		{
			...
			while (x1 >= CELL)
			{
				if (elements[TYP(pmap[{ x1 - 1, y }])].Falldown != 2 || bitmap[(y * XRES) + x1 - 1])
					break;
				x1--;
			}
			while (x2 < XRES-CELL)
			{
				if (elements[TYP(pmap[{ x2 + 1, y }])].Falldown != 2 || bitmap[(y * XRES) + x1 - 1])
					break;
				x2++;
			}
			for (int x = x1; x <= x2; x++)
			...
			if (y >= CELL + 1)
				for (int x = x1; x <= x2; x++)
					if (elements[TYP(pmap[{ x, y - 1 }])].Falldown == 2 && !bitmap[((y - 1) * XRES) + x])
						cs.push(x, y - 1);
			if (y < YRES - CELL - 1)
				for (int x = x1; x <= x2; x++)
					if (elements[TYP(pmap[{ x, y + 1 }])].Falldown == 2 && !bitmap[((y + 1) * XRES) + x])
...
void Simulation::clear_sim(void)
{
	...
	std::fill(parts.begin(), parts.end(), Particle{});
...
int Simulation::create_part(int p, int x, int y, int t, int v)
{
	...
	if (t == PT_SPRK && !(p == -2 && elements[TYP(pmap[{ x, y }])].CtypeDraw))
	{
	...
	if (p == -1)//creating from anything but brush
	{
		...
		if (pmap[{ x, y }] ? (eval_move(t, x, y, NULL) != 2) : (bmap[{ x/CELL, y/CELL }] && eval_move(t, x, y, NULL) == 0))
		{
			return -1;
		}
		if (pfree == -1)
...
void Simulation::GetGravityField(int x, int y, float particleGrav, float newtonGrav, float & pGravX, float & pGravY)
{
	...
	if (newtonGrav)
	{
		pGravX += newtonGrav * gravx[{ x / CELL, y / CELL }];
		pGravY += newtonGrav * gravy[{ x / CELL, y / CELL }];
	}
}
...
void Simulation::create_gain_photon(int pp)//photons from PHOT going through GLOW
{
	...
	parts[i].temp = parts[ID(pmap[{ nx, ny }])].temp;
	parts[i].tmp = 0;
	parts[i].tmp3 = 0;
	parts[i].tmp4 = 0;
	photons[{ nx, ny }] = PMAP(i, PT_PHOT);
...
void Simulation::create_cherenkov_photon(int pp)//photons from NEUT going through GLAS
{
	...
	if (TYP(pmap[{ nx, ny }]) != PT_GLAS && TYP(pmap[{ nx, ny }]) != PT_BGLA)
		return;
...
void Simulation::UpdateParticles(int start, int end)
{
	...
	for (auto i = start; i < end && i <= parts_lastActiveIndex; i++)
	{
		if (parts[i].type)
		{
			...
			if (!legacy_enable)
			{
				...
				if (cond)
				{
					if (aheat_enable && !(elements[t].Properties&PROP_NOAMBHEAT))
					{
						if constexpr (LATENTHEAT)
						{
							auto c_heat = parts[i].temp*96.645/elements[t].HeatConduct*gel_scale*std::fabs(elements[t].Weight) + hv[{ x/CELL, y/CELL }]*100*(pv[{ x/CELL, y/CELL }]-MIN_PRESSURE)/(MAX_PRESSURE-MIN_PRESSURE)*2;
							float c_Cm = 96.645/elements[t].HeatConduct*gel_scale*std::fabs(elements[t].Weight) + 100*(pv[{ x/CELL, y/CELL }]-MIN_PRESSURE)/(MAX_PRESSURE-MIN_PRESSURE)*2;
							auto pt = c_heat/c_Cm;
							pt = restrict_flt(pt, -MAX_TEMP+MIN_TEMP, MAX_TEMP-MIN_TEMP);
							parts[i].temp = pt;
							//Pressure increase from heat (temporary)
							pv[{ x/CELL, y/CELL }] += (pt-hv[{ x/CELL, y/CELL }])*0.004;
							hv[{ x/CELL, y/CELL }] = pt;
						}
						else
						{
			...
			{
				auto s = 1;
				auto gravtot = fabs(gravy[{ x / CELL, y / CELL }])+fabs(gravx[{ x / CELL, y / CELL }]);
	...
	if (!sys_pause || framerender)
	{
		...
		for (y = 0; y < YCELLS; y++)
		{
			for (x = 0; x < XCELLS; x++)
			{
				...
				air->bmap_blockairh[{ x, y }] = (bmap[{ x, y }]==WL_WALL || bmap[{ x, y }]==WL_WALLELEC || bmap[{ x, y }]==WL_BLOCKAIR || bmap[{ x, y }]==WL_GRAV || (bmap[{ x, y }]==WL_EWALL && !emap[{ x, y }])) ? 0x8:0;
			}
		}
