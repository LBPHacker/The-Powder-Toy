...
const std::vector<StructProperty> &Particle::GetProperties()
{
...
const std::vector<StructPropertyAlias> &Particle::GetPropertyAliases()
{
...
const std::vector<unsigned int> &Particle::PossiblyCarriesType()
{
	struct DoOnce
	{
		std::vector<unsigned int> indices = {
			FIELD_LIFE, FIELD_CTYPE, FIELD_TMP, FIELD_TMP2, FIELD_TMP3, FIELD_TMP4,
		};
		...
		DoOnce()
		{
			...
			for (auto index : indices)
			{
				...
				assert(
					properties[index].Type == StructProperty::Integer ||
