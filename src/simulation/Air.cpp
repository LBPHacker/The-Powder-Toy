	...
	for (auto y = 0; y < YCELLS; y++) // update air temp and velocity
	{
		for (auto x = 0; x < XCELLS; x++)
		{
			...
			for (auto j = -1; j < 2; j++)
			{
				for (auto i = -1; i < 2; i++)
				{
					if (y + j > 0 && y + j < YCELLS - 2 && x + i > 0 && x + i < XCELLS - 2 &&
			...
			if ((dx * advDistanceMult > 1.0f || dy * advDistanceMult > 1.0f) &&
			    (tx >= 2 && tx < XCELLS - 2 && ty >= 2 && ty < YCELLS - 2))
			{
				float stepX, stepY;
				int stepLimit;
		...
		for (auto y = 0; y < YCELLS; y++) // update velocity and pressure
		{
			for (auto x = 0; x < XCELLS; x++)
			{
				...
				for (auto j = -1; j < 2; j++)
				{
					for (auto i = -1; i < 2; i++)
					{
						if (y + j > 0 && y + j < YCELLS - 1 && x + i > 0 && x + i < XCELLS - 1 &&
				    ...
				    (tx >= 2 && tx < XCELLS - 2 && ty >= 2 && ty < YCELLS - 2))
				{
					...
					// Step from current position to desired source location, looking for walls, with either the x or y
					// step size being 1 cell
					float stepX, stepY;
					int stepLimit;
					if (std::abs(dx) > std::abs(dy))
					{
