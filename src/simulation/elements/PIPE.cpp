...
void Element::Element_PIPE()
{
	...
	HeatConduct = 0;
	Description =
		"PIPE, moves particles around. Once the BRCK generates, erase some for the exit. Then the PIPE generates and is usable.";
...
{
	if (part.tmp & 0x00000200)
	{
		part.tmp = (part.tmp & 0xFFFFE3FF) | (offsetMap[(part.tmp & 0x00001C00) >> 10] << 10);
	}
	if (part.tmp & 0x00002000)
	{
		part.tmp = (part.tmp & 0xFFFE3FFF) | (offsetMap[(part.tmp & 0x0001C000) >> 14] << 14);
	}
}
...
int Element_PIPE_update(UPDATE_FUNC_ARGS)
{
	...
	if ((parts[i].tmp & PFLAG_COLORS) && !(parts[i].tmp & PPIP_TMPFLAG_PAUSED))
	{
		...
		{
			if (parts[i].flags &
			    PFLAG_NORMALSPEED) // skip particle push to prevent particle number being higher causing speed up
			{
			...
			if (nt) // there is something besides PIPE around current particle
			{
				...
				else if (!TYP(parts[i].ctype) && TYP(r) == PT_STOR && sd.IsElement(parts[ID(r)].tmp) &&
		...
		if (!parts[i].life)
		{
			for (auto rx = -1; rx <= 1; rx++)
			{
				for (auto ry = -1; ry <= 1; ry++)
				{
					if (rx || ry)
					{
						if (!pmap[y + ry][x + rx] && sim->bmap[(y + ry) / CELL][(x + rx) / CELL] != WL_ALLOWAIR &&
						    sim->bmap[(y + ry) / CELL][(x + rx) / CELL] != WL_WALL &&
						    sim->bmap[(y + ry) / CELL][(x + rx) / CELL] != WL_WALLELEC &&
						    (sim->bmap[(y + ry) / CELL][(x + rx) / CELL] != WL_EWALL ||
						     sim->emap[(y + ry) / CELL][(x + rx) / CELL]))
						{
							parts[i].life = 50;
						}
					}
...
int Element_PIPE_graphics(GRAPHICS_FUNC_ARGS)
{
	...
	if (t > 0 && t < PT_NUM && elements[t].Enabled)
	{
		...
		{
			// We emulate the graphics of the stored particle. We need a const Particle *cpart to pass to the graphics
			// function, but we don't have a Particle that is populated the way the graphics function expects, so we
			// construct a temporary one and present that to it.
			//
			// Native graphics functions are well-behaved and use the cpart we give them, no questions asked, so we can
			// just have the Particle on stack. Swapping the pointers in cpart with tpart takes care of passing the
			// particle on stack to the native graphics function. Lua graphics functions are more complicated to
			// appease: they access particle data through the particle ID, so not only do we have to give them a
			// correctly populated Particle, it also has to be somewhere in Simulation. luaGraphicsWrapper takes care of
			// this.
...
static void pushParticle(Simulation *sim, int i, int count, int original)
{
	...
	if (!(sim->parts[i].tmp & 0x200))
	{
		...
		for (auto q = 0; q < 3; q++) // try to push 3 times
		{
			...
			else if ((TYP(r) == PT_PIPE || TYP(r) == PT_PPIP) && (sim->parts[ID(r)].tmp & PFLAG_COLORS) != notctype &&
			         !TYP(sim->parts[ID(r)].ctype))
			{
				...
				for (int nnx = 0; nnx < 80; nnx++)
				{
					if (!sim->portalp[portaltmp][count][nnx].type)
					{
						Element_PIPE_transfer_pipe_to_part(
							sim, sim->parts + i, &(sim->portalp[portaltmp][count][nnx]), false
						);
						count++;
						break;
					}
	...
	else // predefined 1 pixel thick pipe movement
	{
		...
		if ((TYP(r) == PT_PIPE || TYP(r) == PT_PPIP) && (sim->parts[ID(r)].tmp & PFLAG_COLORS) != notctype &&
		    !TYP(sim->parts[ID(r)].ctype))
		{
			transfer_pipe_to_pipe(sim->parts + i, sim->parts + (ID(r)), false);
			if (ID(r) > original)
