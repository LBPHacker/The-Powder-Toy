 ...
 *  - +<----+---->+ - -
 *  - - - - H - - - - -
 * Where H is the head with tail length 4, it checks the + area to see if it can hit any of the edges, then it is called
 * safe, or picks the biggest area if none safe. .tmp bit values: 1st head, 2nd no tail growth, 3rd wait flag, 4th
 * Nodie, 5th Dying, 6th & 7th is direction, 8th - 16th hue, 17th Norandom .tmp2 is tail length (gets longer every few
 * hundred frames) .life is the timer that kills the end of the tail (the head uses life for how often it grows longer)
...
static int update(UPDATE_FUNC_ARGS)
{
	...
	if (parts[i].tmp & TRON_HEAD)
	{
		...
		if (new_tronhead(sim, x + tron_rx[direction], y + tron_ry[direction], i, direction) == -1)
		{
			...
			// trigger tail death for TRON_NODIE, or is that mode even needed? just set a high tail length(but it still
			// won't start dying when it crashes)
		}
	...
	else // fade tail deco, or prevent tail from dying
	{
		...
		// parts[i].dcolour =  clamp_flt((float)parts[i].life/(float)parts[i].tmp2,0,1.0f) << 24 |
		// parts[i].dcolour&0x00FFFFFF;
	}
	return 0;
}
...
static int new_tronhead(Simulation *sim, int x, int y, int i, int direction)
{
	...
	sim->parts[np].tmp = 1 | direction << 5 | (sim->parts[i].tmp & (TRON_NOGROW | TRON_NODIE | TRON_NORANDOM)) |
		(sim->parts[i].tmp & 0xF800);
	if (np > i)
	{
...
static int trymovetron(Simulation *sim, int x, int y, int dir, int i, int len)
{
	...
	for (k = 1; k <= len; k++)
	{
		...
		ry += tron_ry[dir];
		r = sim->pmap[ry][rx];
		if (canmovetron(sim, r, k - 1) && !sim->bmap[(ry) / CELL][(rx) / CELL] && ry >= CELL && rx >= CELL &&
		    ry < YRES - CELL && rx < XRES - CELL)
		{
			count++;
			for (tx = rx - tron_ry[dir], ty = ry - tron_rx[dir], j = 1;
			     abs(tx - rx) < (len - k) && abs(ty - ry) < (len - k);
			     tx -= tron_ry[dir], ty -= tron_rx[dir], j++)
			{
				r = sim->pmap[ty][tx];
				if (canmovetron(sim, r, j + k - 1) && !sim->bmap[(ty) / CELL][(tx) / CELL] && ty >= CELL &&
				    tx >= CELL && ty < YRES - CELL && tx < XRES - CELL)
				{
...
static bool canmovetron(Simulation *sim, int r, int len)
{
	...
	if (!r || (TYP(r) == PT_SWCH && sim->parts[ID(r)].life >= 10) ||
	    (TYP(r) == PT_INVIS && sim->parts[ID(r)].tmp2 == 1))
	{
		return true;
	}
	if ((((elements[TYP(r)].Properties & PROP_LIFE_KILL_DEC) && sim->parts[ID(r)].life > 0) ||
	     ((elements[TYP(r)].Properties & PROP_LIFE_KILL) && (elements[TYP(r)].Properties & PROP_LIFE_DEC))) &&
	    sim->parts[ID(r)].life < len)
	{
		return true;
	}
	return false;
