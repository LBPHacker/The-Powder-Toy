...
int Element_FIRE_update(UPDATE_FUNC_ARGS)
{
	...
	case PT_LAVA:
	{
		...
		else if ((parts[i].ctype == PT_STNE || !parts[i].ctype) && pres >= 30.0f &&
		         (parts[i].temp > elements[PT_ROCK].HighTemperature || pres < elements[PT_ROCK].HighPressure
		         )) // Form ROCK with pressure, if it will stay molten or not immediately break
		{
		...
		for (auto ry = -2; ry <= 2; ry++)
		{
			if (rx || ry)
			{
				...
				if (t == PT_LAVA)
				{
					...
					else if (rt == PT_LAVA && (parts[ID(r)].ctype == PT_METL || parts[ID(r)].ctype == PT_BMTL) &&
					         parts[i].ctype == PT_SLCN)
					{
						parts[i].tmp = 0;
					...
					else if (parts[i].ctype == PT_ROCK && rt == PT_LAVA && parts[ID(r)].ctype == PT_GOLD &&
					         parts[ID(r)].tmp == 0 && sim->pv[y / CELL][x / CELL] >= 50 &&
					         sim->rng.chance(1, 10000)) // Produce GOLD veins/clusters
					{
						parts[i].ctype = PT_GOLD;
				...
				if ((surround_space || elements[rt].Explosive) && elements[rt].Flammable &&
				    sim->rng.chance(
						int(elements[rt].Flammable + (sim->pv[(y + ry) / CELL][(x + rx) / CELL] * 10.0f)), 1000
					) &&
				    // exceptions, t is the thing causing the spark and rt is what's burning
				    (t != PT_SPRK || (rt != PT_RBDM && rt != PT_LRBD && rt != PT_INSL)) &&
				    (t != PT_PHOT || rt != PT_INSL) && (rt != PT_SPNG || parts[ID(r)].life == 0))
				{
					sim->part_change_type(ID(r), x + rx, y + ry, PT_FIRE);
					parts[ID(r)].temp = restrict_flt(
						elements[PT_FIRE].DefaultProperties.temp + (elements[rt].Flammable / 2), MIN_TEMP, MAX_TEMP
					);
		...
		for (auto ry = -2; ry <= 2; ry++)
		{
			if (rx || ry)
			{
				...
				if (sim->bmap[(y + ry) / CELL][(x + rx) / CELL] &&
				    sim->bmap[(y + ry) / CELL][(x + rx) / CELL] != WL_STREAM)
				{
					continue;
				}
				auto rt = TYP(r);
				...
				if (elements[rt].Meltable && ((rt != PT_RBDM && rt != PT_LRBD) || t != PT_SPRK) &&
				    ((t != PT_FIRE && t != PT_PLSM) ||
				     (rt != PT_METL && rt != PT_IRON && rt != PT_ETRD && rt != PT_PSCN && rt != PT_NSCN &&
				      rt != PT_NTCT && rt != PT_PTCT && rt != PT_BMTL && rt != PT_BRMT && rt != PT_SALT &&
				      rt != PT_INWR)) &&
