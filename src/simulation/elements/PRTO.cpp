		...
		if (rx || ry)
		{
			...
			if (!r)
			{
				...
				for (auto nnx = 0; nnx < 80; nnx++)
				{
					...
					if (sim->portalp[parts[i].tmp][randomness][nnx].type ==
					    PT_SPRK) // TODO: make it look better, spark creation
					{
					...
					else if (sim->portalp[parts[i].tmp][randomness][nnx].type)
					{
						...
						auto np =
							sim->create_part(-1, x + rx, y + ry, sim->portalp[parts[i].tmp][randomness][nnx].type);
						if (np < 0)
						{
							if (sim->portalp[parts[i].tmp][randomness][nnx].type == PT_STKM)
							{
						...
						{
							// Release the fighters[] element allocated by create_part, the one reserved when the
							// fighter went into the portal will be used
							sim->fighters[(unsigned char)parts[np].tmp].spwn = 0;
							sim->fighters[(unsigned char)sim->portalp[parts[i].tmp][randomness][nnx].tmp].spwn = 1;
						}
						if (sim->portalp[parts[i].tmp][randomness][nnx].vx == 0.0f &&
						    sim->portalp[parts[i].tmp][randomness][nnx].vy == 0.0f)
						{
							// particles that have passed from PIPE into PRTI have lost their velocity, so use the
							// velocity of the newly created particle if the particle in the portal has no velocity
