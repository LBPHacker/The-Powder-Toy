		...
		for (int ry = -1; ry <= 1; ry++)
		{
			if (rx || ry)
			{
				...
				if (TYP(r) == PT_SPRK && parts[ID(r)].life == 3)
				{
					...
					int colored = 0, rt;
					for (int docontinue = 1, nxx = 0, nyy = 0, nxi = rx * -1, nyi = ry * -1; docontinue;
					     nyy += nyi, nxx += nxi)
					{
						...
						else if (!destroy)
						{
							...
							else if (rt == PT_STOR)
							{
								if (parts[r].tmp)
								{
									...
									for (int ry1 = 1; ry1 >= -1; ry1--)
									{
										...
										{
											int np = sim->create_part(
												-1, x + nxi + nxx + rx1, y + nyi + nyy + ry1, TYP(parts[r].tmp)
											);
											if (np != -1)
											{
							...
							else if (rt != PT_INWR && (rt != PT_SPRK || parts[r].ctype != PT_INWR) && rt != PT_ARAY &&
							         rt != PT_WIFI && !(rt == PT_SWCH && parts[r].life >= 10))
							{
								if (nyy != 0 || nxx != 0)
								{
								...
								if (!(nostop && parts[r].type == PT_SPRK && parts[r].ctype >= 0 &&
								      parts[r].ctype < PT_NUM && (elements[parts[r].ctype].Properties & PROP_CONDUCTS)))
								{
									docontinue = 0;
								}
								else
						...
						else if (destroy)
						{
							...
							else if (rt == PT_STOR || rt == PT_INWR || (rt == PT_SPRK && parts[r].ctype == PT_INWR) ||
							         rt == PT_ARAY || rt == PT_WIFI || rt == PT_FILT ||
							         (rt == PT_SWCH && parts[r].life >= 10))
							{
								if (rt == PT_STOR)
								{
									parts[r].tmp = 0;
