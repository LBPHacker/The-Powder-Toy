...
{
	if ((sim->parts[i].ctype & 2) == 2 && validIndex(sim->parts[i].tmp) &&
	...
	if ((sim->parts[i].ctype & 4) == 4 && validIndex(sim->parts[i].tmp2) &&
	...
	if (parts[i].ctype & 1)
	{
		...
		if (parts[i].temp > FREEZING)
		{
			if (parts[i].life <= 0)
			{
				...
				if (validIndex(parts[i].tmp) && (parts[i].ctype & 6) == 6 && (parts[parts[i].tmp].ctype & 6) == 6 &&
						...
						if (rx || ry)
						{
							...
							if (parts[i].temp > FREEZING)
							{
								if (sim->bmap[(y + ry) / CELL][(x + rx) / CELL] ||
								    (r && !(elements[TYP(r)].Properties & TYPE_GAS) && TYP(r) != PT_SOAP &&
								     TYP(r) != PT_GLAS))
								{
									Element_SOAP_detach(sim, i);
									continue;
		...
		if (parts[i].ctype & 2 && validIndex(parts[i].tmp))
		{
			...
			if ((parts[parts[i].tmp].ctype & 2) && (parts[parts[i].tmp].ctype & 1) &&
			    validIndex(parts[parts[i].tmp].tmp) && (parts[parts[parts[i].tmp].tmp].ctype & 2) &&
			    (parts[parts[parts[i].tmp].tmp].ctype & 1))
			{
