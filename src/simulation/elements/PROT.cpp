...
static int update(UPDATE_FUNC_ARGS)
{
	...
	case PT_DEUT:
		if (sim->rng.chance(-((int)sim->pv[y / CELL][x / CELL] - 4) + (parts[uID].life / 100), 200))
		{
			DeutImplosion(
				...
				x,
				y,
				restrict_flt(parts[uID].temp + parts[uID].life * 500, MIN_TEMP, MAX_TEMP),
				PT_PROT
			);
			sim->kill_part(uID);
		}
	...
	default:
		...
		if (parts[i].temp > 273.15f + 500.0f &&
		    (elements[utype].Flammable || elements[utype].Explosive || utype == PT_BANG))
		{
	...
	// make temp of other things closer to it's own temperature. This will change temp of things that don't conduct, and
	// won't change the PROT's temperature
