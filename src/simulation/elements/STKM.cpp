...
int Element_STKM_run_stickman(playerst *playerp, UPDATE_FUNC_ARGS)
{
	...
	// Death
	if (parts[i].life < 1 ||
	    (sim->pv[y / CELL][x / CELL] >= 4.5f && !playerp->fan)) // If his HP is less than 0 or there is very big wind...
	{
		die(sim, playerp, i);
		return 1;
	}
	...
	if (((int)(playerp->comm) & 0x01) == 0x01)
	{
		...
		if (dl > dr)
		{
			if (InBounds(int(playerp->legs[4]), int(playerp->legs[5])) &&
			    !sim->eval_move(t, int(playerp->legs[4]), int(playerp->legs[5]), NULL))
			{
		...
		{
			if (InBounds(int(playerp->legs[12]), int(playerp->legs[13])) &&
			    !sim->eval_move(t, int(playerp->legs[12]), int(playerp->legs[13]), NULL))
			{
	...
	if (((int)(playerp->comm) & 0x04) == 0x04)
	{
		...
		else if ((InBounds(int(playerp->legs[4]), int(playerp->legs[5])) &&
		          !sim->eval_move(t, int(playerp->legs[4]), int(playerp->legs[5]), NULL)) ||
		         (InBounds(int(playerp->legs[12]), int(playerp->legs[13])) &&
		          !sim->eval_move(t, int(playerp->legs[12]), int(playerp->legs[13]), NULL)))
		{
			parts[i].vx -= 4 * mvx;
			parts[i].vy -= 4 * mvy;
		...
		for (ry = -2; ry < 3; ry++)
		{
			if (x + rx >= 0 && y + ry > 0 && x + rx < XRES && y + ry < YRES && (rx || ry))
			{
				...
				if (TYP(r) == PT_NEUT)
				{
					...
					{
						parts[i].life = int(parts[i].life * 0.9f);
					}
					sim->kill_part(ID(r));
				}
		...
		else
		{
			...
			if (playerp->fan)
			{
				for (int j = -4; j < 5; j++)
				{
					for (int k = -4; k < 5; k++)
					{
						int airx = rx + 3 * ((((int)playerp->pcomm) & 0x02) == 0x02) -
							3 * ((((int)playerp->pcomm) & 0x01) == 0x01) + j;
						int airy = ry + k;
			...
			if ((np < NPART) && np >= 0)
			{
				if (playerp->elem == PT_PHOT)
				{
					...
					else
					{
						...
						{
							parts[np].vx = float(
								(((((int)playerp->pcomm) & 0x02) == 0x02) - (((int)(playerp->pcomm) & 0x01) == 0x01)) *
								random
							);
						}
						else
				...
				{
					parts[np].vx -= -mvy *
						(5 * ((((int)playerp->pcomm) & 0x02) == 0x02) - 5 * (((int)(playerp->pcomm) & 0x01) == 0x01));
					parts[np].vy -= mvx *
						(5 * ((((int)playerp->pcomm) & 0x02) == 0x02) - 5 * (((int)(playerp->pcomm) & 0x01) == 0x01));
	...
	d = 25 / (pow((playerp->legs[0] - playerp->legs[4]), 2) + pow((playerp->legs[1] - playerp->legs[5]), 2) + 25) -
		0.5; // Fast distance
	...
	d = 25 / (pow((playerp->legs[8] - playerp->legs[12]), 2) + pow((playerp->legs[9] - playerp->legs[13]), 2) + 25) -
		0.5;
...
void Element_STKM_interact(Simulation *sim, playerst *playerp, int i, int x, int y)
{
	...
	if (r)
	{
		...
		if (elements[TYP(r)].HeatConduct && (TYP(r) != PT_HSWC || sim->parts[ID(r)].life == 10) &&
		    ((playerp->elem != PT_LIGH && sim->parts[ID(r)].temp >= 323) || sim->parts[ID(r)].temp <= 243) &&
		    (!playerp->rocketBoots || TYP(r) != PT_PLSM))
		{
			damage += 2;
			playerp->accs[3] -= 1;
		}
		...
		if (TYP(r) == PT_PRTI && sim->parts[i].type)
		{
			...
			if (sim->parts[ID(r)].tmp >= CHANNELS)
			{
				sim->parts[ID(r)].tmp = CHANNELS - 1;
			}
		...
		if ((TYP(r) == PT_BHOL || TYP(r) == PT_NBHL) && sim->parts[i].type)
		{
			...
			{
				sim->parts[ID(r)].temp =
					restrict_flt(sim->parts[ID(r)].temp + sim->parts[i].temp / 2, MIN_TEMP, MAX_TEMP);
			}
			sim->kill_part(i);
		}
		if ((TYP(r) == PT_VOID || (TYP(r) == PT_PVOD && sim->parts[ID(r)].life == 10)) &&
		    (!sim->parts[ID(r)].ctype || (sim->parts[ID(r)].ctype == sim->parts[i].type) != (sim->parts[ID(r)].tmp & 1)
		    ) &&
		    sim->parts[i].type)
		{
			sim->kill_part(i);
...
void Element_STKM_set_element(Simulation *sim, playerst *playerp, int element)
{
	auto &sd = SimulationData::CRef();
	auto &elements = sd.elements;
	if (elements[element].Falldown != 0 || elements[element].Properties & TYPE_GAS ||
	    elements[element].Properties & TYPE_LIQUID || elements[element].Properties & TYPE_ENERGY ||
