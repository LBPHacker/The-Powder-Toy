...
static void initDeltaPos()
{
	...
	std::stable_sort(
		deltaPos.begin(),
		deltaPos.end(),
		[](const ETRD_deltaWithLength &a, const ETRD_deltaWithLength &b) {
...
int Element_ETRD_nearestSparkablePart(Simulation *sim, int targetId)
{
	...
	int foundDistance =
		parts[targetId].tmp2 ? std::min(parts[targetId].tmp2, maxDistance) : maxDistance; // tmp2 sets max distance
	int foundI = -1;
	...
	if (sim->etrd_count_valid)
	{
		...
		// If the simulation contains lots of particles, check near the target position first since going through all
		// particles will be slow. Threshold = number of positions checked, *2 because it's likely to access memory all
		// over the place (less cache friendly) and there's extra logic needed
		// TODO: probably not optimal if excessive stacking is used
		if (sim->parts_lastActiveIndex > (int)deltaPos.size() * 2)
		{
			for (std::vector<ETRD_deltaWithLength>::iterator iter = deltaPos.begin(), end = deltaPos.end(); iter != end;
			     ++iter)
			{
				...
				{
					// deltaPos is sorted in order of ascending length, so foundDistance < checkDistance means all later
					// items are further away.
					break;
				}
				if (InBounds(checkPos.X, checkPos.Y) && checkDistance <= foundDistance)
				{
					int r = sim->pmap[checkPos.Y][checkPos.X];
					if (r && TYP(r) == PT_ETRD && !parts[ID(r)].life && ID(r) != targetId &&
					    checkDistance < foundDistance)
					{
		...
		if (foundI < 0)
		{
			for (int i = 0; i <= sim->parts_lastActiveIndex; i++)
			{
				if (parts[i].type == PT_ETRD && !parts[i].life)
				{
					...
					if (checkDistance < foundDistance && checkDistance > parts[targetId].tmp &&
					    i != targetId) // tmp sets min distance
					{
						foundDistance = checkDistance;
						foundI = i;
					}
