...
void Element::Element_LDTC()
{
	...
	HeatConduct = 0;
	Description =
		"Linear detector. Scans in 8 directions for particles with its ctype and creates a spark on the opposite side.";
...
//  0x01: Inverts the CTYPE filter so that the element in ctype is the only thing that doesn't trigger LDTC, instead of
//  the opposite. 0x02: Ignore energy particles 0x04: Ignore FILT (do not use color copying mode) 0x08: Keep searching
//  even after finding a particle
...
static int update(UPDATE_FUNC_ARGS)
{
	int ctype = TYP(parts[i].ctype), ctypeExtra = ID(parts[i].ctype), detectLength = parts[i].tmp,
		detectSpaces = parts[i].tmp2;
	...
	for (int rx = -1; rx <= 1; rx++)
	{
		for (int ry = -1; ry <= 1; ry++)
		{
			if (rx || ry)
			{
				...
				for (; !parts[i].tmp || (xStep * (xCurrent - x) <= maxRange && yStep * (yCurrent - y) <= maxRange);
				     xCurrent += xStep, yCurrent += yStep)
				{
					...
					if (filtMode)
					{
						...
						int nx = x + rx, ny = y + ry;
						int photonWl =
							TYP(rr) == PT_FILT ? Element_FILT_getWavelengths(&parts[ID(rr)]) : parts[ID(rr)].ctype;
						while (TYP(r) == PT_FILT)
						{
