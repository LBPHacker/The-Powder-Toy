...
static void create_line_par(
	Simulation *sim, int x1, int y1, int x2, int y2, int c, float temp, int life, int tmp, int tmp2, int i
);
...
static int update(UPDATE_FUNC_ARGS)
{
	...
	for (auto rx = -2; rx <= 2; rx++)
	{
		for (auto ry = -2; ry <= 2; ry++)
		{
			if (rx || ry)
			{
				...
				auto rt = TYP(r);
				if ((surround_space || elements[rt].Explosive) && (rt != PT_SPNG || parts[ID(r)].life == 0) &&
				    elements[rt].Flammable &&
				    sim->rng.chance(
						elements[rt].Flammable + int(sim->pv[(y + ry) / CELL][(x + rx) / CELL] * 10.0f), 1000
					))
				{
				...
				if (elements[TYP(r)].HeatConduct)
				{
					parts[ID(r)].temp = restrict_flt(parts[ID(r)].temp + powderful / 1.3, MIN_TEMP, MAX_TEMP);
				}
			}
		}
	}
	...
	if (parts[i].tmp2 == 2) // && pNear == -1)
	{
		...
		create_line_par(
			sim, x, y, x + rx, y + ry, PT_LIGH, parts[i].temp, parts[i].life, int(angle2), parts[i].tmp2, i
		);
	}
...
static bool create_LIGH(Simulation *sim, int x, int y, int c, float temp, int life, int tmp, int tmp2, bool last, int i)
{
	...
	else if (x >= 0 && x < XRES && y >= 0 && y < YRES)
	{
		...
		if (((TYP(r) == PT_VOID || (TYP(r) == PT_PVOD && sim->parts[ID(r)].life >= 10)) &&
		     (!sim->parts[ID(r)].ctype || (sim->parts[ID(r)].ctype == c) != (sim->parts[ID(r)].tmp & 1))) ||
		    TYP(r) == PT_BHOL || TYP(r) == PT_NBHL) // VOID, PVOD, VACU, and BHOL eat LIGH here
		{
			return true;
		}
	}
...
static void create(ELEMENT_CREATE_FUNC_ARGS)
{
	...
	sim->parts[i].tmp =
		(static_cast<int>(atan2f(-gy, gx) * (180.0f / TPT_PI_FLT)) + sim->rng.between(-20, 20) + 360) % 360;
	sim->parts[i].tmp2 = 4;
}
