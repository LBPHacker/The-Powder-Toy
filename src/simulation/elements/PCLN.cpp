	...
	if (parts[i].ctype <= 0 || parts[i].ctype >= PT_NUM || !elements[parts[i].ctype].Enabled)
	{
		for (auto rx = -1; rx <= 1; rx++)
		{
			for (auto ry = -1; ry <= 1; ry++)
			{
				...
				auto rt = TYP(r);
				if (rt != PT_CLNE && rt != PT_PCLN && rt != PT_BCLN && rt != PT_SPRK && rt != PT_NSCN &&
				    rt != PT_PSCN && rt != PT_STKM && rt != PT_STKM2 && rt != PT_PBCN && rt < PT_NUM)
				{
					parts[i].ctype = rt;
				...
				for (auto ry = -1; ry <= 1; ry++)
				{
					if (rx || ry)
					{
						...
						if (r != -1)
						{
							...
							if (r > i)
							{
								// Make sure movement doesn't happen until next frame, to avoid gaps in the beams of
								// photons produced
