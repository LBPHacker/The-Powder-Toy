				...
				if (!stopgrow) // try to grow
				{
					if (!pmap[y + sry][x + srx] && parts[i].tmp != 0)
					{
						...
						if (np > -1)
						{
							...
							if (t == PT_PQRT)
							{
								// If PQRT is stationary and has started growing particles of QRTZ, the PQRT is
								// basically part of a new QRTZ crystal. So turn it back into QRTZ so that it behaves
								// more like part of the crystal.
