		...
		for (auto ry = -rd; ry < rd + 1; ry++)
		{
			if (x + rx >= 0 && y + ry >= 0 && x + rx < XRES && y + ry < YRES && (rx || ry))
			{
				...
				if (TYP(r) == parts[i].ctype &&
				    (parts[i].ctype != PT_LIFE || parts[i].tmp == parts[ID(r)].ctype || !parts[i].tmp))
				{
					parts[i].life = 1;
				}
				if (TYP(r) == PT_PHOT || (TYP(r) == PT_BRAY && parts[ID(r)].tmp != 2) || TYP(r) == PT_BIZR ||
				    TYP(r) == PT_BIZRG || TYP(r) == PT_BIZRS)
				{
					setFilt = true;
