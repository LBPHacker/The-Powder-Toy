...
{
	void operator()(std::complex<float> ptr[]) const
	{
		fftwf_free(ptr);
	}
};
...
{
	void operator()(fftwf_plan ptr) const
	{
...
{
	// select best algorithm, could use FFTW_PATIENT or FFTW_EXHAUSTIVE but that increases the time taken to plan, and I
	// don't see much increase in execution speed
	...
	massForward = FftwPlanPtr(fftwf_plan_dft_r2c_2d(
		blocks.Y, blocks.X, massBig.get(), reinterpret_cast<fftwf_complex *>(massBigT.get()), fftwPlanFlags
	));
	forceXInverse = FftwPlanPtr(fftwf_plan_dft_c2r_2d(
		blocks.Y, blocks.X, reinterpret_cast<fftwf_complex *>(forceXBigT.get()), forceXBig.get(), fftwPlanFlags
	));
	forceYInverse = FftwPlanPtr(fftwf_plan_dft_c2r_2d(
		blocks.Y, blocks.X, reinterpret_cast<fftwf_complex *>(forceYBigT.get()), forceYBig.get(), fftwPlanFlags
	));
	...
	auto kernelXForward = FftwPlanPtr(fftwf_plan_dft_r2c_2d(
		blocks.Y, blocks.X, kernelXRaw.get(), reinterpret_cast<fftwf_complex *>(kernelXT.get()), fftwPlanFlags
	));
	auto kernelYForward = FftwPlanPtr(fftwf_plan_dft_r2c_2d(
		blocks.Y, blocks.X, kernelYRaw.get(), reinterpret_cast<fftwf_complex *>(kernelYT.get()), fftwPlanFlags
	));
