...
std::unique_ptr<Snapshot> Simulation::CreateSnapshot() const
{
	...
	snap->PortalParticles.insert(
		snap->PortalParticles.begin(), &portalp[0][0][0], &portalp[0][0][0] + CHANNELS * 8 * 80
	);
...
void Simulation::Restore(const Snapshot &snap)
{
	...
	player2 = snap.stickmen[snap.stickmen.size() - 2];
	{
		...
		// we apply the old grav values but Newtonian gravity enable state is not part of the snapshot so this may be
		// pointless
...
void Simulation::clear_area(int area_x, int area_y, int area_w, int area_h)
{
	...
	{
		if (signs[i].text.length() && signs[i].x >= area_x && signs[i].y >= area_y && signs[i].x <= area_x + area_w &&
		    signs[i].y <= area_y + area_h)
		{
...
int Simulation::CreateWalls(int x, int y, int rx, int ry, int wall, const Brush *cBrush)
{
	if (cBrush)
	{
			...
			if (wallX >= 0 && wallX < XCELLS && wallY >= 0 && wallY < YCELLS)
			{
				...
				else if (wall == WL_STREAM)
				{
					...
					for (int tempY = wallY - 1; tempY < wallY + 2; tempY++)
					{
						...
						{
							if (tempX >= 0 && tempX < XCELLS && tempY >= 0 && tempY < YCELLS &&
							    bmap[tempY][tempX] == WL_STREAM)
							{
								return 1;
							}
						}
				...
				if (wall == WL_ERASEALL)
				{
					...
					for (int i = signs.size() - 1; i >= 0; i--)
					{
						if (signs[i].x >= wallX * CELL && signs[i].y >= wallY * CELL &&
						    signs[i].x <= (wallX + 1) * CELL && signs[i].y <= (wallY + 1) * CELL)
						{
							signs.erase(signs.begin() + i);
						}
					}
...
void Simulation::CreateWallLine(int x1, int y1, int x2, int y2, int rx, int ry, int wall, const Brush *cBrush)
{
	int x, y, dx, dy, sy;
...
void Simulation::ApplyDecorationPoint(
	int positionX, int positionY, int colR, int colG, int colB, int colA, int mode, const Brush &cBrush
)
{
...
void Simulation::ApplyDecorationLine(
...
void Simulation::ApplyDecorationFill(
...
int Simulation::CreateParts(int positionX, int positionY, int c, const Brush &cBrush, int flags)
{
	if (flags == -1)
	{
...
void Simulation::CreateLine(int x1, int y1, int x2, int y2, int c, const Brush &cBrush, int flags)
{
...
int Simulation::FloodParts(int x, int y, int fullc, int cm, int flags)
{
	...
	do
	{
		...
		{
			if (bitmap[(y * XRES) + x1 - 1] || !FloodFillPmapCheck(x1 - 1, y, cm) ||
			    (c != 0 && IsWallBlocking(x1 - 1, y, c)))
			{
				break;
			}
			x1--;
		...
		{
			if (bitmap[(y * XRES) + x2 + 1] || !FloodFillPmapCheck(x2 + 1, y, cm) ||
			    (c != 0 && IsWallBlocking(x2 + 1, y, c)))
			{
				break;
			}
			x2++;
		...
		if (c ? y >= CELL + dy : y >= dy)
		{
			for (x = x1; x <= x2; x++)
			{
				if (!bitmap[((y - dy) * XRES) + x] && FloodFillPmapCheck(x, y - dy, cm) &&
				    (c == 0 || !IsWallBlocking(x, y - dy, c)))
				{
					coord_stack[coord_stack_size][0] = x;
