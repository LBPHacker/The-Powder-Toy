...
VideoBuffer::VideoBuffer(const pixel *data, Vec2<int> size) :
	VideoBuffer(size)
{
	std::copy_n(data, size.X * size.Y, video.data());
}

VideoBuffer::VideoBuffer(const pixel *data, Vec2<int> size, size_t rowStride) :
	VideoBuffer(size)
{
...
void VideoBuffer::Crop(Rect<int> rect)
{
	...
	{
		std::copy_n(video.RowIterator(rect.pos + Vec2(0, y)), newVideo.Size().X, newVideo.RowIterator(Vec2(0, y)));
	}
	...
	if (resample)
	{
		...
		for (auto &ptr : resamplers)
		{
			ptr = std::make_unique<Resampler>(
				...
				Size().Y, // source size
				size.X,
				size.Y, // destination size
				Resampler::BOUNDARY_CLAMP,
				0.0f,
				255.0f, // upper and lower bounds for channel values
				"lanczos12",
				clist_x,
				clist_y,
				0.75f,
				0.75f // X and Y filter scales, values < 1.0 cause aliasing, but create sharper looking mips.
			);
		...
		for (int sourceY = 0; sourceY < Size().Y; sourceY++)
		{
			...
			for (int c = 0; c < PIXELCHANNELS; c++)
			{
				...
				while (const float *output = resamplers[c]->get_line())
				{
...
std::unique_ptr<VideoBuffer> VideoBuffer::FromPNG(const std::vector<char> &data)
{
...
void Graphics::RenderZoom()
{
	...
	}
	{
		...
		DrawFilledRect(
			RectSized(zoomWindowPosition, { zoomScopeSize * ZFACTOR, zoomScopeSize * ZFACTOR }), 0x000000_rgb
		);
		DrawRect(
			RectSized(
				zoomWindowPosition - Vec2{ 2, 2 }, Vec2{ zoomScopeSize * ZFACTOR + 3, zoomScopeSize * ZFACTOR + 3 }
			),
			0xC0C0C0_rgb
		);
		DrawRect(
			RectSized(
				zoomWindowPosition - Vec2{ 1, 1 }, Vec2{ zoomScopeSize * ZFACTOR + 1, zoomScopeSize * ZFACTOR + 1 }
			),
			0x000000_rgb
		);
