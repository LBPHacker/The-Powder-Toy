...
bool FontReader::MergeExternal(ByteString path)
{
	...
	// read bdf
	{
		...
		while (std::getline(f, line_buf))
		{
			...
			if (!(ss >> first))
			{
				std::cerr << __func__ << ": line " << line_cnt << ": something is up with the format of the line" << std::endl;
				continue;
			}
			...
			if (in_char)
			{
				if (first == "encoding")
				{
					if (!(ss >> code_point))
					{
						std::cerr << __func__ << ": line " << line_cnt << ": something is up with the format of encoding" << std::endl;
						continue;
					}
					if (code_point >= unicode_max)
					{
						std::cerr << __func__ << ": line " << line_cnt << ": out of bounds" << std::endl;
						continue;
					}
					code_point_ok = true;
				}
				else if (first == "dwidth")
				{
					...
					{
						std::cerr << __func__ << ": line " << line_cnt << ": something is up with the format of dwidth" << std::endl;
						continue;
					}
					if (dwidth_y != 0)
					{
						std::cerr << __func__ << ": line " << line_cnt << ": step vector isn't parallel to x axis" << std::endl;
						continue;
					}
					if (dwidth_x > 32)
					{
						std::cerr << __func__ << ": line " << line_cnt << ": dwidth too wide" << std::endl;
						continue;
					}
					dwidth_ok = true;
				}
				else if (first == "bbx")
				{
					...
					{
						std::cerr << __func__ << ": line " << line_cnt << ": something is up with the format of bbx" << std::endl;
						continue;
					}
					if (bbox_w > 32)
					{
						std::cerr << __func__ << ": line " << line_cnt << ": bbox too wide" << std::endl;
						continue;
					}
					bbox_ok = true;
				...
				else if (first == "endchar")
				{
					if (!code_point_ok)
					{
						std::cerr << __func__ << ": line " << line_cnt << ": encoding is bad" << std::endl;
					}
					else if (!dwidth_ok)
					{
						std::cerr << __func__ << ": line " << line_cnt << ": dwidth is bad" << std::endl;
					}
					else if (!bbox_ok)
					{
						std::cerr << __func__ << ": line " << line_cnt << ": bbox is bad" << std::endl;
					}
				...
				else if (first == "bitmap")
				{
					if (!bbox_ok)
					{
						std::cerr << __func__ << ": line " << line_cnt << ": no bbox yet" << std::endl;
						continue;
					}
