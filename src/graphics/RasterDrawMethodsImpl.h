...
static inline void
	drawPixelUnchecked(RasterDrawMethods<Derived> &self, V Derived::*video, Vec2<int> pos, RGB<uint8_t> colour)
{
...
template<typename Derived, typename V>
static inline void
	blendPixelUnchecked(RasterDrawMethods<Derived> &self, V Derived::*video, Vec2<int> pos, RGBA<uint8_t> colour)
{
...
{
	pixel &px = (static_cast<Derived &>(self).*video)[pos];
	const auto c = RGB<uint8_t>::Unpack(px);
...
{
	RasterizeEllipsePoints(
		Vec2(float(size.X * size.X), float(size.Y * size.Y)),
		[this, center, colour](Vec2<int> delta) {
...
{
	RasterizeEllipseRows(
		Vec2(float(size.X * size.X), float(size.Y * size.Y)),
		[this, center, colour](int xLim, int dy) {
...
template<typename Derived>
void RasterDrawMethods<Derived>::BlendImage(const pixel *data, uint8_t alpha, Rect<int> rect)
{
	BlendImage(data, alpha, rect, rect.size.X);
}

template<typename Derived>
void RasterDrawMethods<Derived>::BlendImage(const pixel *data, uint8_t alpha, Rect<int> rect, size_t rowStride)
{
	...
	else
	{
		for (auto pos : rect)
		{
			const pixel px = data[(pos.X - origin.X) + (pos.Y - origin.Y) * rowStride];
...
template<typename Derived>
void RasterDrawMethods<Derived>::XorImage(const unsigned char *data, Rect<int> rect)
{
	XorImage(data, rect, rect.size.X);
}

template<typename Derived>
void RasterDrawMethods<Derived>::XorImage(const unsigned char *data, Rect<int> rect, size_t rowStride)
{
	auto origin = rect.pos;
	rect &= clipRect();
	for (auto pos : rect)
...
template<typename Derived>
void RasterDrawMethods<Derived>::BlendRGBAImage(const pixel_rgba *data, Rect<int> rect)
{
	BlendRGBAImage(data, rect, rect.size.X);
}

template<typename Derived>
void RasterDrawMethods<Derived>::BlendRGBAImage(const pixel_rgba *data, Rect<int> rect, size_t rowStride)
{
	auto origin = rect.pos;
	rect &= clipRect();
	for (auto pos : rect)
...
int RasterDrawMethods<Derived>::BlendChar(Vec2<int> pos, String::value_type ch, RGBA<uint8_t> colour)
{
	...
	const auto rect = RectSized(Vec2(0, -2), Vec2(reader.GetWidth(), FONT_H));
...
{
	FontReader reader(ch);
	const RGB<uint8_t> c = colour.NoAlpha();
	const auto rect = RectSized(Vec2(0, -2), Vec2(reader.GetWidth(), FONT_H));
	for (auto off : rect.template Range<TOP_TO_BOTTOM, LEFT_TO_RIGHT>())
	{
...
template<typename Derived>
Vec2<int> RasterDrawMethods<Derived>::BlendText(Vec2<int> orig_pos, const String &str, RGBA<uint8_t> orig_colour)
{
...
template<typename Derived>
Vec2<int> RasterDrawMethods<Derived>::BlendTextOutline(Vec2<int> pos, const String &str, RGBA<uint8_t> colour)
{
...
template<typename Derived>
Vec2<int> RasterDrawMethods<Derived>::TextSize(const String &str)
{
...
template<typename Derived>
String::const_iterator RasterDrawMethods<Derived>::TextFit(const String &str, int width)
{
	int curX = 0;
	for (size_t i = 0; i < str.length(); i++)
	{
