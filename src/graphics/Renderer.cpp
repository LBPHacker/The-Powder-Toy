...
void Renderer::draw_air()
{
	...
	for (y=0; y<YCELLS; y++)
		for (x=0; x<XCELLS; x++)
		{
			...
			{
				if (pv[{ x, y }] > 0.0f)
					c = RGB<uint8_t>(clamp_flt(pv[{ x, y }], 0.0f, 8.0f), 0, 0);//positive pressure is red!
				else
					c = RGB<uint8_t>(0, 0, clamp_flt(-pv[{ x, y }], 0.0f, 8.0f));//negative pressure is blue!
			}
			else if (display_mode & DISPLAY_AIRV)
			{
				c = RGB<uint8_t>(clamp_flt(fabsf(vx[{ x, y }]), 0.0f, 8.0f),//vx adds red
			...
			{
				c = RGB<uint8_t>::Unpack(HeatToColour(hv[{ x, y }]));
				//c = RGB<uint8_t>(clamp_flt(fabsf(vx[{ x, y }]), 0.0f, 8.0f),//vx adds red
				//	clamp_flt(hv[{ x, y }], 0.0f, 1600.0f),//heat adds green
				//	clamp_flt(fabsf(vy[{ x, y }]), 0.0f, 8.0f)).Pack();//vy adds blue
			}
