...
void Renderer::RenderSimulation()
{
	...
	{
		std::transform(
			video.RowIterator({ 0, 0 }),
			video.RowIterator({ 0, YRES }),
			persistentVideo.begin(),
			[](pixel p) {
...
void Renderer::render_gravlensing(const RendererFrame &source)
{
	...
	{
		auto cp = p / CELL;
		auto rp = Vec2{ int(p.X - sim->gravOut.forceX[cp] * 0.75f + 0.5f),
			            int(p.Y - sim->gravOut.forceY[cp] * 0.75f + 0.5f) };
		auto gp = Vec2{ int(p.X - sim->gravOut.forceX[cp] * 0.875f + 0.5f),
			            int(p.Y - sim->gravOut.forceY[cp] * 0.875f + 0.5f) };
		auto bp = Vec2{ int(p.X - sim->gravOut.forceX[cp] + 0.5f), int(p.Y - sim->gravOut.forceY[cp] + 0.5f) };
		if (RES.OriginRect().Contains(rp) && RES.OriginRect().Contains(gp) && RES.OriginRect().Contains(bp))
		{
...
std::unique_ptr<VideoBuffer> Renderer::WallIcon(int wallID, Vec2<int> size)
{
	...
	const wall_type &wtype = wtypes[wallID];
	...
	for (i = 0; i <= sim->parts_lastActiveIndex; i++)
	{
		if (sim->parts[i].type && sim->parts[i].type >= 0 && sim->parts[i].type < PT_NUM)
		{
...

			{
				...
				if ((elements[t].Properties & PROP_HOT_GLOW) &&
				    sim->parts[i].temp > (elements[t].HighTemperature - 800.0f))
				{
					...
					auto caddress =
						int((sim->parts[i].temp > elements[t].HighTemperature) ?
				...
				if (colorMode & COLOUR_HEAT)
				{
					...
					firea = 255;
					RGB<uint8_t> color =
						heatTableAt(int((sim->parts[i].temp - min_temp) / (max_temp - min_temp) * 1024));
				...
				if (findingElement)
				{
					...
					else
					{
						...
						case StructProperty::Integer:
							matchesFindingElement = *((int *)(((char *)&sim->parts[i]) + findingElement->property.Offset
							                        )) == std::get<int>(findingElement->value);
							break;

						case StructProperty::UInteger:
							matchesFindingElement =
								*((unsigned int *)(((char *)&sim->parts[i]) + findingElement->property.Offset)) ==
								std::get<unsigned int>(findingElement->value);
							break;
				...
				if (pixel_mode & EFFECT_LINES)
				{
					if (t == PT_SOAP)
					{
						if ((parts[i].ctype & 3) == 3 && parts[i].tmp >= 0 && parts[i].tmp < NPART)
						{
							BlendLine(
								{ nx, ny },
								{ int(parts[parts[i].tmp].x + 0.5f), int(parts[parts[i].tmp].y + 0.5f) },
								RGBA<uint8_t>(colr, colg, colb, cola)
							);
						}
					}
				}
				if (pixel_mode & PSPEC_STICKMAN)
				{
					...
					if (mousePos.X > (nx - 3) && mousePos.X < (nx + 3) && mousePos.Y < (ny + 3) &&
					    mousePos.Y > (ny - 3)) // If mouse is in the head
					{
						String hp = String::Build(Format::Width(sim->parts[i].life, 3));
						BlendText(
							mousePos + Vec2{ -8 - 2 * (sim->parts[i].life < 100) - 2 * (sim->parts[i].life < 10), -12 },
							hp,
							0xFFFFFF_rgb .WithAlpha(255)
						);
					}
					...
					// legs
					DrawLine(
						{ nx, ny + 3 }, { int(cplayer->legs[0]), int(cplayer->legs[1]) }, RGB<uint8_t>(legr, legg, legb)
					);
					DrawLine(
						{ int(cplayer->legs[0]), int(cplayer->legs[1]) },
						{ int(cplayer->legs[4]), int(cplayer->legs[5]) },
						RGB<uint8_t>(legr, legg, legb)
					);
					DrawLine(
						{ nx, ny + 3 }, { int(cplayer->legs[8]), int(cplayer->legs[9]) }, RGB<uint8_t>(legr, legg, legb)
					);
					DrawLine(
						{ int(cplayer->legs[8]), int(cplayer->legs[9]) },
						{ int(cplayer->legs[12]), int(cplayer->legs[13]) },
						RGB<uint8_t>(legr, legg, legb)
					);
				...
				if (pixel_mode & PMODE_FLARE)
				{
					...
					BlendPixel(
						{ nx + 1, ny }, RGBA<uint8_t>(colr, colg, colb, int((gradv * 2) > 255 ? 255 : (gradv * 2)))
					);
					BlendPixel(
						{ nx - 1, ny }, RGBA<uint8_t>(colr, colg, colb, int((gradv * 2) > 255 ? 255 : (gradv * 2)))
					);
					BlendPixel(
						{ nx, ny + 1 }, RGBA<uint8_t>(colr, colg, colb, int((gradv * 2) > 255 ? 255 : (gradv * 2)))
					);
					BlendPixel(
						{ nx, ny - 1 }, RGBA<uint8_t>(colr, colg, colb, int((gradv * 2) > 255 ? 255 : (gradv * 2)))
					);
				...
				if (pixel_mode & EFFECT_GRAVIN)
				{
					...
					for (r = 0; r < 4; r++)
					{
						...
						if (ny + nyo > 0 && ny + nyo < YRES && nx + nxo > 0 && nx + nxo < XRES &&
						    TYP(sim->pmap[ny + nyo][nx + nxo]) != PT_PRTI)
						{
				...
				if (pixel_mode & EFFECT_GRAVOUT)
				{
					...
					for (r = 0; r < 4; r++)
					{
						...
						nyo = (int)(ddist * sin(drad));
						if (ny + nyo > 0 && ny + nyo < YRES && nx + nxo > 0 && nx + nxo < XRES &&
						    TYP(sim->pmap[ny + nyo][nx + nxo]) != PT_PRTO)
						{
							AddPixel({ nx + nxo, ny + nyo }, RGBA<uint8_t>(colr, colg, colb, 255 - orbd[r]));
						}
					}
...
void Renderer::draw_air()
{
	...
	for (y = 0; y < YCELLS; y++)
	{
		for (x = 0; x < XCELLS; x++)
		{
			...
			{
				c = RGB<uint8_t>(
					clamp_flt(fabsf(vx[y][x]), 0.0f, 8.0f), // vx adds red
