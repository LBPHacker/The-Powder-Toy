...
// June 4, 2012: v2.21: Switched to unlicense.org, integrated GCC fixes supplied by Peter Nagy <petern@crytek.com>,
// Anteru at anteru.net, and clay@coge.net, added Codeblocks project (for testing with MinGW and GCC), VS2008 static
// code analysis pass.
...
static Resample_Real kaiser_filter(Resample_Real t)
{
	...
	if (t < KAISER_SUPPORT)
	{
		...
		const Resample_Real alpha =
			(Resample_Real)(exp(log((double)0.58417 * (att - 20.96)) * 0.4) + 0.07886 * (att - 20.96));
...
# endif
	...
	{
		m_Pclist_x =
			make_clist(m_resample_src_x, m_resample_dst_x, m_boundary_op, func, support, filter_x_scale, src_x_ofs);
		if (!m_Pclist_x)
		{
			m_status = STATUS_OUT_OF_MEMORY;
			return;
	...
	{
		m_Pclist_y =
			make_clist(m_resample_src_y, m_resample_dst_y, m_boundary_op, func, support, filter_y_scale, src_y_ofs);
		if (!m_Pclist_y)
		{
			m_status = STATUS_OUT_OF_MEMORY;
			return;
