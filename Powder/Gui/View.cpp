	...
	void View::BeginTextbox(ComponentKey key, std::string &str, TextboxFlags textboxFlags, Rgba8 color)
	{
		...
		if (HasInputFocus())
		{
			...
			while (inputFocus->events.size())
			{
				...
				textContext.selectionBegin = std::clamp(textContext.selectionBegin, 0, textContext.selectionMax);
				textContext.selectionEnd = std::clamp(textContext.selectionEnd, 0, textContext.selectionMax);
				auto lo = std::min(textContext.selectionBegin, textContext.selectionEnd);
				auto hi = std::max(textContext.selectionBegin, textContext.selectionEnd);
				auto insert = [&](TextWrapper::ClearIndex begin, std::string_view istr) {
					// TODO-REDO_UI: clear istr
				...
				auto copy = [&]() {
					GetHost()->CopyTextToClipboard(str.substr(lo, hi - lo));
				};
				auto cut = [&]() {
					copy();
					remove(lo, hi);
				...
				auto nextClear = [&](int32_t direction) {
					return std::clamp(textContext.selectionEnd + direction, 0, textContext.selectionMax);
				};
				...
				if (keyDown && !keyDown->alt)
				{
					...
					case SDLK_DOWN:
						...
						}
						{
							...
							if (textContext.selectionEnd == 0 || textContext.selectionEnd == textContext.selectionMax)
							{
