...
namespace Powder::Gui
{
	namespace
	{
		constexpr Point windowSize = { 629, 424 };
		constexpr int32_t integerScale = 2;
	...
	void Host::CreateShapedTextTexture(ShapedText &shapedText)
	{
		...
		for (auto it = Utf8Begin(text), end = Utf8End(text); it != end && regionIt != regionEnd; ++it)
		{
			...
			case '\x10':
				{
					auto offx = int32_t((*++it).codePoint);
					auto offy = int32_t((*++it).codePoint);
					if (offx >= 0x80) offx -= 0x100;
					if (offy >= 0x80) offy -= 0x100;
	...
	{
		fontData = Bz2Decompress(std::span(reinterpret_cast<const char *>(Powder::Resource::FontBz2.data()), Powder::Resource::FontBz2.size()), std::nullopt);
	...
	void Host::Close()
	{
		if (viewStackHolder)
		{
			viewStackHolder->SetRendererUp(false);
		}
	...
	{
		Close();
		Uint32 windowFlags = 0; // or SDL_WINDOW_RESIZABLE
		sdlWindow = SdlAssertPtr(SDL_CreateWindow("", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, integerScale * windowSize.X, integerScale * windowSize.Y, windowFlags));
		SdlAssertTrue(SDL_SetHintWithPriority(SDL_HINT_MOUSE_FOCUS_CLICKTHROUGH, "1", SDL_HINT_OVERRIDE));
		SdlAssertTrue(SDL_SetHintWithPriority(SDL_HINT_RENDER_SCALE_QUALITY, "nearest", SDL_HINT_OVERRIDE)); // or "linear"
		...
		SdlAssertZero(SDL_RenderSetLogicalSize(sdlRenderer, windowSize.X, windowSize.Y));
		SdlAssertZero(SDL_RenderSetIntegerScale(sdlRenderer, SDL_FALSE)); // or SDL_TRUE
	}
	...
	void Host::HandleFrame()
	{
		...
		while (repeatCount < maxFrameRepeatCount)
		{
			...
			{
				auto currentViewStackHolder = viewStackHolder;
				internedTextStore.BeginFrame();
				shapedTextStore.BeginFrame();
				lastTick = Ticks(SDL_GetTicks64());
				if (currentViewStackHolder)
				{
					currentViewStackHolder->SetHost(this);
					currentViewStackHolder->SetRendererUp(true);
					currentViewStackHolder->viewStack->HandleFrame();
				}
				shapedTextStore.EndFrame();
				internedTextStore.EndFrame();
				// currentViewStackHolder goes out of scope here and potentially calls ~ViewStackHolder
			}
			if (currentWindowParameters != windowParameters)
			{
				Open();
			}
			else
		...
		Log("stats: ", internedTextStore.debugAllocCount, " ", internedTextStore.debugFreeCount, " ", internedTextStore.textData.size());
#endif
		if (explicitDelay)
		{
	...
	void Host::SetViewStack(std::shared_ptr<ViewStack> newViewStack)
	{
		viewStackHolder.reset();
		if (newViewStack)
		{
			viewStackHolder = std::make_shared<ViewStackHolder>(*this, newViewStack);
		}
	}
...
#if DebugGuiHost
			DrawRect({ pos, shapedText.wrapper.GetContentSize() }, 0x8000FF00_rgba);
#endif
		}
	}
	...
	Point Host::GetSize() const
	{
		return windowSize;
	}

	void Host::SetWindowParameters(WindowParameters newWindowParameters)
	{
		windowParameters = newWindowParameters;
	}

	Host::ViewStackHolder::ViewStackHolder(Host &newHost, std::shared_ptr<ViewStack> newViewStack) : host(newHost), viewStack(newViewStack)
	{
	}

	Host::ViewStackHolder::~ViewStackHolder()
	{
		SetRendererUp(false);
		SetHost(nullptr);
	}

	void Host::ViewStackHolder::SetHost(Host *newHost)
	{
		if (viewStack->GetHost() != newHost)
		{
			viewStack->SetHost(newHost);
		}
	}

	void Host::ViewStackHolder::SetRendererUp(bool newRendererUp)
	{
		if (viewStack->GetRendererUp() != newRendererUp)
		{
			viewStack->SetRendererUp(newRendererUp);
		}
	}
