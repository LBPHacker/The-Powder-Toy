...
namespace Powder::Activity
{
	...
	ElementSearch::ElementSearch(Game &newGame) : game(newGame)
	{
		InitActions();
		Update();
	}
	...
	void ElementSearch::Gui()
	{
		...
		BeginModal("elementsearch");
		{
			SetTextAlignment(Gui::Alignment::left, Gui::Alignment::center);
			BeginText("title", "\biElement search", Gui::View::TextFlags::none); // TODO-REDO_UI-TRANSLATE
			SetSize(21);
			SetTextPadding(4);
			EndText();
			Separator("titlesep");
			BeginVPanel("content"); // TODO-REDO_UI: scrolling
			{
				SetPadding(3);
				SetSpacing(3);
			...
			BeginHPanel("cancel");
			{
				...
				SetPadding(3);
				SetSpacing(3);
				if (Button("Cancel")) // TODO-REDO_UI-TRANSLATE
				{
					Exit();
				}
			}
