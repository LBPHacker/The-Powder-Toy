...
// #include "gui/game/tool/SampleTool.h" // TODO-REDO_UI
...
namespace Powder::Activity
{
	namespace
	{
		...
		std::unique_ptr<VideoBuffer> CreateDecoSlotBackground()
		{
			...
			{
				data[p.Y * Game::toolTextureDataSize.X + p.X] = (((p.X / patternSize + p.Y / patternSize) & 1) ? 0xFFFFFFFF_rgba : 0xFFC8C8C8_rgba).Pack();
			}
			return texture;
		}
	}
	...
	Game::Game()
	{
		...
		{
			auto data = reinterpret_cast<const char *>(Powder::Resource::Save1844659.data());
			auto size = Powder::Resource::Save1844659.size();
			save = LocalSave{
				"(none)",
				"Save1844659",
				std::nullopt,
				std::make_unique<GameSave>(std::vector(data, data + size)),
			};
			ReloadSim(true);
			SetSimPaused(false);
		}
	...
	void Game::InitActions()
	{
		...
		//  SDL_SCANCODE_Z: zoom
		//  SDL_SCANCODE_P: prop tool / screenshot
		//  SDL_SCANCODE_F2: prop tool / screenshot
		...
		{
			ActionKey actionKey = ByteString::Build(actionKeyPrefix, "_LOADWITHOUTPRESSURE");
		...
		{
			ActionKey actionKey = ByteString::Build(actionKeyPrefix, "_TOOLSTRENGTHMUL10");
			AddAction(actionKey, Action{ [this]() {
		...
		{
			ActionKey actionKey = ByteString::Build(actionKeyPrefix, "_TOOLSTRENGTHDIV10");
			AddAction(actionKey, Action{ [this]() {
	...
	void Game::ResetToolSlot(ToolSlotIndex toolSlotIndex)
	{
		...
		// case 2: tool = // TODO-REDO_UI: sample tool
	...
	void Game::GetBasicSampleText(std::vector<std::string> &lines, std::optional<int> &wavelengthGfx, const SimulationSample &sample)
	{
		...
		if (type)
		{
			...
			if (debugHud)
			{
				...
				format::RenderTemperature(sb, sample.particle.temp, int32_t(temperatureScale));
	...
	void Game::GetDebugSampleText(std::vector<std::string> &lines, const SimulationSample &sample)
	{
		...
		if (GetAmbientHeat())
		{
			...
			StringBuilder sb;
			sb << Format::Precision(2);
			format::RenderTemperature(sb, sample.AirTemperature, int32_t(temperatureScale));
			sampleInfo << sb.Build().ToUtf8();
		}
		lines.push_back(sampleInfo.Build());
	}
	...
	void Game::GetFpsLines(std::vector<std::string> &lines, const SimulationSample &sample)
	{
		     ...
		     std::holds_alternative<HdispLimitAuto>(rendererSettings.wantHdispLimitMax)) && rendererStats.hdispLimitValid)
		{
			...
			sb << Format::Precision(2);
			sb << " [TEMP L:";
			format::RenderTemperature(sb, rendererStats.hdispLimitMin, int32_t(temperatureScale));
			sb << " H:";
			format::RenderTemperature(sb, rendererStats.hdispLimitMax, int32_t(temperatureScale));
			sb << "]";
	...
	void Game::GuiTools()
	{
		...
		if (activeMenuSection == SC_DECO)
		{
			...
			BeginHPanel("colorpicker");
			{
				...
				BeginComponent(0);
				SetHandleButtons(true);
				SetSize(toolTextureDataSize.Y + 4);
				// .Y below is correct, do not fix
				SetSizeSecondary(toolTextureDataSize.Y + 4);
				auto rr = GetRect();
				decoRect(rr.Inset(2), decoColor);
				g.DrawRect(rr.Inset(1), 0xFFFFFFFF_rgba);
				if (IsClicked())
				{
		...
		for (auto &info : tools)
		{
			...
			{
				QueueToolTip(info->tool->Description.ToUtf8(), r.pos + r.size - Pos2(12, 12), Gui::Alignment::right, Gui::Alignment::bottom); // TODO-REDO_UI-TRANSLATE
		...
		if (overflow)
		{
			...
			g.FillRect({ Pos2(scrollLeft + barOffset, r.pos.Y + r.size.Y - 2), Pos2(barSize, 2) }, 0xFFC8C8C8_rgba);
		}
		EndPanel();
		EndPanel();
	}
	...
	void Game::GuiQuickOptions()
	{
		...
		if (Button("P", 15, GetSandEffect() ? ButtonFlags::stuck : ButtonFlags::none))
		{
			ToggleSandEffectAction();
		}
		if (Button("G", 15, GetAmbientHeat() ? ButtonFlags::stuck : ButtonFlags::none))
		{
			ToggleAmbientHeatAction();
		}
		if (Button("D", 15, GetDrawDeco() ? ButtonFlags::stuck : ButtonFlags::none))
		{
			ToggleDrawDecoAction();
		}
		if (Button("N", 15, GetDrawGravity() ? ButtonFlags::stuck : ButtonFlags::none))
		{
			ToggleDrawGravityAction();
		}
		if (Button("A", 15, GetNewtonianGravity() ? ButtonFlags::stuck : ButtonFlags::none))
		{
	...
	void Game::GuiBottom()
	{
		...
		SetSpacing(1);
		{
			...
			SetTextAlignment(Gui::Alignment::left, Gui::Alignment::center);
			EndButton();
			Button(Gui::iconCheckmark, 15);
			Button(Gui::iconColors1, 15);
	...
	{
		while (logEntries.size() && !logEntries.front().alpha.GetValue())
		{
...
#if DebugGuiView
		if (event.type == SDL_KEYDOWN && event.key.keysym.scancode == SDL_SCANCODE_KP_MULTIPLY)
		{
			shouldDoDebugDump = true;
		}
#endif
		if (event.type == SDL_KEYDOWN && event.key.keysym.scancode == SDL_SCANCODE_KP_DIVIDE)
		{
			static int32_t testValue = 0;
			testValue += 1;
			GetHost()->SetWindowParameters({ testValue });
		}
	...
	void Game::BeginBrush(ToolSlotIndex toolSlotIndex, DrawMode drawMode)
	{
		...
		if (m && RES.OriginRect().Contains(*m))
		{
			...
			if (tool && brush)
			{
				...
				{
				case DrawMode::free:
					tool->DrawLine(sim, *brush, *m, *m, false);
					break;
	...
	void Game::VisualLog(std::string message)
	{
		...
		logEntries.push_back(LogEntry{ { Gui::Animation<int32_t>::LinearProfile{ 170 }, 0, 600 }, std::move(message) });
	...
	{
		auto &g = *view.GetViewStack()->GetHost();
		...
		else
		{
			...
			view.SetPadding(4); // Triggers the weird negative alignment behaviour in View::BeginText, see TruncateDiv explanation there.
			view.EndText();
		}
		bool selected = false;
		for (int32_t i = 0; i < toolSlotCount; ++i)
